var SeekmodoSuggest=(function(exports){'use strict';var k=class extends Error{status;body;tool;constructor(n,e,t,r){super(n),this.name="SeekmodoError",this.status=e,this.body=t,this.tool=r;}},N=class extends k{constructor(n,e,t){super(`Seekmodo auth failed (HTTP ${n})`,n,e,t),this.name="SeekmodoAuthError";}},ge=class extends k{code;bucket;limit;used;constructor(n,e){super("Seekmodo over quota (HTTP 402)",402,n,e),this.name="SeekmodoQuotaError";let t=n??{};this.code=t.code??"over_quota",this.bucket=t.bucket,this.limit=t.limit,this.used=t.used;}},me=class extends k{constructor(n,e,t){super(`Seekmodo server error (HTTP ${n})`,n,e,t),this.name="SeekmodoServerError";}},fe=class extends k{constructor(n,e,t){super(`Seekmodo request rejected (HTTP ${n})`,n,e,t),this.name="SeekmodoRequestError";}},K=class extends k{constructor(n,e){super(`Seekmodo network failure${n instanceof Error?`: ${n.message}`:""}`,0,n,e),this.name="SeekmodoNetworkError";}};function L(n,e){if(n instanceof K)return L(n.body);if(n instanceof TypeError){let t=n.message.toLowerCase();return t.includes("failed to fetch")||t.includes("networkerror")||t.includes("network request failed")||t.includes("load failed")}if(n instanceof Error){let t=n.message.toLowerCase();return t.includes("cors")||t.includes("access-control-allow-origin")||t.includes("cross-origin")}return  false}var be="https://gateway.seekmodo.com",ve=8e3,we=class{config;cachedToken=null;constructor(n){if(!n.tenantId)throw new Error("Seekmodo SDK: tenantId is required");if(typeof n.getToken!="function")throw new Error("Seekmodo SDK: getToken callback is required");this.config={tenantId:n.tenantId,getToken:n.getToken,baseUrl:(n.baseUrl??be).replace(/\/+$/,""),fetch:n.fetch??globalThis.fetch.bind(globalThis),timeoutMs:n.timeoutMs??ve,signal:n.signal,onError:n.onError,getRegion:n.getRegion};}clearTokenCache(){this.cachedToken=null;}async call(n,e,t={}){try{return await this.callOnce(n,e,t,!1)}catch(r){if(r instanceof N){this.clearTokenCache();try{return await this.callOnce(n,e,t,!0)}catch(o){throw this.config.onError?.(o,{tool:n}),o}}throw this.config.onError?.(r,{tool:n}),r}}async callOnce(n,e,t,r){let o=await this.resolveToken(r),i=`${this.config.baseUrl}/v1/${encodeURIComponent(n)}`,s=new AbortController,d=t.timeoutMs??this.config.timeoutMs,c=setTimeout(()=>s.abort(),d),u=()=>s.abort();this.config.signal?.addEventListener("abort",u,{once:true}),t.signal?.addEventListener("abort",u,{once:true});let h={"Content-Type":"application/json",Authorization:`Bearer ${o}`,"X-Seekmodo-Tenant":this.config.tenantId,"X-Seekmodo-SDK":"@seekmodo/sdk@0.1.0"};if(this.config.getRegion)try{let p=await this.config.getRegion();typeof p=="string"&&p.length>0&&(h["Seekmodo-Region"]=p);}catch{}let l;try{l=await this.config.fetch(i,{method:"POST",headers:h,body:JSON.stringify(e),signal:s.signal});}catch(p){throw new K(p,n)}finally{clearTimeout(c),this.config.signal?.removeEventListener("abort",u),t.signal?.removeEventListener("abort",u);}let g=await l.text(),m=g?ye(g):null;if(l.status===401||l.status===403)throw new N(l.status,m,n);if(l.status===402)throw new ge(m,n);if(l.status>=500)throw new me(l.status,m,n);if(!l.ok)throw new fe(l.status,m,n);return m}async resolveToken(n){let e=Date.now();if(!n&&this.cachedToken&&this.cachedToken.expiresAt-1e4>e)return this.cachedToken.token;let t=await this.config.getToken();if(typeof t=="string")return this.cachedToken={token:t,expiresAt:e+6e4},t;if(t&&typeof t=="object"&&typeof t.token=="string"&&typeof t.expiresAt=="number")return this.cachedToken={token:t.token,expiresAt:t.expiresAt},t.token;throw new Error("Seekmodo SDK: getToken must return a string or { token, expiresAt }")}};function ye(n){try{return JSON.parse(n)}catch{return n}}var q=class{transport;recommend;bundle;constructor(n){this.transport=new we(n),this.recommend={related:(e,t)=>this.transport.call("recommend.related",{...e},t??{}),alsoBought:(e,t)=>this.transport.call("recommend.also_bought",{...e},t??{}),alsoViewed:(e,t)=>this.transport.call("recommend.also_viewed",{...e},t??{}),trending:(e,t)=>this.transport.call("recommend.trending",{...e},t??{})},this.bundle={suggest:(e,t)=>this.transport.call("bundle.suggest",{...e},t??{})};}search(n,e){return this.transport.call("search",{...n},e??{})}suggest(n,e){return this.transport.call("suggest",{...n},e??{})}searchByImage(n,e){return this.transport.call("search.byImage",{...n},e??{})}chat(n,e){return this.transport.call("chat",{...n},e??{})}ask(n,e){return this.transport.call("ask",{...n},e??{})}catalogGet(n,e){return this.transport.call("catalog.get",{...n},e??{})}event(n,e){return this.transport.call("events",{...n},e??{})}};var _=null,b=null;function x(n){if(typeof document>"u")return null;let t=document.head?.querySelector(`meta[name="${n}"]`)?.getAttribute("content");return t&&t.length>0?t:null}function V(){return _!==null||(_=ke()),_}async function ke(){let n=x("seekmodo:tenant");if(!n)throw new Error('@seekmodo/web-components: <meta name="seekmodo:tenant"> is required');let e=x("seekmodo:token"),t=x("seekmodo:refresh");if(!e&&!t)throw new Error('@seekmodo/web-components: either <meta name="seekmodo:token"> or <meta name="seekmodo:refresh"> must be set');e&&(b={token:e,expiresAt:Date.now()+3e4});let r=x("seekmodo:gateway")??void 0;return new q({tenantId:n,baseUrl:r,getRegion:()=>Le(),getToken:async()=>{let o=Date.now();if(b&&b.expiresAt-1e4>o)return {token:b.token,expiresAt:b.expiresAt};if(!t){if(b)return {token:b.token,expiresAt:b.expiresAt};throw new Error("seekmodo:refresh meta missing; no way to refresh token")}let i=await fetch(t,{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/json"}});if(!i.ok)throw new Error(`seekmodo:refresh route returned HTTP ${i.status}`);let s=await i.json();if(!s.token||typeof s.expires_at!="number")throw new Error("seekmodo:refresh route returned a malformed envelope");return b={token:s.token,expiresAt:s.expires_at*1e3},{token:b.token,expiresAt:b.expiresAt}}})}var Ee="seekmodo_region";function Se(n){if(typeof n!="string")return null;let e=n.trim().toLowerCase();return /^[a-z0-9][a-z0-9_-]{1,63}$/.test(e)?e:null}function Le(){if(typeof document>"u")return null;let n=document.cookie??"";if(n.length===0)return null;let e=Ee.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),t=new RegExp(`(?:^|; )${e}=([^;]+)`).exec(n);if(!t)return null;try{return Se(decodeURIComponent(t[1]))}catch{return null}}var R=class extends HTMLElement{root;rafId=null;constructor(){super(),this.root=this.attachShadow({mode:"open"});}scheduleRender(){this.rafId===null&&(this.rafId=requestAnimationFrame(()=>{this.rafId=null;try{this.render();}catch(e){console.warn("[seekmodo] render failure",e);try{this.renderError("internal_error");}catch{this.root.innerHTML="";}}}));}async getClient(){return V()}renderError(e){this.root.innerHTML="";}disconnectedCallback(){this.rafId!==null&&(cancelAnimationFrame(this.rafId),this.rafId=null);}};function a(n,e,t){let r=document.createElement(n);if(e){for(let[o,i]of Object.entries(e))if(!(i==null||i===false))if(o==="class")r.className=String(i);else if(o==="part")r.setAttribute("part",String(i));else if(o==="text")r.textContent=String(i);else if(o==="html")r.innerHTML=String(i);else if(o==="attrs"&&typeof i=="object"&&i!==null)for(let[s,d]of Object.entries(i))r.setAttribute(s,d);else r.setAttribute(o,String(i));}return r}function j(n,e){let t=null;return (...r)=>{t!==null&&clearTimeout(t),t=setTimeout(()=>n(...r),e);}}function y(n,e,t){n.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:true,composed:true}));}var A="Search suggestions couldn't load because this site is blocked from reaching Seekmodo (CORS). Ask your store administrator to allowlist this domain on the Seekmodo gateway, or enable the connector's same-origin suggest proxy.";function G(n,e,t){let r=document.createElement("style");r.textContent=e;let o=a("div",{class:"wrap seekmodo-cors-blocked",part:"wrap cors-blocked",attrs:{role:"status"}});o.append(a("div",{class:"cors-notice",part:"cors-notice",text:t?.message??A})),n.replaceChildren(r,o);}var Q="seekmodo-cors-notice";function W(n,e){if(!n||typeof document>"u")return;let t=n.closest(".search-form")??n.parentNode;if(!t)return;t.style.position=t.style.position||"relative";let r=t.querySelector(`.${Q}`);if(!r){r=document.createElement("div"),r.className=Q,r.setAttribute("role","status"),r.style.cssText=["position:absolute","top:100%","left:0","right:0","z-index:10050","display:none","background:#fff8e6","border:1px solid #f0c040","border-top:none","padding:8px 12px","font-size:13px","line-height:1.4","color:#5c4a00","box-shadow:0 4px 12px rgba(0,0,0,.08)"].join(";"),t.appendChild(r);let o=()=>{if(!r)return;let i=(n.value||"").trim();r.style.display=i.length>=2?"block":"none";};n.addEventListener("input",o),n.addEventListener("focus",o);}r.textContent=e??A;}function Y(){typeof window>"u"||(window.seekmodoShowCorsNotice=W,window.seekmodoScriptLoadFailed=(n,e)=>{let t=n??document.querySelectorAll('input[data-seekmodo-suggest],input[data-seekmodo-typeahead],input[name="s"],input[name="keyword"],input[name="search_query"],input[name="q"],input[type="search"]');for(let r=0;r<t.length;r++){let o=t[r];o instanceof HTMLInputElement&&W(o,e);}});}function z(n){for(let e of ["image_url","image","thumbnail_url"]){let t=n[e];if(typeof t=="string"&&t.trim()!=="")return t.trim()}}function J(n,e,t){try{let r=typeof window<"u"?window.location.origin:"http://localhost",o=new URL(n,r);return o.searchParams.set(e,t),/^https?:\/\//i.test(n)?o.toString():`${o.pathname}${o.search}${o.hash}`}catch{let r=n.includes("?")?"&":"?";return `${n}${r}${encodeURIComponent(e)}=${encodeURIComponent(t)}`}}function _e(n,e){let t=(n??"").toLowerCase(),r=(e??"").toLowerCase();if(r.startsWith("http"))try{r=new URL(r).pathname;}catch{r="";}return t==="page"&&/\/tools\/[^/]+\/?$/.test(r)?"Tool":t==="page"&&/\/tools\/?$/.test(r)?"Tools":t==="page"?"Page":t==="post"?"Article":""}function X(n){let e=typeof n.post_type=="string"?n.post_type.toLowerCase():"",t=typeof n.url=="string"?n.url:typeof n.permalink=="string"?n.permalink:"";return {postType:e,label:_e(e,t)}}var U="split-rail",xe=15;function T(n,e){let r={"split-rail":5,"command-bar":5,"cinema-grid":6,magazine:6,classic:5}[n]??5,o=Math.max(e,r*3);return Math.min(xe,o)}var ne=`
...
`,Z=.15,ee=.85,Re=.28,te="seekmodo:split-rail-mobile-ratio-v3",Ae="(max-width: 900px)",Te=1-Re;function Ce(){return a("div",{class:"split-divider",part:"split-divider",html:'<svg class="split-divider-icon" viewBox="0 0 36 12" aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg"><rect x="6" y="1" width="24" height="2" rx="1" fill="currentColor"/><rect x="6" y="5" width="24" height="2" rx="1" fill="currentColor"/><rect x="6" y="9" width="24" height="2" rx="1" fill="currentColor"/></svg>'})}function oe(n){let e=n.querySelector(".split-divider");if(!e)return ()=>{};let t=window.matchMedia(Ae),r=Te;try{let m=sessionStorage.getItem(te);if(m){let p=parseFloat(m);p>=Z&&p<=ee&&(r=p);}}catch{}let o=m=>{r=Math.min(ee,Math.max(Z,m)),n.style.setProperty("--split-rail-bottom-grow",String(r)),n.style.setProperty("--split-rail-top-grow",String(1-r));},i=()=>{n.style.removeProperty("--split-rail-top-grow"),n.style.removeProperty("--split-rail-bottom-grow");},s=()=>{t.matches?o(r):i();};s();let d=false,c=0,u=r,h=m=>{!t.matches||m.button!==0||(d=true,c=m.clientY,u=r,e.classList.add("is-dragging"),e.setPointerCapture(m.pointerId),m.preventDefault());},l=m=>{if(!d)return;let f=n.getBoundingClientRect().height-e.offsetHeight;f<=0||o(u+(m.clientY-c)/f);},g=m=>{if(d){d=false,e.classList.remove("is-dragging");try{e.releasePointerCapture(m.pointerId);}catch{}try{sessionStorage.setItem(te,String(r));}catch{}}};return e.setAttribute("role","separator"),e.setAttribute("aria-orientation","horizontal"),e.setAttribute("aria-label","Resize product and suggestion panels"),e.setAttribute("tabindex","0"),e.addEventListener("pointerdown",h),e.addEventListener("pointermove",l),e.addEventListener("pointerup",g),e.addEventListener("pointercancel",g),t.addEventListener("change",s),()=>{e.removeEventListener("pointerdown",h),e.removeEventListener("pointermove",l),e.removeEventListener("pointerup",g),e.removeEventListener("pointercancel",g),t.removeEventListener("change",s),e.classList.remove("is-dragging"),i();}}var He="https://seekmodo.com/email-assets/seekmodo-lockup.png";function ie(n){return String(n.name??n.title??"").trim()}function se(n,e,t){e.productTitleTooltip&&t&&n.setAttribute("title",t);}function ae(n,e){return n.productTitleTooltip&&e?e:""}function le(n,e,t){let r={block:"products",data:e,value:String(e.name??e.title??""),id:e.id!==void 0?String(e.id):void 0},o=n.rows.length;return n.rows.push(r),o}function Me(n,e,t){let r=a("div",{class:"thumb-frame",part:"thumb-frame"}),o=z(n);return o?r.append(a("img",{class:"thumb",part:"thumb",attrs:{src:o,"data-src":o,alt:ae(e,t),loading:"eager",decoding:"async"}})):r.append(a("div",{class:"thumb-empty",part:"thumb thumb--empty"})),r}function Ie(n,e,t){let r=z(n);return r?a("img",{class:"thumb",part:"thumb",attrs:{src:r,"data-src":r,alt:ae(e,t),loading:"eager",decoding:"async"}}):a("div",{class:"thumb-empty",part:"thumb thumb--empty"})}function de(n,e,t="card-price"){if(n.price===void 0||n.price===null)return null;let r=a("div",{class:t,part:"price"});return n.on_sale&&typeof n.sale_price=="number"?(r.append(a("del",{text:e.formatPrice(n.price,n.currency)})),r.append(document.createTextNode(e.formatPrice(n.sale_price,n.currency)))):r.append(document.createTextNode(e.formatPrice(n.price,n.currency))),r}function ce(n,e,t){n.classList.add("row"),n.setAttribute("data-seekmodo-surface","suggest"),n.setAttribute("data-seekmodo-block","products"),n.setAttribute("data-seekmodo-pos",String(t));let r=e.rows[t];r?.id&&n.setAttribute("data-seekmodo-id",r.id),n.addEventListener("click",()=>e.onRowClick(t));}function Pe(n,e,t,r=false){let o=le(n,e),i=ie(e),s=a("button",{class:"product-card",part:"row",type:"button"});s.append(Me(e,n,i));let d=a("span",{class:"card-title",part:"name",text:i});se(d,n,i),s.append(d);let c=de(e,n,"card-price");return c&&s.append(c),ce(s,n,o),s}function ze(n,e,t,r){let o=le(n,e),i=ie(e),s=a("button",{class:"hero-card",part:"row",type:"button"});r&&s.append(a("span",{class:"hero-badge",text:r})),s.append(Ie(e,n,i));let d=a("div",{class:"hero-info"}),c=a("span",{class:"card-title",part:"name",text:i});se(c,n,i),d.append(c);let u=de(e,n);return u&&d.append(u),s.append(d),ce(s,n,o),s}function D(n){let e=n.res.meta?.total??0,t=a("div",{class:"meta-bar",part:"meta-bar"}),r=a("div");r.append(a("span",{class:"count",text:`${e} results for `})),r.append(a("span",{class:"query",text:`"${n.lastQuery}"`})),t.append(r);let o=a("a",{class:"view-all view-all-cta",part:"view-all",attrs:{href:n.viewAllHref},text:n.label("view_all").replace("{total}",String(e))});return o.addEventListener("click",i=>{i.preventDefault(),n.onViewAll();}),t.append(o),t}function De(n){let e=n.res.did_you_mean;if(!e)return null;let t=a("div",{class:"did-you-mean-bar",part:"did-you-mean"});t.append(document.createTextNode(`Showing results for "${n.lastQuery}". Search instead for `));let r=a("button",{class:"swap",type:"button",text:e}),o=n.rows.length;return n.rows.push({block:"did_you_mean",data:{value:e},value:e}),r.addEventListener("click",()=>n.onRowClick(o)),t.append(r),t.append(document.createTextNode("?")),t}function $(n,e){let t=a("div",{class:"chip-row filter-bar",part:"filter-bar"});return t.append(a("span",{class:"filter-label",text:"Category"})),e.forEach((r,o)=>{let i=a("button",{class:`chip${o===0?" active":""}`,type:"button",text:`${r.name}${typeof r.count=="number"?` ${r.count}`:""}`}),s=n.rows.length;n.rows.push({block:"categories",data:r,value:String(r.name??"")}),i.addEventListener("click",()=>n.onRowClick(s)),t.append(i);}),t}function re(n,e,t="Try"){let r=a("div",{class:"chip-row",part:"filter-bar"});return r.append(a("span",{class:"filter-label",text:t})),e.forEach(o=>{let i=a("button",{class:"chip",type:"button",text:o.keyword}),s=n.rows.length;n.rows.push({block:"keywords",data:o,value:o.keyword}),i.addEventListener("click",()=>n.onRowClick(s)),r.append(i);}),r}function O(n,e,t,r,o){let i=n.rows.length;n.rows.push({block:e,data:t,value:r});let s=a("button",{class:"row",part:"row",type:"button"});return s.append(a("div",{class:"name",part:"name",text:r})),o&&s.append(a("span",{class:"badge",part:"badge",text:o})),s.setAttribute("data-seekmodo-surface","suggest"),s.setAttribute("data-seekmodo-block",e),s.setAttribute("data-seekmodo-pos",String(i)),s.addEventListener("click",()=>n.onRowClick(i)),s}function F(n,e){if(e.length===0)return null;let t=a("div",{class:"rail-section"});return t.append(a("div",{class:"group-title",part:"group-title",text:n})),e.forEach(r=>t.append(r)),t}function $e(n){if(!n.showBranding)return null;let e=a("a",{class:"brand-footer",part:"brand-footer",attrs:{href:n.brandUrl,target:"_blank",rel:"noopener noreferrer"}});return e.append(a("span",{class:"brand-by",text:"Powered by "})),e.append(a("img",{class:"brand-logo",part:"brand-logo",attrs:{src:n.brandLogoUrl||He,alt:"Seekmodo",height:"16"}})),e}function E(n,e,t){let r=a("div",{class:`product-grid cols-${t}`,part:"product-grid"});return e.forEach((o,i)=>r.append(Pe(n,o,i,true))),r}function ue(n,e){let t=["wrap","wide"];n==="split-rail"&&t.push("split-rail-panel"),e.splitMobileResize&&t.push("split-rail-mobile"),e.productTitleTooltip&&t.push("product-title-tooltip");let r=a("div",{class:t.join(" "),part:"wrap"});r.append(a("slot",{attrs:{name:"header"}}));let o=(e.res.products??[]).slice(0,T(n,e.limit)),i=(e.res.keywords??[]).slice(0,e.limit),s=(e.res.categories??[]).slice(0,e.limit),d=(e.res.recent??[]).slice(0,5);(e.res.trending??[]).slice(0,5);if(n==="split-rail"){r.append(D(e));let h=e.splitMobileResize?"split-body split-body--mobile-resize":"split-body",l=a("div",{class:h}),g=a("aside",{class:"rail",part:"rail"}),m=i.map(v=>O(e,"keywords",v,v.keyword)),p=F(e.label("keywords"),m);p&&g.append(p);let f=d.map(v=>O(e,"recent",v,v.keyword)),w=F(e.label("recent"),f);w&&g.append(w);let P=s.map(v=>O(e,"categories",v,String(v.name),typeof v.count=="number"?String(v.count):void 0)),S=F(e.label("categories"),P);S&&g.append(S);let B=a("div",{class:"canvas"});B.append(E(e,o,5)),l.append(B),e.splitMobileResize&&l.append(Ce()),l.append(g),r.append(l);}else if(n==="cinema-grid"){let h=De(e);h&&r.append(h),r.append(D(e)),s.length&&r.append($(e,s)),i.length&&r.append(re(e,i));let l=a("div",{class:"canvas"});l.append(E(e,o,6)),r.append(l);}else if(n==="command-bar"){let h=e.res.meta?.total??0,l=a("div",{class:"command-header",part:"meta-bar"});l.append(a("div",{class:"query-display",text:`"${e.lastQuery}"`})),l.append(a("span",{class:"result-pill",text:`${h} products`}));let g=a("a",{class:"view-all-link",part:"view-all",attrs:{href:e.viewAllHref},text:"View all \u2192"});if(g.addEventListener("click",p=>{p.preventDefault(),e.onViewAll();}),l.append(g),r.append(l),e.res.did_you_mean){let p=a("div",{class:"chip-row"});p.append(a("span",{class:"filter-label",text:"Did you mean"}));let f=a("button",{class:"chip",type:"button",text:e.res.did_you_mean}),w=e.rows.length;e.rows.push({block:"did_you_mean",data:{value:e.res.did_you_mean},value:e.res.did_you_mean}),f.addEventListener("click",()=>e.onRowClick(w)),p.append(f),r.append(p);}i.length&&r.append(re(e,i,"Related")),s.length&&r.append($(e,s));let m=a("div",{class:"canvas"});m.append(E(e,o,5)),r.append(m);}else if(n==="magazine"){r.append(D(e)),s.length&&r.append($(e,s));let h=a("div",{class:"canvas"}),l=o.slice(0,3),g=o.slice(3);if(l.length){h.append(a("div",{class:"group-title",part:"group-title",text:"Best matches"}));let m=a("div",{class:"hero-row",part:"hero-row"});l.forEach((p,f)=>{m.append(ze(e,p,f,f===0?"Top match":void 0));}),h.append(m);}g.length?(h.append(a("div",{class:"group-title",part:"group-title",text:"More results"})),h.append(E(e,g,6))):l.length||h.append(E(e,o,6)),r.append(h);}let u=$e(e);return u&&r.append(u),r.append(a("slot",{attrs:{name:"footer"}})),r}function C(n){return n!=="classic"&&n!==""}var pe=["recent","did_you_mean","keywords","trending","products","redirects","categories"],Oe={recent:"Recently searched",trending:"Trending",keywords:"Suggestions",products:"Products",redirects:"Go to",categories:"Categories",did_you_mean:"Did you mean",view_all:"View all {total} results",empty:"No matches yet \u2014 keep typing.",cors_blocked:A},he=`
  :host {
