                ...
                if ($html !== '' && $html !== 'false'
                    ...
                    if ($parsed !== '') {
                        ...
                        if ($abs !== '') {
                            ...
                            // miss. On KIP demo, resized thumbs are often
                            // absent so zen_get_products_image() returns
                            // no_picture.gif even though products_image and
                            // /images/<file> exist — without this fallthrough
                            // hydration overwrites good Typesense thumbs.
                            if (stripos($abs, '/bmz_cache/') !== false
                                || stripos($abs, '/images/cache/') !== false
                                || preg_match('#\.image\.\d+x\d+\.(jpe?g|png|gif|webp)$#i', $abs) === 1
                                || preg_match('#(?:^|/)no_picture\.(?:gif|png|jpe?g|webp)$#i', $abs) === 1
                            ) {
                                $catalog = numinix_seekmodo_suggest_product_image_url_catalog($pid);
                                if ($catalog !== '') {
                                    return $catalog;
                                }
                                // Don't promote a placeholder over nothing —
                                // caller / gateway may still have a usable URL.
                                if (preg_match('#(?:^|/)no_picture\.(?:gif|png|jpe?g|webp)$#i', $abs) === 1) {
                                    return '';
                                }
                            }
    ...
    function numinix_seekmodo_suggest_product_prices(array $productIds): array
    {
        ...
        foreach ($productIds as $pid) {
            ...
            if (function_exists('zen_get_products_display_price')) {
                $display = trim(strip_tags((string) @zen_get_products_display_price($pid)));
            }
            $numeric = null;
...
if (!function_exists('_numinix_seekmodo_typeahead_attach_image_url')) {
    ...
    function _numinix_seekmodo_typeahead_attach_image_url(array &$item, int $pid, ?array $doc = null): void
    {
        ...
        // Gateway / stale index docs sometimes carry Zen's placeholder.
        // Prefer products_image from the live catalog instead.
        if ($imageUrl !== ''
            && preg_match('#(?:^|/)no_picture\.(?:gif|png|jpe?g|webp)$#i', $imageUrl) === 1
        ) {
            $imageUrl = '';
        }
        ...
        if (!isset($item['image']) && function_exists('zen_get_products_image')) {
            try {
                ...
                if ($html !== '' && $html !== 'false'
                    && preg_match('#(?:^|/)no_picture\.(?:gif|png|jpe?g|webp)#i', $html) !== 1
                ) {
                    $item['image'] = $html;
                }
            } catch (\Throwable $e) {
        ...
        if (isset($item['image']) && is_string($item['image'])
            ...
            $parsed = trim($m[2]);
            if ($parsed !== ''
                && preg_match('#(?:^|/)no_picture\.(?:gif|png|jpe?g|webp)$#i', $parsed) !== 1
            ) {
                if (preg_match('#^https?://#i', $parsed) === 1) {
