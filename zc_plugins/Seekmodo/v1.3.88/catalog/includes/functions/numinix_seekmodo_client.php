...
if (!function_exists('numinix_seekmodo_mode')) {
     ...
     *
     * v1.0.5 (W6b): when `NUMINIX_SEEKMODO_MODE` is empty / unset /
     * not one of the four known values, fall through to
     * `NUMINIX_SEEKMODO_DEFAULT_MODE` before defaulting to `'off'`.
     * This lets seekmodo.com manage a tenant-wide default that
     * survives a manual `MODE` row clear without flipping the
     * storefront to bypass mode.
     */
    function numinix_seekmodo_mode(): string
    {
        $valid = ['off', 'shadow', 'enforce', 'active'];
        $m = strtolower(_numinix_seekmodo_cfg('NUMINIX_SEEKMODO_MODE', ''));
        if (in_array($m, $valid, true)) {
            return $m;
        }
        $fallback = strtolower(_numinix_seekmodo_cfg('NUMINIX_SEEKMODO_DEFAULT_MODE', ''));
...
if (!function_exists('numinix_seekmodo_enabled')) {
    ...
    function numinix_seekmodo_enabled(): bool
    {
        ...
        // Sprint 12 — tenant domain lock as a global gate. Each
        // hot-path entry (run_search / run_typeahead / mirror_click /
        // run_bulk_upsert) already checks this directly so the
        // breaker / FSM never sees lockout traffic; this is the
        // safety net for any future caller that forgets.
        if (
...
if (!function_exists('numinix_seekmodo_suggest')) {
     ...
     *
     * Thin wrapper over {@see Client::suggest()}. Returns the decoded
     * gateway response on success, null on any failure (auth /
     * transport / circuit-open / non-2xx). The caller (typically
     * {@see numinix_seekmodo_run_typeahead()}) MUST fall back to
     * the storefront's existing typeahead path (direct Typesense /
     * native LIKE) on null — typeahead UX never breaks because the
     * gateway is unreachable.
     *
     * Contract: only called when MODE != off. Mode-aware behavior
     * (shadow returns null after recording the impression, enforce
     * returns the items) lives in `numinix_seekmodo_run_typeahead`.
     *
     * @param array<string,mixed> $params {q, limit, session_id, ua, ip, ...}
     * @return array<string,mixed>|null
     */
    function numinix_seekmodo_suggest(array $params): ?array
    {
        $client = _numinix_seekmodo_client();
        if ($client === null) {
            return null;
...
if (!function_exists('numinix_seekmodo_recommend')) {
     ...
     *
     * Thin wrapper over {@see Client::recommend()}. Returns the
     * decoded gateway envelope on success, null on any failure
     * (disabled / locked-out / auth / transport / circuit-open /
     * non-2xx / 402 feature-not-in-plan).
     *
     * The caller (typically the AJAX endpoint backing a
     * `<div data-seekmodo-placement="...">` placeholder) MUST treat
     * null as "render nothing in this placement". The connector
     * never substitutes a fallback recommendation set — when the
     * gateway can't serve the call, the placement renders empty so
     * the storefront's static layout doesn't shift.
     *
     * Contract: only called when MODE != off. Domain-lock-out is
     * already enforced inside {@see Client::call()} so this helper
     * doesn't double-check.
     *
     * @param string $algorithm One of 'related' | 'also_bought' |
     *                          'also_viewed' | 'trending' |
     *                          'bundle.suggest'.
     * @param array<string,mixed> $params Tool-specific params.
     * @return array<string,mixed>|null
     */
    function numinix_seekmodo_recommend(string $algorithm, array $params): ?array
    {
        ...
        return $client->recommend($algorithm, $params);
    }
}
