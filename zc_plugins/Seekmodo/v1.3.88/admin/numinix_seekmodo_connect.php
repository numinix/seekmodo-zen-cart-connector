...
$billingEnvelope = class_exists('Numinix\\Seekmodo\\Client')
    ? \Numinix\Seekmodo\Client::readOverQuotaEnvelope()
    : null;
$billingReason = ($isPaired && class_exists('Numinix\\Seekmodo\\BillingAdminNotice'))
    ? \Numinix\Seekmodo\BillingAdminNotice::resolveReasonCode($subState, $billingEnvelope)
    : null;
...
<div class="seekmodo-card">
  ...
  <?php if ($billingReason !== null): ?>
    <?php
      $billingSoft = \Numinix\Seekmodo\BillingAdminNotice::isSoftStyle($billingReason);
      $billingCss = $billingSoft ? 'msg-warn' : 'msg-paused';
      $billingText = \Numinix\Seekmodo\BillingAdminNotice::softCopy($billingReason);
      $billingTitle = $billingReason === 'trial_expired'
        ? 'Seekmodo trial ended'
        : ($billingReason === 'cancelled'
          ? 'Seekmodo account paused'
          : 'Seekmodo cloud quota reached');
    ?>
    <div class="msg <?= htmlspecialchars($billingCss, ENT_QUOTES, CHARSET) ?>">
      <strong><?= htmlspecialchars($billingTitle, ENT_QUOTES, CHARSET) ?></strong>
      <?= htmlspecialchars($billingText, ENT_QUOTES, CHARSET) ?>
  ...
  <?php elseif ($subscriptionCancelled): ?>
    <div class="msg msg-paused">
      ...
      store, so the storefront has fallen back to Enhanced Native.
...
systemctl restart <?= htmlspecialchars($fpmSvcA, ENT_QUOTES, CHARSET) ?></pre>
  ...
  <form method="post" action="<?= zen_href_link('numinix_seekmodo_connect.php') ?>" style="margin-top:8px;">
    ...
    <?php if ($isPaired): ?>
      ...
      <button type="submit" name="action" value="pair" class="btn" formtarget="_blank"<?= $sodiumOk ? '' : ' disabled title="PHP sodium extension is required — see warning above"' ?>>
        Re-pair
      </button>
      ...
      <?php endif; ?>
      <p class="smallText" style="margin-top:8px;">
        Re-pair opens seekmodo.com in a new tab. When pairing finishes, close that tab and return here.
      </p>
    <?php else: ?>
      <button type="submit" name="action" value="pair" class="btn" formtarget="_blank"<?= $sodiumOk ? '' : ' disabled title="PHP sodium extension is required — see warning above"' ?>>
        Connect to Seekmodo
      ...
      <p class="smallText" style="margin-top:8px;">
        Opens seekmodo.com in a new tab. When pairing finishes, close that tab and return here.
      </p>
    <?php endif; ?>
  </form>
</div>
