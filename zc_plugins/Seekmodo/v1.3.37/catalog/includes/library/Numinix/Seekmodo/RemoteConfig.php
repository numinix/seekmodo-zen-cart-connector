...
final class RemoteConfig
{
    ...
    private function writeThrough(array $row): void
    {
        ...
        $writes = [
            ...
            'NUMINIX_SEEKMODO_INDEXER_CRON_OFFSET_MIN' => isset($row['indexer_cron_offset_min'])
                ? (string) max(0, min(59, (int) $row['indexer_cron_offset_min'])) : null,
            ...
            // Category-name 302 on SERP (gateway migration 076). Default
            // true when the snapshot omits the field so older gateways
        ...
        try {
            if (class_exists(CronReconciler::class)) {
                (new CronReconciler())->reconcile();
            }
        } catch (\Throwable $e) {
