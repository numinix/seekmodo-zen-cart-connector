...
class Client
{
    ...
    // as a benign fallback rather than an error. Distinct from a
    // bare 4xx (`signature_mismatch`, `rate_limited`, malformed
    // request), which usually points at a real config bug and
    // should NOT silently degrade to native search.
    //
    ...
    private function call(string $path, array $body, int $timeoutMs): ?array
    {
        ...
        if ($status >= 400) {
            // v1.0.17 — peek at the body BEFORE logging so the
            // structured log event distinguishes a tenant-unavailable
            // degradation from a real config bug (signature_mismatch,
            // rate_limited, malformed request). Both still take the
            // same code path — return null and let the caller fall
            // through to native — but the log line carries
            // `fallback_reason` so admin observability can attribute
            // volume correctly.
            $errorCode = self::classifyTenantUnavailable($status, is_string($resp) ? $resp : null);
            $tenantUnavailable = $errorCode !== null;
            $this->log('warn', $tenantUnavailable ? 'tenant_unavailable' : 'caller_error', [
                'path' => $path,
                'status' => $status,
                'elapsed_ms' => $elapsedMs,
                'body_preview' => is_string($resp) ? substr($resp, 0, 200) : '',
                'error_code' => $errorCode,
                'fallback_reason' => $tenantUnavailable ? 'tenant_unavailable' : null,
            ]);
     ...
     *
     * Why parse the body at all: the gateway uses the SAME HTTP
     * status (403) for `tenant_paused` (operator pulled the plug —
     * fallback to native is correct) AND for `signature_mismatch`
     * / `rate_limited` (real client bugs — surface the failure to
     * the admin so the operator can fix the config). Without the
     * body peek we can't tell the two apart at classification time.
     *
