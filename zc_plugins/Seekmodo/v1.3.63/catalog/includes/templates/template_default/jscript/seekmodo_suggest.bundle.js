var SeekmodoSuggest=(function(exports){'use strict';var E=class extends Error{status;body;tool;constructor(n,e,t,r){super(n),this.name="SeekmodoError",this.status=e,this.body=t,this.tool=r;}},Q=class extends E{constructor(n,e,t){super(`Seekmodo auth failed (HTTP ${n})`,n,e,t),this.name="SeekmodoAuthError";}},B=class extends E{code;bucket;limit;used;constructor(n,e){super("Seekmodo over quota (HTTP 402)",402,n,e),this.name="SeekmodoQuotaError";let t=n??{};this.code=t.code??"over_quota",this.bucket=t.bucket,this.limit=t.limit,this.used=t.used;}},ye=class extends E{constructor(n,e,t){super(`Seekmodo server error (HTTP ${n})`,n,e,t),this.name="SeekmodoServerError";}},we=class extends E{constructor(n,e,t){super(`Seekmodo request rejected (HTTP ${n})`,n,e,t),this.name="SeekmodoRequestError";}},j=class extends E{constructor(n,e){super(`Seekmodo network failure${n instanceof Error?`: ${n.message}`:""}`,0,n,e),this.name="SeekmodoNetworkError";}};function R(n,e){if(n instanceof j)return R(n.body);if(n instanceof TypeError){let t=n.message.toLowerCase();return t.includes("failed to fetch")||t.includes("networkerror")||t.includes("network request failed")||t.includes("load failed")}if(n instanceof Error){let t=n.message.toLowerCase();return t.includes("cors")||t.includes("access-control-allow-origin")||t.includes("cross-origin")}return  false}var ke="https://gateway.seekmodo.com",_e=8e3,Ee=class{config;cachedToken=null;constructor(n){if(!n.tenantId)throw new Error("Seekmodo SDK: tenantId is required");if(typeof n.getToken!="function")throw new Error("Seekmodo SDK: getToken callback is required");this.config={tenantId:n.tenantId,getToken:n.getToken,baseUrl:(n.baseUrl??ke).replace(/\/+$/,""),fetch:n.fetch??globalThis.fetch.bind(globalThis),timeoutMs:n.timeoutMs??_e,signal:n.signal,onError:n.onError,getRegion:n.getRegion};}clearTokenCache(){this.cachedToken=null;}async call(n,e,t={}){try{return await this.callOnce(n,e,t,!1)}catch(r){if(r instanceof Q){this.clearTokenCache();try{return await this.callOnce(n,e,t,!0)}catch(o){throw this.config.onError?.(o,{tool:n}),o}}throw this.config.onError?.(r,{tool:n}),r}}async callOnce(n,e,t,r){let o=await this.resolveToken(r),i=`${this.config.baseUrl}/v1/${encodeURIComponent(n)}`,s=new AbortController,d=t.timeoutMs??this.config.timeoutMs,l=setTimeout(()=>s.abort(),d),h=()=>s.abort();this.config.signal?.addEventListener("abort",h,{once:true}),t.signal?.addEventListener("abort",h,{once:true});let m={"Content-Type":"application/json",Authorization:`Bearer ${o}`,"X-Seekmodo-Tenant":this.config.tenantId,"X-Seekmodo-SDK":"@seekmodo/sdk@0.1.0"};if(this.config.getRegion)try{let p=await this.config.getRegion();typeof p=="string"&&p.length>0&&(m["Seekmodo-Region"]=p);}catch{}let c;try{c=await this.config.fetch(i,{method:"POST",headers:m,body:JSON.stringify(e),signal:s.signal});}catch(p){throw new j(p,n)}finally{clearTimeout(l),this.config.signal?.removeEventListener("abort",h),t.signal?.removeEventListener("abort",h);}let u=await c.text(),g=u?Se(u):null;if(c.status===401||c.status===403)throw new Q(c.status,g,n);if(c.status===402)throw new B(g,n);if(c.status>=500)throw new ye(c.status,g,n);if(!c.ok)throw new we(c.status,g,n);return g}async resolveToken(n){let e=Date.now();if(!n&&this.cachedToken&&this.cachedToken.expiresAt-1e4>e)return this.cachedToken.token;let t=await this.config.getToken();if(typeof t=="string")return this.cachedToken={token:t,expiresAt:e+6e4},t;if(t&&typeof t=="object"&&typeof t.token=="string"&&typeof t.expiresAt=="number")return this.cachedToken={token:t.token,expiresAt:t.expiresAt},t.token;throw new Error("Seekmodo SDK: getToken must return a string or { token, expiresAt }")}};function Se(n){try{return JSON.parse(n)}catch{return n}}var G=class{transport;recommend;bundle;constructor(n){this.transport=new Ee(n),this.recommend={related:(e,t)=>this.transport.call("recommend.related",{...e},t??{}),alsoBought:(e,t)=>this.transport.call("recommend.also_bought",{...e},t??{}),alsoViewed:(e,t)=>this.transport.call("recommend.also_viewed",{...e},t??{}),trending:(e,t)=>this.transport.call("recommend.trending",{...e},t??{})},this.bundle={suggest:(e,t)=>this.transport.call("bundle.suggest",{...e},t??{})};}search(n,e){return this.transport.call("search",{...n},e??{})}suggest(n,e){return this.transport.call("suggest",{...n},e??{})}searchByImage(n,e){return this.transport.call("search.byImage",{...n},e??{})}chat(n,e){return this.transport.call("chat",{...n},e??{})}ask(n,e){return this.transport.call("ask",{...n},e??{})}event(n,e){return this.transport.call("events",{...n},e??{})}};var A=null,y=null;function T(n){if(typeof document>"u")return null;let t=document.head?.querySelector(`meta[name="${n}"]`)?.getAttribute("content");return t&&t.length>0?t:null}function W(){return A!==null||(A=xe()),A}async function xe(){let n=T("seekmodo:tenant");if(!n)throw new Error('@seekmodo/web-components: <meta name="seekmodo:tenant"> is required');let e=T("seekmodo:token"),t=T("seekmodo:refresh");if(!e&&!t)throw new Error('@seekmodo/web-components: either <meta name="seekmodo:token"> or <meta name="seekmodo:refresh"> must be set');e&&(y={token:e,expiresAt:Date.now()+3e4});let r=T("seekmodo:gateway")??void 0;return new G({tenantId:n,baseUrl:r,getRegion:()=>Ae(),getToken:async()=>{let o=Date.now();if(y&&y.expiresAt-1e4>o)return {token:y.token,expiresAt:y.expiresAt};if(!t){if(y)return {token:y.token,expiresAt:y.expiresAt};throw new Error("seekmodo:refresh meta missing; no way to refresh token")}let i=await fetch(t,{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/json"}});if(!i.ok)throw new Error(`seekmodo:refresh route returned HTTP ${i.status}`);let s=await i.json();if(!s.token||typeof s.expires_at!="number")throw new Error("seekmodo:refresh route returned a malformed envelope");return y={token:s.token,expiresAt:s.expires_at*1e3},{token:y.token,expiresAt:y.expiresAt}}})}var Le="seekmodo_region";function Re(n){if(typeof n!="string")return null;let e=n.trim().toLowerCase();return /^[a-z0-9][a-z0-9_-]{1,63}$/.test(e)?e:null}function Ae(){if(typeof document>"u")return null;let n=document.cookie??"";if(n.length===0)return null;let e=Le.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),t=new RegExp(`(?:^|; )${e}=([^;]+)`).exec(n);if(!t)return null;try{return Re(decodeURIComponent(t[1]))}catch{return null}}var C=class extends HTMLElement{root;rafId=null;constructor(){super(),this.root=this.attachShadow({mode:"open"});}scheduleRender(){this.rafId===null&&(this.rafId=requestAnimationFrame(()=>{this.rafId=null;try{this.render(),this.afterRender();}catch(e){console.warn("[seekmodo] render failure",e);try{this.renderError("internal_error");}catch{this.root.innerHTML="";}}}));}afterRender(){}async getClient(){return W()}renderError(e){this.root.innerHTML="";}disconnectedCallback(){this.rafId!==null&&(cancelAnimationFrame(this.rafId),this.rafId=null);}};function a(n,e,t){let r=document.createElement(n);if(e){for(let[o,i]of Object.entries(e))if(!(i==null||i===false))if(o==="class")r.className=String(i);else if(o==="part")r.setAttribute("part",String(i));else if(o==="text")r.textContent=String(i);else if(o==="html")r.innerHTML=String(i);else if(o==="attrs"&&typeof i=="object"&&i!==null)for(let[s,d]of Object.entries(i))r.setAttribute(s,d);else r.setAttribute(o,String(i));}return r}function P(n,e){let t=null;return (...r)=>{t!==null&&clearTimeout(t),t=setTimeout(()=>n(...r),e);}}function w(n,e,t){n.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:true,composed:true}));}var M="Search suggestions couldn't load because this site is blocked from reaching Seekmodo (CORS). Ask your store administrator to allowlist this domain on the Seekmodo gateway, or enable the connector's same-origin suggest proxy.";function X(n,e,t){let r=document.createElement("style");r.textContent=e;let o=a("div",{class:"wrap seekmodo-cors-blocked",part:"wrap cors-blocked",attrs:{role:"status"}});o.append(a("div",{class:"cors-notice",part:"cors-notice",text:t?.message??M})),n.replaceChildren(r,o);}var Y="seekmodo-cors-notice";function J(n,e){if(!n||typeof document>"u")return;let t=n.closest(".search-form")??n.parentNode;if(!t)return;t.style.position=t.style.position||"relative";let r=t.querySelector(`.${Y}`);if(!r){r=document.createElement("div"),r.className=Y,r.setAttribute("role","status"),r.style.cssText=["position:absolute","top:100%","left:0","right:0","z-index:10050","display:none","background:#fff8e6","border:1px solid #f0c040","border-top:none","padding:8px 12px","font-size:13px","line-height:1.4","color:#5c4a00","box-shadow:0 4px 12px rgba(0,0,0,.08)"].join(";"),t.appendChild(r);let o=()=>{if(!r)return;let i=(n.value||"").trim();r.style.display=i.length>=2?"block":"none";};n.addEventListener("input",o),n.addEventListener("focus",o);}r.textContent=e??M;}function Z(){typeof window>"u"||(window.seekmodoShowCorsNotice=J,window.seekmodoScriptLoadFailed=(n,e)=>{let t=n??document.querySelectorAll('input[data-seekmodo-suggest],input[data-seekmodo-typeahead],input[name="s"],input[name="keyword"],input[name="search_query"],input[name="q"],input[type="search"]');for(let r=0;r<t.length;r++){let o=t[r];o instanceof HTMLInputElement&&J(o,e);}});}function ee(n,e,t){try{let r=typeof window<"u"?window.location.origin:"http://localhost",o=new URL(n,r);return o.searchParams.set(e,t),/^https?:\/\//i.test(n)?o.toString():`${o.pathname}${o.search}${o.hash}`}catch{let r=n.includes("?")?"&":"?";return `${n}${r}${encodeURIComponent(e)}=${encodeURIComponent(t)}`}}function Te(n,e){let t=(n??"").toLowerCase(),r=(e??"").toLowerCase();if(r.startsWith("http"))try{r=new URL(r).pathname;}catch{r="";}return t==="page"&&/\/tools\/[^/]+\/?$/.test(r)?"Tool":t==="page"&&/\/tools\/?$/.test(r)?"Tools":t==="page"?"Page":t==="post"?"Article":""}function _(n){let e=typeof n.post_type=="string"?n.post_type.toLowerCase():"",t=typeof n.url=="string"?n.url:typeof n.permalink=="string"?n.permalink:"";return {postType:e,label:Te(e,t)}}var N="split-rail",Ce=15;function x(n,e){let r={"split-rail":5,"command-bar":5,"cinema-grid":6,magazine:6,classic:5}[n]??5,o=Math.max(e,r*3);return Math.min(Ce,o)}var ie=`
...
`,te=.15,re=.85,Pe=.28,ne="seekmodo:split-rail-mobile-ratio-v3",Me="(max-width: 900px)",He=1-Pe;function Ie(){return a("div",{class:"split-divider",part:"split-divider",html:'<svg class="split-divider-icon" viewBox="0 0 36 12" aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg"><rect x="6" y="1" width="24" height="2" rx="1" fill="currentColor"/><rect x="6" y="5" width="24" height="2" rx="1" fill="currentColor"/><rect x="6" y="9" width="24" height="2" rx="1" fill="currentColor"/></svg>'})}function se(n){let e=n.querySelector(".split-divider");if(!e)return ()=>{};let t=window.matchMedia(Me),r=He;try{let g=sessionStorage.getItem(ne);if(g){let p=parseFloat(g);p>=te&&p<=re&&(r=p);}}catch{}let o=g=>{r=Math.min(re,Math.max(te,g)),n.style.setProperty("--split-rail-bottom-grow",String(r)),n.style.setProperty("--split-rail-top-grow",String(1-r));},i=()=>{n.style.removeProperty("--split-rail-top-grow"),n.style.removeProperty("--split-rail-bottom-grow");},s=()=>{t.matches?o(r):i();};s();let d=false,l=0,h=r,m=g=>{!t.matches||g.button!==0||(d=true,l=g.clientY,h=r,e.classList.add("is-dragging"),e.setPointerCapture(g.pointerId),g.preventDefault());},c=g=>{if(!d)return;let f=n.getBoundingClientRect().height-e.offsetHeight;f<=0||o(h+(g.clientY-l)/f);},u=g=>{if(d){d=false,e.classList.remove("is-dragging");try{e.releasePointerCapture(g.pointerId);}catch{}try{sessionStorage.setItem(ne,String(r));}catch{}}};return e.setAttribute("role","separator"),e.setAttribute("aria-orientation","horizontal"),e.setAttribute("aria-label","Resize product and suggestion panels"),e.setAttribute("tabindex","0"),e.addEventListener("pointerdown",m),e.addEventListener("pointermove",c),e.addEventListener("pointerup",u),e.addEventListener("pointercancel",u),t.addEventListener("change",s),()=>{e.removeEventListener("pointerdown",m),e.removeEventListener("pointermove",c),e.removeEventListener("pointerup",u),e.removeEventListener("pointercancel",u),t.removeEventListener("change",s),e.classList.remove("is-dragging"),i();}}var ze="https://seekmodo.com/email-assets/seekmodo-lockup.png";function ae(n,e){return n.resolveThumbSrc?n.resolveThumbSrc(e):e}function de(n){return String(n.name??n.title??"").trim()}function le(n,e,t){e.productTitleTooltip&&t&&n.setAttribute("title",t);}function ce(n,e){return n.productTitleTooltip&&e?e:""}function ue(n,e,t){let r={block:"products",data:e,value:String(e.name??e.title??""),id:e.id!==void 0?String(e.id):void 0},o=n.rows.length;return n.rows.push(r),o}function De(n,e,t){let r=a("div",{class:"thumb-frame",part:"thumb-frame"}),{postType:o}=_(n);o&&r.setAttribute("data-post-type",o);let i=n.image_url??n.image;if(i)r.append(a("img",{class:"thumb",part:"thumb",attrs:{src:ae(e,i),"data-src":i,alt:ce(e,t),loading:"eager",decoding:"async"}}));else {let s=a("div",{class:"thumb-empty",part:"thumb thumb--empty",text:pe(o)});o&&s.setAttribute("data-content-type",o),r.append(s);}return r}function pe(n){return n==="page"?"P":n==="post"?"A":"\xB7"}function Fe(n,e,t){let{postType:r}=_(n),o=n.image_url??n.image;if(o)return a("img",{class:"thumb",part:"thumb",attrs:{src:ae(e,o),"data-src":o,alt:ce(e,t),loading:"eager",decoding:"async"}});let i=a("div",{class:"thumb-empty",part:"thumb thumb--empty",text:pe(r)});return r&&i.setAttribute("data-content-type",r),i}function he(n,e,t="card-price"){if(n.price===void 0||n.price===null)return null;let r=a("div",{class:t,part:"price"});return n.on_sale&&typeof n.sale_price=="number"?(r.append(a("del",{text:e.formatPrice(n.price,n.currency)})),r.append(document.createTextNode(e.formatPrice(n.sale_price,n.currency)))):r.append(document.createTextNode(e.formatPrice(n.price,n.currency))),r}function ge(n,e,t){n.classList.add("row"),n.setAttribute("data-seekmodo-surface","suggest"),n.setAttribute("data-seekmodo-block","products"),n.setAttribute("data-seekmodo-pos",String(t));let r=e.rows[t];r?.id&&n.setAttribute("data-seekmodo-id",r.id),n.addEventListener("click",()=>e.onRowClick(t));}function Oe(n,e,t,r=false){let o=ue(n,e),i=de(e),{postType:s,label:d}=_(e),l=a("button",{class:"product-card",part:"row",type:"button"});s&&l.setAttribute("data-post-type",s),l.append(De(e,n,i));let h=a("span",{class:"card-title",part:"name",text:i});le(h,n,i),l.append(h),d&&l.append(a("span",{class:"card-meta",part:"card-meta",text:d}));let m=he(e,n,"card-price");return m&&l.append(m),ge(l,n,o),l}function $e(n,e,t,r){let o=ue(n,e),i=de(e),s=a("button",{class:"hero-card",part:"row",type:"button"});r&&s.append(a("span",{class:"hero-badge",text:r})),s.append(Fe(e,n,i));let d=a("div",{class:"hero-info"}),l=a("span",{class:"card-title",part:"name",text:i});le(l,n,i),d.append(l);let h=he(e,n);return h&&d.append(h),s.append(d),ge(s,n,o),s}function U(n){let e=n.res.meta?.total??0,t=a("div",{class:"meta-bar",part:"meta-bar"}),r=a("div");r.append(a("span",{class:"count",text:`${e} results for `})),r.append(a("span",{class:"query",text:`"${n.lastQuery}"`})),t.append(r);let o=a("a",{class:"view-all view-all-cta",part:"view-all",attrs:{href:n.viewAllHref},text:n.label("view_all").replace("{total}",String(e))});return o.addEventListener("click",i=>{i.preventDefault(),n.onViewAll();}),t.append(o),t}function Be(n){let e=n.res.did_you_mean;if(!e)return null;let t=a("div",{class:"did-you-mean-bar",part:"did-you-mean"});t.append(document.createTextNode(`Showing results for "${n.lastQuery}". Search instead for `));let r=a("button",{class:"swap",type:"button",text:e}),o=n.rows.length;return n.rows.push({block:"did_you_mean",data:{value:e},value:e}),r.addEventListener("click",()=>n.onRowClick(o)),t.append(r),t.append(document.createTextNode("?")),t}function K(n,e){let t=a("div",{class:"chip-row filter-bar",part:"filter-bar"});return t.append(a("span",{class:"filter-label",text:"Category"})),e.forEach((r,o)=>{let i=a("button",{class:`chip${o===0?" active":""}`,type:"button",text:`${r.name}${typeof r.count=="number"?` ${r.count}`:""}`}),s=n.rows.length;n.rows.push({block:"categories",data:r,value:String(r.name??"")}),i.addEventListener("click",()=>n.onRowClick(s)),t.append(i);}),t}function oe(n,e,t="Try"){let r=a("div",{class:"chip-row",part:"filter-bar"});return r.append(a("span",{class:"filter-label",text:t})),e.forEach(o=>{let i=a("button",{class:"chip",type:"button",text:o.keyword}),s=n.rows.length;n.rows.push({block:"keywords",data:o,value:o.keyword}),i.addEventListener("click",()=>n.onRowClick(s)),r.append(i);}),r}function H(n,e,t,r,o){let i=n.rows.length;n.rows.push({block:e,data:t,value:r});let s=a("button",{class:"row",part:"row",type:"button"});return s.append(a("div",{class:"name",part:"name",text:r})),o&&s.append(a("span",{class:"badge",part:"badge",text:o})),s.setAttribute("data-seekmodo-surface","suggest"),s.setAttribute("data-seekmodo-block",e),s.setAttribute("data-seekmodo-pos",String(i)),s.addEventListener("click",()=>n.onRowClick(i)),s}function I(n,e){if(e.length===0)return null;let t=a("div",{class:"rail-section"});return t.append(a("div",{class:"group-title",part:"group-title",text:n})),e.forEach(r=>t.append(r)),t}function Ue(n){if(!n.showBranding)return null;let e=a("a",{class:"brand-footer",part:"brand-footer",attrs:{href:n.brandUrl,target:"_blank",rel:"noopener noreferrer"}});return e.append(a("span",{class:"brand-by",text:"Powered by "})),e.append(a("img",{class:"brand-logo",part:"brand-logo",attrs:{src:n.brandLogoUrl||ze,alt:"Seekmodo",height:"16"}})),e}function S(n,e,t){let r=a("div",{class:`product-grid cols-${t}`,part:"product-grid"});return e.forEach((o,i)=>r.append(Oe(n,o,i,true))),r}function me(n,e){let t=["wrap","wide"];n==="split-rail"&&t.push("split-rail-panel"),e.splitMobileResize&&t.push("split-rail-mobile"),e.productTitleTooltip&&t.push("product-title-tooltip");let r=a("div",{class:t.join(" "),part:"wrap"});r.append(a("slot",{attrs:{name:"header"}}));let o=(e.res.products??[]).slice(0,x(n,e.limit)),i=(e.res.keywords??[]).slice(0,e.limit),s=(e.res.categories??[]).slice(0,e.limit),d=(e.res.redirects??[]).slice(0,e.limit),l=(e.res.recent??[]).slice(0,5);(e.res.trending??[]).slice(0,5);if(n==="split-rail"){(!e.productsPending||(e.res.meta?.total??0)>0)&&r.append(U(e));let c=e.splitMobileResize?"split-body split-body--mobile-resize":"split-body",u=a("div",{class:c}),g=a("aside",{class:"rail",part:"rail"}),p=i.map(b=>H(e,"keywords",b,b.keyword)),f=I(e.label("keywords"),p);f&&g.append(f);let v=d.map(b=>H(e,"redirects",b,String(b.label||b.matched_term||b.target_url))),k=I(e.label("redirects"),v);k&&g.append(k);let O=s.map(b=>H(e,"categories",b,String(b.name),typeof b.count=="number"?String(b.count):void 0)),V=I(e.label("categories"),O);V&&g.append(V);let ve=l.map(b=>H(e,"recent",b,b.keyword)),q=I(e.label("recent"),ve);q&&g.append(q);let $=a("div",{class:"canvas"});e.productsPending&&o.length===0?$.append(a("div",{class:"products-pending",part:"products-pending",text:"Matching products appear when you pause typing\u2026"})):$.append(S(e,o,5)),u.append($),e.splitMobileResize&&u.append(Ie()),u.append(g),r.append(u);}else if(n==="cinema-grid"){let c=Be(e);c&&r.append(c),r.append(U(e)),s.length&&r.append(K(e,s)),i.length&&r.append(oe(e,i));let u=a("div",{class:"canvas"});u.append(S(e,o,6)),r.append(u);}else if(n==="command-bar"){let c=e.res.meta?.total??0,u=a("div",{class:"command-header",part:"meta-bar"});u.append(a("div",{class:"query-display",text:`"${e.lastQuery}"`})),u.append(a("span",{class:"result-pill",text:`${c} products`}));let g=a("a",{class:"view-all-link",part:"view-all",attrs:{href:e.viewAllHref},text:"View all \u2192"});if(g.addEventListener("click",f=>{f.preventDefault(),e.onViewAll();}),u.append(g),r.append(u),e.res.did_you_mean){let f=a("div",{class:"chip-row"});f.append(a("span",{class:"filter-label",text:"Did you mean"}));let v=a("button",{class:"chip",type:"button",text:e.res.did_you_mean}),k=e.rows.length;e.rows.push({block:"did_you_mean",data:{value:e.res.did_you_mean},value:e.res.did_you_mean}),v.addEventListener("click",()=>e.onRowClick(k)),f.append(v),r.append(f);}i.length&&r.append(oe(e,i,"Related")),s.length&&r.append(K(e,s));let p=a("div",{class:"canvas"});p.append(S(e,o,5)),r.append(p);}else if(n==="magazine"){r.append(U(e)),s.length&&r.append(K(e,s));let c=a("div",{class:"canvas"}),u=o.slice(0,3),g=o.slice(3);if(u.length){c.append(a("div",{class:"group-title",part:"group-title",text:"Best matches"}));let p=a("div",{class:"hero-row",part:"hero-row"});u.forEach((f,v)=>{p.append($e(e,f,v,v===0?"Top match":void 0));}),c.append(p);}g.length?(c.append(a("div",{class:"group-title",part:"group-title",text:"More results"})),c.append(S(e,g,6))):u.length||c.append(S(e,o,6)),r.append(c);}let m=Ue(e);return m&&r.append(m),r.append(a("slot",{attrs:{name:"footer"}})),r}function L(n){return n!=="classic"&&n!==""}var fe=["recent","did_you_mean","redirects","keywords","trending","products","categories"],Ke={recent:"Recently searched",trending:"Trending",keywords:"Suggestions",products:"Products",categories:"Categories",did_you_mean:"Did you mean",redirects:"Go to",view_all:"View all {total} results",empty:"No matches yet \u2014 keep typing.",cors_blocked:M},be=`
  :host {
    ...
    --_rail-bg: var(--seekmodo-suggest-rail-bg, var(--_row-hover));
  }
  .wrap {
    background: var(--_bg);
    color: var(--_color);
