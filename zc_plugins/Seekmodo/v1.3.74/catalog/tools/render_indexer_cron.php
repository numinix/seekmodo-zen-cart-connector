...
$___smLogLib = dirname(__DIR__) . '/includes/functions/numinix_seekmodo_log_lib.php';
if (is_file($___smLogLib)) {
    require_once $___smLogLib;
}
if (function_exists('numinix_seekmodo_require_cli')) {
    numinix_seekmodo_require_cli('render_indexer_cron.php');
}
...
{
    if (function_exists("numinix_seekmodo_stderr")) { numinix_seekmodo_stderr("render_indexer_cron: {$msg}\n"); } elseif (defined("STDERR") && is_resource(STDERR)) { fwrite(STDERR, "render_indexer_cron: {$msg}\n"); }
    exit($code);
}
