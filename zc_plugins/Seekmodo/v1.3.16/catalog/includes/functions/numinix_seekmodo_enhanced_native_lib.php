...
if (!function_exists('numinix_seekmodo_run_enhanced_native_search')) {
    ...
    function numinix_seekmodo_run_enhanced_native_search(string $q, int $page = 1, int $perPage = 20): ?array
    {
        ...
        if (function_exists('_numinix_seekmodo_parse_products_id_query')) {
            $pidIds = _numinix_seekmodo_parse_products_id_query($q);
            if ($pidIds !== null) {
                global $db;
                if (!isset($db) || !is_object($db)) {
                    return null;
                }
                $idCsv = implode(',', array_map('intval', $pidIds));
                $lang = isset($_SESSION['languages_id']) ? (int) $_SESSION['languages_id'] : 1;
                $offset = max(0, ($page - 1) * $perPage);
                $sql = 'SELECT p.products_id FROM ' . TABLE_PRODUCTS . ' p '
                    . 'INNER JOIN ' . TABLE_PRODUCTS_DESCRIPTION . ' pd ON p.products_id = pd.products_id AND pd.language_id = ' . $lang
                    . ' WHERE p.products_status = 1 AND p.products_id IN (' . $idCsv . ')'
                ...
                return [
                    'product_ids' => $ids,
                    'total'       => count($ids),
                    'source'      => 'enhanced_native_products_id',
                ];
            }
        }
