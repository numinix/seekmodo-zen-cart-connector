...
if (!function_exists('numinix_seekmodo_catalog_live_stock_flags_for_ids')) {
     ...
     *
     * For gateway-sized ID lists (≤500) refine with
     * `numinix_seekmodo_catalog_doc_live_in_stock()` /
     * `zen_get_products_stock()` so attribute / master-SKU stock matches
     * the storefront SOLD OUT badge — raw `products_quantity` alone can
     * leave OOS SKUs ranked as in-stock.
     *
     * @param int[] $productIds
    ...
    function numinix_seekmodo_catalog_live_stock_flags_for_ids(array $productIds): array
    {
        ...
        // Gateway SERP / suggest pools stay small; refine with the same
        // live stock helper used at index time. Skip on huge ID lists
        // (legacy EN 12.5k fallback) to avoid STRIN-style OOM.
        if (
            count($productIds) <= 500
            && function_exists('numinix_seekmodo_catalog_doc_live_in_stock')
        ) {
            foreach ($productIds as $pid) {
                if (isset($flags[$pid]) && $flags[$pid] === false) {
                    // Already OOS via qty / NPF — keep demoted.
                    continue;
                }
                $rowQty = $qtyById[$pid] ?? 0;
                $flags[$pid] = numinix_seekmodo_catalog_doc_live_in_stock($pid, $rowQty);
            }
        }
...
if (!function_exists('numinix_seekmodo_default_language_id')) {
    /**
     * Zen Cart DEFAULT_LANGUAGE row id (fallback 1).
     */
    function numinix_seekmodo_default_language_id(): int
    {
        static $cache = null;
        if ($cache !== null) {
            return $cache;
        }
        $dir = defined('DEFAULT_LANGUAGE')
            ? strtolower(trim((string) constant('DEFAULT_LANGUAGE')))
            : 'english';
        if ($dir === '') {
            return $cache = 1;
        }
        if (isset($GLOBALS['db']) && is_object($GLOBALS['db']) && defined('TABLE_LANGUAGES')) {
            try {
                $row = $GLOBALS['db']->Execute(
                    ...
                    . ' WHERE directory = \''
                    . (function_exists('zen_db_input') ? zen_db_input($dir) : addslashes($dir))
                    . '\' LIMIT 1'
                );
                if ($row && !$row->EOF && isset($row->fields['languages_id'])) {
                    $id = (int) $row->fields['languages_id'];

                    return $cache = ($id > 0 ? $id : 1);
                }
            } catch (\Throwable $e) {
                // fall through
            }
...
if (!function_exists('numinix_seekmodo_default_language_code')) {
    function numinix_seekmodo_default_language_code(): string
    {
        $code = numinix_seekmodo_language_code_for_id(numinix_seekmodo_default_language_id());

        return ($code !== null && $code !== '') ? $code : 'en';
    }
}

if (!function_exists('numinix_seekmodo_storefront_is_multilingual')) {
    /**
     * True when the catalog exposes more than one Zen Cart language.
     */
    function numinix_seekmodo_storefront_is_multilingual(): bool
    {
        static $cache = null;
        if ($cache !== null) {
            return $cache;
        ...
        if (isset($GLOBALS['db']) && is_object($GLOBALS['db']) && defined('TABLE_LANGUAGES')) {
            try {
                $row = $GLOBALS['db']->Execute('SELECT COUNT(*) AS c FROM ' . TABLE_LANGUAGES);
                if ($row && !$row->EOF) {
                    $count = (int) ($row->fields['c'] ?? 0);
                }
            } catch (\Throwable $e) {
                $count = 0;
            }
...
if (!function_exists('numinix_seekmodo_language_path_prefixes')) {
    /**
     * ISO slug => URL path prefix for multilingual suggest client filtering.
     * Default storefront language maps to '' (root); others use /{directory}/.
     *
     * @return array<string, string>
     */
    function numinix_seekmodo_language_path_prefixes(): array
    {
        static $cache = null;
        if ($cache !== null) {
            return $cache;
        ...
        $out = [];
        $defaultId = numinix_seekmodo_default_language_id();
        if (isset($GLOBALS['db']) && is_object($GLOBALS['db']) && defined('TABLE_LANGUAGES')) {
            try {
                $rows = $GLOBALS['db']->Execute(
                    'SELECT languages_id, code, directory FROM ' . TABLE_LANGUAGES
                );
                while ($rows && !$rows->EOF) {
                    $langId = isset($rows->fields['languages_id']) ? (int) $rows->fields['languages_id'] : 0;
                    $slug = numinix_seekmodo_language_code_for_id($langId);
                    $directory = isset($rows->fields['directory'])
                        ? strtolower(trim((string) $rows->fields['directory']))
                        : '';
                    if ($slug !== null && $slug !== '') {
                        if ($langId === $defaultId || $directory === '') {
                            $out[$slug] = '';
                        } else {
                            $out[$slug] = '/' . trim($directory, '/') . '/';
                        }
                    }
                    $rows->MoveNext();
                }
        ...
        if ($out === []) {
            $default = numinix_seekmodo_default_language_code();
            foreach (['en' => '/english/', 'de' => '/german/', 'es' => '/spanish/', 'fr' => '/french/'] as $slug => $prefix) {
                $out[$slug] = ($slug === $default) ? '' : $prefix;
            }
        }
...
if (!function_exists('numinix_seekmodo_suggest_i18n_client_config')) {
    /**
     * Client-side suggest locale filter config (WordPress connector parity).
     *
     * @return array{slug:string,default:string,prefixes:array<string,string>}|null
     */
    function numinix_seekmodo_suggest_i18n_client_config(): ?array
    {
        if (!numinix_seekmodo_storefront_is_multilingual()) {
            return null;
        }
        $slug = numinix_seekmodo_current_language_code();
        if ($slug === null || $slug === '') {
            $slug = numinix_seekmodo_default_language_code();
        }

        return [
            'slug'     => $slug,
            'default'  => numinix_seekmodo_default_language_code(),
            'prefixes' => numinix_seekmodo_language_path_prefixes(),
        ];
    }
}
