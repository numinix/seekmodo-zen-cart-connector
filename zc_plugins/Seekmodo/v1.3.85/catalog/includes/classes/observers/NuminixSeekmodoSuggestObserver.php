...
final class NuminixSeekmodoSuggestObserver extends base
{
    ...
    private function autobootScript(): string
    {
        ...
        $cfg = [
            ...
            'i18n_filter'      => function_exists('numinix_seekmodo_suggest_i18n_client_config')
                ? numinix_seekmodo_suggest_i18n_client_config()
                : null,
            'extras'         => [
                'min-length'  => '2',
                'debounce-ms' => '150',
...
<script>(function () {
  ...
  // Multilingual storefronts: gateway Suggest can return mixed-locale rows.
  // Filter /v1/suggest JSON by URL language prefix (WP connector parity).
  if (CFG.i18n_filter && typeof fetch === 'function' && !window.__seekmodoSuggestI18nFetch) {
    window.__seekmodoSuggestI18nFetch = true;
    var I18N = CFG.i18n_filter;
    ...
    function urlMatches(url) {
      ...
      try {
        ...
        var matched = null;
        var prefixes = I18N.prefixes || {};
        for (var slug in prefixes) {
          if (!Object.prototype.hasOwnProperty.call(prefixes, slug)) continue;
          var prefix = prefixes[slug];
          if (prefix && path.indexOf(prefix) === 0) { matched = slug; break; }
        }
        if (matched) return matched === I18N.slug;
        return I18N.slug === (I18N.default || 'en');
      } catch (_e) { return true; }
    }
    function filterProducts(list) {
      if (!Array.isArray(list)) return list;
      return list.filter(function (row) {
        var u = (row && (row.url || row.permalink)) || '';
        return urlMatches(u);
      });
    }
    function filterEnvelope(data) {
      if (!data || typeof data !== 'object') return data;
      if (Array.isArray(data.products)) {
        data.products = filterProducts(data.products);
        if (typeof data.total === 'number') data.total = data.products.length;
        if (data.meta && typeof data.meta === 'object') data.meta.total = data.products.length;
      }
      if (Array.isArray(data.categories)) {
        ...
        for (var i = 0; i < data.categories.length; i++) {
          ...
          var u = row.url || row.permalink || row.href || '';
          if (u && !urlMatches(u)) continue;
          var name = String(row.name || row.title || row.label || '').toLowerCase();
    ...
    function suggestRequestUrl(input) {
      if (typeof input === 'string') return input;
      if (input && input.url) return String(input.url);
      return '';
    }
    function bumpSuggestLimit(bodyObj) {
      if (!bodyObj || typeof bodyObj !== 'object') return bodyObj;
      var lim = parseInt(bodyObj.limit, 10);
      if (!lim || lim < 48) bodyObj.limit = 48;
      return bodyObj;
    }
    function prepareSuggestFetchArgs(args) {
      var input = args[0];
      var init = args[1];
      var url = suggestRequestUrl(input);
      if (!url || url.indexOf('/v1/suggest') === -1) return args;
      var method = 'GET';
      if (init && init.method) method = String(init.method);
      else if (input && input.method) method = String(input.method);
      if (method.toUpperCase() !== 'POST') return args;
      try {
        if (init && typeof init.body === 'string') {
          var parsed = JSON.parse(init.body);
          bumpSuggestLimit(parsed);
          var newInit = {};
          for (var k in init) {
    ...
    window.fetch = function () {
      var args = prepareSuggestFetchArgs(arguments);
      var input = args[0];
      var url = suggestRequestUrl(input);
      return origFetch.apply(this, args).then(function (res) {
        if (!url || url.indexOf('/v1/suggest') === -1) return res;
        return res.clone().json().then(function (data) {
          var filtered = filterEnvelope(data);
  ...
  function baseSerpPassthrough() {
    return (CFG && CFG.serp_passthrough && typeof CFG.serp_passthrough === 'object')
      ? Object.assign({}, CFG.serp_passthrough) : {};
  }
  function mergeSerpPassthrough(extra) {
    var base = baseSerpPassthrough();
    if (!extra || typeof extra !== 'object') {
      return base;
    }
    var out = Object.assign({}, base, extra);
    if (base.filter_by && extra.filter_by) {
      out.filter_by = '(' + base.filter_by + ') && (' + extra.filter_by + ')';
    }
    return out;
  }
  ...
  function attach(input) {
    ...
    if (CFG.prefer_local) {
      el.setAttribute('prefer-local', 'true');
    }
  ...
  function defaultViewAllHref() {
    if (CFG && CFG.view_all_href) return CFG.view_all_href;
    var path = (location && location.pathname) || '';
    var m = path.match(/^\/(english|french|german|deutsch|spanish|espanol|francais|en|fr|de|es)(?=\/|$)/i);
    var prefix = m ? ('/' + String(m[1]).toLowerCase()) : '';
    return prefix + '/index.php?main_page=advanced_search_result&keyword={q}&pfrom={price_from}&pto={price_to}';
  }
  function applySuggestExactPartUi(nodes, exactPartMode) {
    var viewAll = defaultViewAllHref();
  ...
  function syncSuggestVehicleFilter() {
    if (isVehicleFilterOptedOut()) {
      var viewAllOptOut = defaultViewAllHref();
      ...
      for (var o = 0; o < nodesOptOut.length; o++) {
        ...
        var baseOpt = baseSerpPassthrough();
        if (Object.keys(baseOpt).length > 0) {
          nodesOptOut[o].setAttribute('serp-passthrough', JSON.stringify(baseOpt));
        } else {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (pt) {
        nodes[i].setAttribute('serp-passthrough', JSON.stringify(mergeSerpPassthrough(pt)));
      } else {
        var baseOnly = baseSerpPassthrough();
        if (Object.keys(baseOnly).length > 0) {
          nodes[i].setAttribute('serp-passthrough', JSON.stringify(baseOnly));
        } else {
  ...
  // with reason=quota. Stamp PHP sticky only — keep Seekmodo-styled
  // <seekmodo-suggest> mounted; the bundle fills via typeahead-fallback.
  // Do not destroy the CE or load plain legacy typeahead on quota.
...
JS;
    ...
    private function suggestLabels(): array
    {
        ...
        $map = [
            ...
            'category_filter'      => 'TEXT_SEEKMODO_SUGGEST_CATEGORY_FILTER',
            'related'              => 'TEXT_SEEKMODO_SUGGEST_RELATED',
            'try'                  => 'TEXT_SEEKMODO_SUGGEST_TRY',
            'page'                 => 'TEXT_SEEKMODO_SUGGEST_PAGE',
            'article'              => 'TEXT_SEEKMODO_SUGGEST_ARTICLE',
            'tool'                 => 'TEXT_SEEKMODO_SUGGEST_TOOL',
            'tools'                => 'TEXT_SEEKMODO_SUGGEST_TOOLS',
    ...
    private function viewAllHref(): string
    {
        ...
        $query = 'keyword={q}&pfrom={price_from}&pto={price_to}' . $suffix;

        // Prefer zen_href_link (raw) so catalog base / SEO rewrite matches the storefront.
        if (function_exists('numinix_seekmodo_href_link_raw')) {
            $raw = numinix_seekmodo_href_link_raw(
                $mainPage,
                'keyword=__SEEKMODO_Q__&pfrom=__SEEKMODO_PF__&pto=__SEEKMODO_PT__' . $suffix
            );
            if ($raw !== '') {
                $href = str_replace(
                    ['__SEEKMODO_Q__', '__SEEKMODO_PF__', '__SEEKMODO_PT__'],
                    ['{q}', '{price_from}', '{price_to}'],
                    $raw
                );

                return $this->withLanguagePathPrefix($href);
            }
        }

        if (!defined('DIR_WS_CATALOG')) {
            return $this->withLanguagePathPrefix('/?main_page=' . $mainPage . '&' . $query);
        }

        return $this->withLanguagePathPrefix(
            ((string) constant('DIR_WS_CATALOG'))
            . 'index.php?main_page=' . $mainPage . '&' . $query
        );
    }

    /**
     * Prepend /french, /fr, etc. when DIR_WS_CATALOG is "/" but the request
     * lives under a language path prefix (common multilingual ZC layout).
     */
    private function withLanguagePathPrefix(string $href): string
    {
        $prefix = $this->detectLanguagePathPrefix();
        if ($prefix === '' || $href === '') {
            return $href;
        }
        ...
        if (preg_match('#^https?://#i', $href)) {
            ...
            if ($this->pathHasLanguagePrefix($path, $prefix)) {
                return $href;
            }
            $parts['path'] = rtrim($prefix, '/') . '/' . ltrim($path, '/');
        ...
        $qpos = strpos($href, '?');
        if ($qpos !== false) {
            $pathOnly = substr($href, 0, $qpos);
        }
        if ($this->pathHasLanguagePrefix($pathOnly, $prefix)) {
            return $href;
        }
        if (isset($href[0]) && $href[0] === '/') {
            return rtrim($prefix, '/') . $href;
        }
    ...
    private function pathHasLanguagePrefix(string $path, string $prefix): bool
    {
        $prefix = '/' . trim($prefix, '/');
        $path = '/' . ltrim($path, '/');

        return (bool) preg_match('#^' . preg_quote($prefix, '#') . '(?:/|$)#i', $path);
    }

    /**
     * Detect /french, /deutsch, /fr, etc. from REQUEST_URI (and known session
     * language directory names) when the catalog web root itself has no lang segment.
     */
    private function detectLanguagePathPrefix(): string
    {
        $catalog = defined('DIR_WS_CATALOG') ? (string) constant('DIR_WS_CATALOG') : '/';
        $catalogNorm = '/' . trim(str_replace('\\', '/', $catalog), '/');
        if ($catalogNorm === '/') {
            $catalogNorm = '';
        }

        // If catalog already embeds a language directory, zen_href_link / DIR_WS is enough.
        foreach ($this->knownLanguagePathSegments() as $seg) {
            if ($catalogNorm !== '' && preg_match('#/' . preg_quote($seg, '#') . '(?:/|$)#i', $catalogNorm)) {
                return '';
            }
        }
        ...
        // Strip catalog base from the request path when the shop is in a subdir.
        if ($catalogNorm !== '' && stripos($path, $catalogNorm) === 0) {
            $path = substr($path, strlen($catalogNorm));
            $path = '/' . ltrim($path, '/');
        }
     ...
     * @return list<string>
     */
    private function knownLanguagePathSegments(): array
    {
        $segs = [
            'english', 'french', 'german', 'deutsch', 'spanish', 'espanol', 'francais',
            'en', 'fr', 'de', 'es',
        ];
        $sessionDir = '';
        try {
            $sessionDir = strtolower(trim($this->suggestLanguageDirectory()));
        } catch (\Throwable $e) {
            $sessionDir = '';
        }
        if ($sessionDir !== '' && !in_array($sessionDir, $segs, true)) {
            $segs[] = $sessionDir;
        }

        // Longer names first so "deutsch" wins before a hypothetical shorter clash.
        usort($segs, static function (string $a, string $b): int {
