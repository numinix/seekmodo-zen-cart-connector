...
class NuminixSeekmodoObserver extends \base
{
    ...
    private function onProductInfo(): void
    {
        ...
        $positions = $_SESSION[self::SESSION_POSITION_MAP_KEY] ?? null;
        if (!is_array($positions) || !isset($positions[$productsId])) {
            return;
        }
        $isLocalSerp = $this->referrerIsLocalSerp();
        $isLocalReferer = $this->referrerIsSameHost();
        if (!$isLocalSerp && !$isLocalReferer) {
            // External or empty referer — not our suggest dropdown.
            return;
        }
    ...
    /**
     * NOTIFY_CART_ADD_CART_END handler. Resolves products_id from the
     * notifier param (Zen Cart passes it as $param1 on most versions),
     * then $_GET / $_POST / $_REQUEST fallbacks for AJAX add-to-cart.
     */
    private function onCartAdd($param): void
    {
        ...
        $productsId = $this->resolveCartAddProductsId($param);
        if ($productsId <= 0) {
            return;
        }
        $opts = [];
    ...
    /**
     * @param mixed $param Notifier $param1 (often products_id int).
     */
    private function resolveCartAddProductsId($param): int
    {
        if (is_numeric($param)) {
            $pid = (int) $param;
            if ($pid > 0) {
        ...
        if (is_object($param)) {
            if (isset($param->products_id) && is_numeric($param->products_id)) {
                $pid = (int) $param->products_id;
                if ($pid > 0) {
                    return $pid;
                }
            }
            if (method_exists($param, 'get_products_id')) {
                $raw = $param->get_products_id();
                if (is_numeric($raw)) {
                    $pid = (int) $raw;
                    if ($pid > 0) {
                        return $pid;
        ...
        foreach (['products_id', 'product_id'] as $key) {
