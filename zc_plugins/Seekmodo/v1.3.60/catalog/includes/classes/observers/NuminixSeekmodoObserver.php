...
class NuminixSeekmodoObserver extends \base
{
    ...
    private function emitSerpClickBeacon(): void
    {
        ...
        echo '<script>(function(){'
            ...
            // Reject category / facet URLs — bare -N tails were
            // stamping categories_id as product_id on Klevu SERPs
            // (KIP/Redline LTR first-click poison).
            . 'if(/[?&](?:cPath|categories_id)=/i.test(href))return null;'
            . 'if(/-c-\\d+/i.test(href))return null;'
            . 'var m=href.match(/[?&]products_id=(\\d+)/);if(m)return m[1];'
            . 'm=href.match(/-p-(\\d+)(?:\\.html|[\\/?#]|$)/i);if(m)return m[1];'
            . 'm=href.match(/-(\\d+)\\.html(?:\\?|#|$)/i);return m?m[1]:null;'
            . '}'
