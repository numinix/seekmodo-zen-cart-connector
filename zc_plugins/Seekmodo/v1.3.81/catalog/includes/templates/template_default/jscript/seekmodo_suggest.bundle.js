var SeekmodoSuggest=(function(exports){'use strict';var x=class extends Error{status;body;tool;constructor(n,e,t,r){super(n),this.name="SeekmodoError",this.status=e,this.body=t,this.tool=r;}},X=class extends x{constructor(n,e,t){super(`Seekmodo auth failed (HTTP ${n})`,n,e,t),this.name="SeekmodoAuthError";}},K=class extends x{code;bucket;limit;used;constructor(n,e){super("Seekmodo over quota (HTTP 402)",402,n,e),this.name="SeekmodoQuotaError";let t=n??{};this.code=t.code??"over_quota",this.bucket=t.bucket,this.limit=t.limit,this.used=t.used;}},Be=class extends x{constructor(n,e,t){super(`Seekmodo server error (HTTP ${n})`,n,e,t),this.name="SeekmodoServerError";}},De=class extends x{constructor(n,e,t){super(`Seekmodo request rejected (HTTP ${n})`,n,e,t),this.name="SeekmodoRequestError";}},Z=class extends x{constructor(n,e){super(`Seekmodo network failure${n instanceof Error?`: ${n.message}`:""}`,0,n,e),this.name="SeekmodoNetworkError";}};function P(n,e){if(n instanceof Z)return P(n.body);if(n instanceof TypeError){let t=n.message.toLowerCase();return t.includes("failed to fetch")||t.includes("networkerror")||t.includes("network request failed")||t.includes("load failed")}if(n instanceof Error){let t=n.message.toLowerCase();return t.includes("cors")||t.includes("access-control-allow-origin")||t.includes("cross-origin")}return  false}var Oe="https://gateway.seekmodo.com",Fe=8e3,$e=class{config;cachedToken=null;constructor(n){if(!n.tenantId)throw new Error("Seekmodo SDK: tenantId is required");if(typeof n.getToken!="function")throw new Error("Seekmodo SDK: getToken callback is required");this.config={tenantId:n.tenantId,getToken:n.getToken,baseUrl:(n.baseUrl??Oe).replace(/\/+$/,""),fetch:n.fetch??globalThis.fetch.bind(globalThis),timeoutMs:n.timeoutMs??Fe,signal:n.signal,onError:n.onError,getRegion:n.getRegion};}clearTokenCache(){this.cachedToken=null;}async call(n,e,t={}){try{return await this.callOnce(n,e,t,!1)}catch(r){if(r instanceof X){this.clearTokenCache();try{return await this.callOnce(n,e,t,!0)}catch(o){throw this.config.onError?.(o,{tool:n}),o}}throw this.config.onError?.(r,{tool:n}),r}}async callOnce(n,e,t,r){let o=await this.resolveToken(r),i=`${this.config.baseUrl}/v1/${encodeURIComponent(n)}`,s=new AbortController,l=t.timeoutMs??this.config.timeoutMs,d=setTimeout(()=>s.abort(),l),u=()=>s.abort();this.config.signal?.addEventListener("abort",u,{once:true}),t.signal?.addEventListener("abort",u,{once:true});let m={"Content-Type":"application/json",Authorization:`Bearer ${o}`,"X-Seekmodo-Tenant":this.config.tenantId,"X-Seekmodo-SDK":"@seekmodo/sdk@0.1.0"};if(this.config.getRegion)try{let h=await this.config.getRegion();typeof h=="string"&&h.length>0&&(m["Seekmodo-Region"]=h);}catch{}let c;try{c=await this.config.fetch(i,{method:"POST",headers:m,body:JSON.stringify(e),signal:s.signal});}catch(h){throw new Z(h,n)}finally{clearTimeout(d),this.config.signal?.removeEventListener("abort",u),t.signal?.removeEventListener("abort",u);}let p=await c.text(),g=p?qe(p):null;if(c.status===401||c.status===403)throw new X(c.status,g,n);if(c.status===402)throw new K(g,n);if(c.status>=500)throw new Be(c.status,g,n);if(!c.ok)throw new De(c.status,g,n);return g}async resolveToken(n){let e=Date.now();if(!n&&this.cachedToken&&this.cachedToken.expiresAt-1e4>e)return this.cachedToken.token;let t=await this.config.getToken();if(typeof t=="string")return this.cachedToken={token:t,expiresAt:e+6e4},t;if(t&&typeof t=="object"&&typeof t.token=="string"&&typeof t.expiresAt=="number")return this.cachedToken={token:t.token,expiresAt:t.expiresAt},t.token;throw new Error("Seekmodo SDK: getToken must return a string or { token, expiresAt }")}};function qe(n){try{return JSON.parse(n)}catch{return n}}var ee=class{transport;recommend;bundle;constructor(n){this.transport=new $e(n),this.recommend={related:(e,t)=>this.transport.call("recommend.related",{...e},t??{}),alsoBought:(e,t)=>this.transport.call("recommend.also_bought",{...e},t??{}),alsoViewed:(e,t)=>this.transport.call("recommend.also_viewed",{...e},t??{}),trending:(e,t)=>this.transport.call("recommend.trending",{...e},t??{})},this.bundle={suggest:(e,t)=>this.transport.call("bundle.suggest",{...e},t??{})};}search(n,e){return this.transport.call("search",{...n},e??{})}suggest(n,e){return this.transport.call("suggest",{...n},e??{})}searchByImage(n,e){return this.transport.call("search.byImage",{...n},e??{})}chat(n,e){return this.transport.call("chat",{...n},e??{})}ask(n,e){return this.transport.call("ask",{...n},e??{})}event(n,e){return this.transport.call("events",{...n},e??{})}};var C=null,v=null;function M(n){if(typeof document>"u")return null;let t=document.head?.querySelector(`meta[name="${n}"]`)?.getAttribute("content");return t&&t.length>0?t:null}function te(){return C!==null||(C=je()),C}async function je(){let n=M("seekmodo:tenant");if(!n)throw new Error('@seekmodo/web-components: <meta name="seekmodo:tenant"> is required');let e=M("seekmodo:token"),t=M("seekmodo:refresh");if(!e&&!t)throw new Error('@seekmodo/web-components: either <meta name="seekmodo:token"> or <meta name="seekmodo:refresh"> must be set');e&&(v={token:e,expiresAt:Date.now()+3e4});let r=M("seekmodo:gateway")??void 0;return new ee({tenantId:n,baseUrl:r,getRegion:()=>Ne(),getToken:async()=>{let o=Date.now();if(v&&v.expiresAt-1e4>o)return {token:v.token,expiresAt:v.expiresAt};if(!t){if(v)return {token:v.token,expiresAt:v.expiresAt};throw new Error("seekmodo:refresh meta missing; no way to refresh token")}let i=await fetch(t,{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/json"}});if(!i.ok)throw new Error(`seekmodo:refresh route returned HTTP ${i.status}`);let s=await i.json();if(!s.token||typeof s.expires_at!="number")throw new Error("seekmodo:refresh route returned a malformed envelope");return v={token:s.token,expiresAt:s.expires_at*1e3},{token:v.token,expiresAt:v.expiresAt}}})}var Ke="seekmodo_region";function Ue(n){if(typeof n!="string")return null;let e=n.trim().toLowerCase();return /^[a-z0-9][a-z0-9_-]{1,63}$/.test(e)?e:null}function Ne(){if(typeof document>"u")return null;let n=document.cookie??"";if(n.length===0)return null;let e=Ke.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),t=new RegExp(`(?:^|; )${e}=([^;]+)`).exec(n);if(!t)return null;try{return Ue(decodeURIComponent(t[1]))}catch{return null}}var z=class extends HTMLElement{root;rafId=null;constructor(){super(),this.root=this.attachShadow({mode:"open"});}scheduleRender(){this.rafId===null&&(this.rafId=requestAnimationFrame(()=>{this.rafId=null;try{this.render(),this.afterRender();}catch(e){console.warn("[seekmodo] render failure",e);try{this.renderError("internal_error");}catch{this.root.innerHTML="";}}}));}afterRender(){}async getClient(){return te()}renderError(e){this.root.innerHTML="";}disconnectedCallback(){this.rafId!==null&&(cancelAnimationFrame(this.rafId),this.rafId=null);}};function a(n,e,t){let r=document.createElement(n);if(e){for(let[o,i]of Object.entries(e))if(!(i==null||i===false))if(o==="class")r.className=String(i);else if(o==="part")r.setAttribute("part",String(i));else if(o==="text")r.textContent=String(i);else if(o==="html")r.innerHTML=String(i);else if(o==="attrs"&&typeof i=="object"&&i!==null)for(let[s,l]of Object.entries(i))r.setAttribute(s,l);else r.setAttribute(o,String(i));}return r}function I(n,e){let t=null;return (...r)=>{t!==null&&clearTimeout(t),t=setTimeout(()=>n(...r),e);}}function w(n,e,t){n.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:true,composed:true}));}var H="Search suggestions couldn't load because this site is blocked from reaching Seekmodo (CORS). Ask your store administrator to allowlist this domain on the Seekmodo gateway, or enable the connector's same-origin suggest proxy.";function oe(n,e,t){let r=document.createElement("style");r.textContent=e;let o=a("div",{class:"wrap seekmodo-cors-blocked",part:"wrap cors-blocked",attrs:{role:"status"}});o.append(a("div",{class:"cors-notice",part:"cors-notice",text:t?.message??H})),n.replaceChildren(r,o);}var re="seekmodo-cors-notice";function ne(n,e){if(!n||typeof document>"u")return;let t=n.closest(".search-form")??n.parentNode;if(!t)return;t.style.position=t.style.position||"relative";let r=t.querySelector(`.${re}`);if(!r){r=document.createElement("div"),r.className=re,r.setAttribute("role","status"),r.style.cssText=["position:absolute","top:100%","left:0","right:0","z-index:10050","display:none","background:#fff8e6","border:1px solid #f0c040","border-top:none","padding:8px 12px","font-size:13px","line-height:1.4","color:#5c4a00","box-shadow:0 4px 12px rgba(0,0,0,.08)"].join(";"),t.appendChild(r);let o=()=>{if(!r)return;let i=(n.value||"").trim();r.style.display=i.length>=2?"block":"none";};n.addEventListener("input",o),n.addEventListener("focus",o);}r.textContent=e??H;}function ie(){typeof window>"u"||(window.seekmodoShowCorsNotice=ne,window.seekmodoScriptLoadFailed=(n,e)=>{let t=n??document.querySelectorAll('input[data-seekmodo-suggest],input[data-seekmodo-typeahead],input[name="s"],input[name="keyword"],input[name="search_query"],input[name="q"],input[type="search"]');for(let r=0;r<t.length;r++){let o=t[r];o instanceof HTMLInputElement&&ne(o,e);}});}function se(n){return n.trim().toUpperCase()==="GBP"?[{min:0,max:15},{min:15,max:25},{min:25,max:50},{min:50,max:null}]:[{min:0,max:20},{min:20,max:50},{min:50,max:100},{min:100,max:null}]}function B(n){return `${n.min}:${n.max??""}`}function D(n,e){return !n||!e?n===e:n.min===e.min&&n.max===e.max}function ae(n){return n.max===null?`price:>=${n.min}`:`price:>=${n.min} && price:<=${n.max}`}function le(n,e){let t=(n??"").trim();return t?`(${t}) && (${e})`:e}function de(n,e){return n.max===null?`${e(n.min)}+`:`${e(n.min)} \u2013 ${e(n.max)}`}function ce(n,e){try{let t=typeof window<"u"?window.location.origin:"http://localhost",r=new URL(n,t),o=e??[...r.searchParams.keys()];for(let i of o){let s=r.searchParams.get(i);(s===null||s==="")&&r.searchParams.delete(i);}return /^https?:\/\//i.test(n)?r.toString():`${r.pathname}${r.search}${r.hash}`}catch{return n}}function ue(n,e,t){try{let r=typeof window<"u"?window.location.origin:"http://localhost",o=new URL(n,r);return o.searchParams.set(e,t),/^https?:\/\//i.test(n)?o.toString():`${o.pathname}${o.search}${o.hash}`}catch{let r=n.includes("?")?"&":"?";return `${n}${r}${encodeURIComponent(e)}=${encodeURIComponent(t)}`}}function Ve(n,e,t){let r=(n??"").toLowerCase(),o=(e??"").toLowerCase();if(o.startsWith("http"))try{o=new URL(o).pathname;}catch{o="";}return r==="page"&&/\/tools\/[^/]+\/?$/.test(o)?t?.("tool")??"Tool":r==="page"&&/\/tools\/?$/.test(o)?t?.("tools")??"Tools":r==="page"?t?.("page")??"Page":r==="post"?t?.("article")??"Article":""}function S(n,e){let t=typeof n.post_type=="string"?n.post_type.toLowerCase():"",r=typeof n.url=="string"?n.url:typeof n.permalink=="string"?n.permalink:"";return {postType:t,label:Ve(t,r,e)}}var Q="split-rail",Qe=15;function A(n,e){let r={"split-rail":5,"command-bar":5,"cinema-grid":6,magazine:6,classic:5}[n]??5,o=Math.max(e,r*3);return Math.min(Qe,o)}var fe=`
