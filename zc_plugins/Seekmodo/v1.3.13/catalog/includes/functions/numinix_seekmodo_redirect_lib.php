    ...
    function numinix_seekmodo_match_redirect_rules(array $rules, string $q, string $mode = 'search'): ?array
    {
        ...
        foreach ($rules as $row) {
            ...
            foreach ($terms as $term) {
                ...
                if (!numinix_seekmodo_redirect_term_matches($qNorm, $term, $matchMode, $mode)) {
                    continue;
                }
...
if (!function_exists('numinix_seekmodo_redirect_term_matches')) {
    function numinix_seekmodo_redirect_term_matches(
        ...
        if ($matchMode === 'contains') {
            if ($mode === 'search') {
                return stripos($qNorm, $termNorm) !== false
                    || stripos($termNorm, $qNorm) !== false;
            }
            return stripos($termNorm, $qNorm) === 0
        ...
        if ($mode === 'suggest') {
            return strncmp($termNorm, $qNorm, strlen($qNorm)) === 0;
        }
        return false;
    }
}
