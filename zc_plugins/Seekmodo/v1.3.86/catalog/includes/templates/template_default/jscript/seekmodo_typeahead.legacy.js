...
(function () {
    ...
    function suggestLabel(key, fallback) {
        try {
            var L = window.SeekmodoSuggestLabels;
    ...
    function endpointBase() {
        ...
        for (var i = 0; i < scripts.length; i++) {
            ...
            // Bundle: seekmodo_typeahead.legacy.js (plugin tree).
            // Older copies: jscript_seekmodo_typeahead.js under template_default.
            if (src.indexOf('seekmodo_typeahead') === -1
                && src.indexOf('jscript_seekmodo_typeahead') === -1) {
                continue;
            }
            // Only resolve from a real template jscript URL — NOT from
            // zc_plugins/.../catalog/includes/... (that "catalog" dir is
            // not where the PHP shim is installed).
        ...
        // Site-root fallback (works for DIR_WS_CATALOG = /).
        try {
    ...
    function renderDropdown(dropdown, q, payload) {
        ...
        section(suggestLabel('keywords', 'Suggestions'), payload.keywords || [], function (kw) {
            var word = kw.keyword || '';
            if (!word) return '';
        ...
        section(suggestLabel('products', 'Products'), (payload.products || []).slice(0, MAX_PRODUCTS), function (p) {
            var label = p.value || p.name || '';
            if (!label) return '';
            var href = p.url || ('index.php?main_page=product_info&products_id=' + (p.products_id || ''));
            var priceText = plainPrice(p.price);
            var price = priceText ? ('<span style="margin-left:8px;color:#888;">' + escapeHtml(priceText) + '</span>') : '';
            var image = p.image
        ...
        section(suggestLabel('categories', 'Categories'), payload.categories || [], function (cat) {
            var name = cat.name || '';
            if (!name) return '';
