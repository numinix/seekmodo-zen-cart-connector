    ...
    foreach ($rows as $row) {
        ...
        if ($pid > 0) {
            ...
            if (function_exists('numinix_seekmodo_plain_display_price')) {
                $row['price_formatted'] = numinix_seekmodo_plain_display_price($pid);
            } elseif (function_exists('zen_get_products_display_price')) {
                $rawPrice = (string) @zen_get_products_display_price($pid);
                $row['price_formatted'] = function_exists('numinix_seekmodo_plain_price_text')
                    ? numinix_seekmodo_plain_price_text($rawPrice)
                    : trim(strip_tags($rawPrice));
            }
