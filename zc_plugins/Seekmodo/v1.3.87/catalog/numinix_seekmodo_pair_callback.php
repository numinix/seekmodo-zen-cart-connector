...
// v1.3.24 — Zen Cart 1.5.7 / file-only installs often boot application_top
// without merging zc_plugins auto_loaders. Load catalog init explicitly so
// Numinix\Seekmodo\Pairing is available (otherwise pair POSTs return
// Class "…\Pairing" not found while the shim itself is live).
$ensureHelpers = [
...
if (!class_exists(Pairing::class)) {
    http_response_code(503);
    echo json_encode([
        'ok' => false,
        'error' => 'seekmodo_plugin_init_missing',
        'hint' => 'Install/Upgrade Seekmodo via Plugin Manager, or ensure zc_plugins/Seekmodo/v*/catalog is present.',
    ]);
    return;
}
