...
// Load catalog language pack from the plugin tree so suggest UI
// labels follow the shopper's Zen Cart language (english / german /
// deutsch / spanish / french). File-only installs never merge
// zc_plugins language files into DIR_FS_CATALOG/includes/languages/.
(static function (): void {
    $dir = '';
    if (isset($_SESSION['language']) && is_string($_SESSION['language'])) {
        $dir = strtolower(trim($_SESSION['language']));
    }
    // Match SuggestObserver::suggestLangCode(): when the session is
    // not ready yet (common on the first catalog hit of a request),
    // fall back to Zen Cart's DEFAULT_LANGUAGE instead of hard-coding
    // english. German-primary shops (NS-26042 Cannapot) otherwise ship
    // English suggest chrome while lang="de".
    ...
    foreach ($candidates as $path) {
        ...
        // Swallow UTF-8 BOM / stray whitespace from language packs so
        // it never leaks into the storefront HTML stream.
        ob_start();
        $loaded = include $path;
        ob_end_clean();
        if (is_array($loaded)) {
...
foreach ([
    ...
    'numinix_seekmodo_enhanced_native_lib.php',
    ...
    // v1.3.87 — exact stock + complete order-line sync for merchant
    // restock guidance. Independent from shopper search/indexing.
    'numinix_seekmodo_restock_lib.php',
...
// v1.3.69 — subscribed default is the split-rail `<seekmodo-suggest>`
// widget. These constants used to exist only when recovery scripts
// (or extra_configures) defined them; missing meant modern, but a
// leftover true in `configuration` forced the legacy dropdown. Define
// false/true here only when the configuration table did not already
// load them (Zen Cart defines configuration keys first).
if (!defined('NUMINIX_SEEKMODO_SUGGEST_ENABLED')) {
    define('NUMINIX_SEEKMODO_SUGGEST_ENABLED', 'true');
}
if (!defined('NUMINIX_SEEKMODO_SUGGEST_USE_LEGACY')) {
    define('NUMINIX_SEEKMODO_SUGGEST_USE_LEGACY', 'false');
}
