...
class Client
{
    ...
    /**
     * Tenant-scoped tool invocation for connector batch jobs.
     *
     * Inventory snapshots and historical order syncs can carry hundreds
     * of rows, so they use the existing index timeout instead of the
     * shopper hot-path timeout used by callTool().
     *
     * @param array<string,mixed> $args
     * @return array<string,mixed>|null
     */
    public function callBulkTool(string $toolName, array $args): ?array
    {
        if (preg_match('/^[A-Za-z0-9_.\\-]{2,128}$/', $toolName) !== 1) {
            $this->log('warn', 'call_bulk_tool_bad_name', ['tool' => $toolName]);
            return null;
        }
        return $this->call('/v1/' . $toolName, $args, $this->indexTimeoutMs);
    }
