...
final class CronReconciler
{
    ...
    public function reconcile(): array
    {
        ...
        $excludedStatuses = [];
        if (defined('NUMINIX_SEEKMODO_RESTOCK_EXCLUDED_STATUS_IDS')) {
            $excludedStatuses = array_values(array_unique(array_filter(
                array_map(
                    'intval',
                    explode(',', (string) constant('NUMINIX_SEEKMODO_RESTOCK_EXCLUDED_STATUS_IDS'))
                ),
                static fn (int $id): bool => $id > 0
            )));
        }
        $restockArgs = ' --days=365';
        if ($excludedStatuses !== []) {
            $restockArgs .= ' --exclude-statuses=' . implode(',', $excludedStatuses);
        }
        $fullCmd = 'cd ' . $docroot . ' && (/usr/bin/php '
            . $pluginVer . '/catalog/numinix_seekmodo_push_catalog.php'
            . ' >>' . $this->logPath('indexer') . ' 2>&1; /usr/bin/php '
            . $pluginVer . '/catalog/numinix_seekmodo_sync_restock.php' . $restockArgs
            . ' >>' . $this->logPath('restock') . ' 2>&1)';
