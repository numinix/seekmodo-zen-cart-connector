...
final class Pairing
{
     ...
     *   - Resolve a PHP binary (PHP_BINARY in CLI is empty in the
     *     fpm context — fall back to /usr/bin/php which is the
     *     one EasyApache + most distros install).
     *   - shell_exec a `nohup php <pusher> >> <log> 2>&1 &` so
     *     the child detaches and the parent returns immediately.
    ...
    private static function fork_initial_push(): array
    {
        ...
        // Resolve a PHP binary. PHP_BINARY in fpm typically points
        // at php-fpm itself which can't run a script — prefer the
        // operator's $PATH php.
        $php = self::resolve_php_binary();
        if ($php === '') {
            return ['forked' => false, 'err' => 'no php binary found in $PATH', 'cmd' => ''];
        }
    ...
    /**
     * Best-effort PHP binary resolution. Prefers `which php` (the
     * operator's $PATH); falls back to a small set of well-known
     * locations on cPanel / Debian / Ubuntu / RHEL hosts. Returns
     * '' if nothing resolves — the caller turns that into a clean
     * "fork skipped" report rather than crashing the pair.
     */
