...
if (!function_exists('numinix_seekmodo_log_append')) {
    $___smLogLib = dirname(__DIR__, 2) . '/functions/numinix_seekmodo_log_lib.php';
 ...
 *     `$result->sql_query` + `$GLOBALS['listing_sql']` to a
 *     `SELECT ... WHERE products_id IN (...) ORDER BY FIELD(products_id, ...)`
 *     clause so the rest of the page render (header count from `$result`,
 *     pagination + product grid from `$listing_sql`) pulls from the
 *     gateway-ranked subset. Same notifier covers `ajax/ajax_search.php`
...
class NuminixSeekmodoObserver extends \base
{
    ...
    public function __construct()
    {
        $this->attach($this, [
            ...
            'NOTIFY_HEADER_END_ADVANCED_SEARCH_RESULTS',
            'NOTIFY_HEADER_START_PRODUCT_INFO',
            // Numinix product-type templates (services, downloads, docs, music)
            // fire typed notifiers instead of NOTIFY_HEADER_START_PRODUCT_INFO.
            // Without these, SERP→PDP click mirroring never runs on
            // www.numinix.ca SEO product URLs (body=serviceproductinfoBody).
            'NOTIFY_HEADER_START_SERVICE_PRODUCT_INFO',
            'NOTIFY_HEADER_START_DOWNLOAD_PRODUCT_INFO',
    ...
    public function update(&$class, $eventID, &$param1, &$param2 = null, &$param3 = null)
    {
        try {
            switch ($eventID) {
                ...
                case 'NOTIFY_HEADER_START_SERVICE_PRODUCT_INFO':
                case 'NOTIFY_HEADER_START_DOWNLOAD_PRODUCT_INFO':
                case 'NOTIFY_HEADER_START_DOCUMENT_PRODUCT_INFO':
                case 'NOTIFY_HEADER_START_PRODUCT_MUSIC_INFO':
                case 'NOTIFY_HEADER_START_PRODUCT_FREE_SHIPPING_INFO':
                    $this->onProductInfo();
                    break;
    ...
    /**
     * Ensure $categories_filter_title exists for SERP templates that
     * echo it without isset() (Winchester Responsive, PHP 8+).
     */
    private function ensureCategoriesFilterTitle(): void
    {
        if (array_key_exists('categories_filter_title', $GLOBALS)
            && is_string($GLOBALS['categories_filter_title'])
        ) {
            return;
        }
        $title = '';
        $cid = isset($_GET['categories_id']) ? (int) $_GET['categories_id'] : 0;
        if ($cid > 0 && function_exists('zen_get_category_name')) {
            $langId = (int) ($_SESSION['languages_id'] ?? 1);
            try {
                $name = zen_get_category_name($cid, $langId);
                if (is_string($name) && $name !== '') {
                    $title = $name;
                }
            } catch (\Throwable $e) {
                // leave empty — warning avoidance only
            }
        }
        $GLOBALS['categories_filter_title'] = $title;
    }
    ...
    {
        // Winchester Responsive (and similar custom themes) echo
        // $categories_filter_title in
        // tpl_modules_advanced_search_result_listing_display_order.php
        // without an isset() guard. Stock Zen Cart often leaves the
        // variable unset on keyword SERPs; PHP 8+ then floods the
        // error log. Define it early (before any early-return) so the
        // template always has a string — use the category name when
        // categories_id is present (NS-26042 Cannapot).
        $this->ensureCategoriesFilterTitle();
    ...
    private function onSearchResults(&$listingSql, $keywords, &$result): void
    {
        ...
        $params = [
            ...
            // Missing pfrom/pto must stay unbound. Defaulting to 0 made
            // every unfiltered SERP `price:>=0 && price:<=0` (STRIN: 3
            // free "guitar" hits vs suggest meta.total ~10k).
            'pfrom'                  => function_exists('numinix_seekmodo_parse_price_bound')
                ? numinix_seekmodo_parse_price_bound($_GET['pfrom'] ?? null)
                : null,
            'pto'                    => function_exists('numinix_seekmodo_parse_price_bound')
                ? numinix_seekmodo_parse_price_bound($_GET['pto'] ?? null)
                : null,
        ];
        if (($_GET['seekmodo_skip_category_redirect'] ?? '') === '1') {
        ...
        // Gateway path only when paired / not sticky-unpaid. Enhanced
        // Native must still rewrite the SERP when:
        //   - unpaired / mode=off / missing credentials
        //     (`numinix_seekmodo_enabled()` false)
        //   - sticky trial_expired / over_quota / cancelled
        //     (`Client::shouldPreferLocalSuggest()`)
        //   - gateway null (shadow / breaker / 402 / transport)
        // Typeahead already had this split (v1.1.6 + v1.3.51); SERP
        // wrongly early-returned on !enabled and left Zen Cart's
        // native LIKE + default name sort.
        $envelope = null;
        $attemptGateway = function_exists('numinix_seekmodo_should_attempt_gateway_search')
            ? numinix_seekmodo_should_attempt_gateway_search()
            : (
                function_exists('numinix_seekmodo_enabled')
                && numinix_seekmodo_enabled()
            );
        if ($attemptGateway && function_exists('numinix_seekmodo_run_search')) {
            $envelope = numinix_seekmodo_run_search($params);
        }
        // Null / empty / unpaid sticky / unpaired — Enhanced Native.
        // Prefer WHERE-based listing SQL (no ID-list cap) so SERP totals
        // and pagination match the full SQL match set.
        if (!is_array($envelope) || empty($envelope['products']) || !is_array($envelope['products'])) {
            if (
                ...
                if ($enSql !== null && $enTotal > 0) {
                    ...
                    // Listing SQL already demotes qty=0 (see EN order SQL);
                    // do not re-partition here or beacon ranks diverge from
                    // the rendered listing. Gateway ID-list path still runs
                    // live stock (zen_get_products_stock) before rewrite.
            ...
            // Legacy fallback: id-list rewrite (older builds / unexpected nulls).
            if (function_exists('numinix_seekmodo_run_enhanced_native_search')) {
     ...
     * is all of v1.5.x and v2.x). We update them in place so the SERP
     * header templates that read `$result->number_of_rows` stay in sync.
     *
     * IMPORTANT: product_listing.php (stock + most custom themes)
     * rebuilds a *second* splitPageResults from the global
     * `$listing_sql` string, not from `$result`. NOTIFY_SEARCH_RESULTS
     * passes `$listing_sql` as notifier param1 (by-value), so assigning
    ...
    private function emitSerpClickBeacon(): void
    {
        ...
        echo '<script>(function(){'
            ...
            . 'if(!href)return null;'
            . 'try{href=String(href);}catch(e){return null;}'
            // Category-only URLs: -c-N slugs (Klevu / some SEO packs).
            // Do NOT reject every href that merely *contains* cPath —
            // Numinix (and other zen_href_link listings) append
            // ?cPath=... as a breadcrumb on *product* links like
            // /tableau-for-woocommerce-1172?cPath=403_407_462, which
            // v1.3.59's blanket cPath check dropped so SERP beacons
            // never fired (jlew-00e070bd tracking_clicks_missing).
            . 'if(/-c-\\d+/i.test(href))return null;'
    ...
    /**
     * Inject a "Relevance" option into SERP sort menus and label the
     * control when Seekmodo ranking is active.
     *
     * Themes (e.g. Cannapot) ship Artikelname / price / date menus with
     * no Relevance choice. Zen Cart also injects the store default
     * name sort into $_GET, so shoppers never see that results are
     * relevance-ranked. This lightweight DOM patch prepends a
     * sort=relevance link next to existing sort=Xd anchors / options.
     */
    private function emitSerpRelevanceSortUi(): void
    {
        static $emitted = false;
        if ($emitted) {
            return;
        }
        // Show Relevance/Relevanz on gateway OR Enhanced Native SERPs.
        $cloudOrEn = (
            function_exists('numinix_seekmodo_enabled')
            && numinix_seekmodo_enabled()
        ) || (
            function_exists('numinix_seekmodo_enhanced_native_enabled')
            && numinix_seekmodo_enhanced_native_enabled()
        );
        if (!$cloudOrEn) {
            return;
        }
        if (!$this->isSearchResultsPage($this->currentMainPage())) {
            return;
        }
        if (!function_exists('_numinix_seekmodo_is_relevance_sort_value')
            || !function_exists('_numinix_seekmodo_query_sort_value')
        ) {
            return;
        }
        $activeRelevance = _numinix_seekmodo_is_relevance_sort_value(
            _numinix_seekmodo_query_sort_value()
        );
        $label = defined('TEXT_SEEKMODO_SORT_RELEVANCE')
            ? (string) TEXT_SEEKMODO_SORT_RELEVANCE
            : 'Relevance';
        $labelJson = json_encode($label, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        $activeJson = $activeRelevance ? 'true' : 'false';
        $emitted = true;
        echo '<script>(function(){'
            . 'var LABEL=' . $labelJson . ',ACTIVE=' . $activeJson . ';'
            . 'function withSort(href,sortVal){'
            . 'try{var u=new URL(href,window.location.href);'
            . 'u.searchParams.set("sort",sortVal);u.searchParams.delete("page");'
            . 'return u.pathname+u.search+u.hash;}catch(e){return href;}'
            . '}'
            . 'function ensureAnchor(menu){'
            . 'if(!menu||menu.querySelector("a[data-seekmodo-relevance]"))return;'
            . 'var sample=menu.querySelector("a[href*=\\"sort=\\"]");if(!sample)return;'
            . 'var li=document.createElement("li");'
            . 'var a=document.createElement("a");'
            . 'a.setAttribute("data-seekmodo-relevance","1");'
            . 'a.href=withSort(sample.href,"relevance");'
            . 'a.textContent=LABEL;'
            . 'li.appendChild(a);'
            . 'menu.insertBefore(li,menu.firstChild);'
            . '}'
            . 'function ensureSelect(sel){'
            . 'if(!sel||sel.querySelector("option[data-seekmodo-relevance]"))return;'
            . 'var opt=document.createElement("option");'
            . 'opt.setAttribute("data-seekmodo-relevance","1");'
            . 'opt.value="relevance";opt.textContent=LABEL;'
            . 'if(ACTIVE)opt.selected=true;'
            . 'sel.insertBefore(opt,sel.firstChild);'
            . '}'
            . 'document.querySelectorAll("ul").forEach(function(ul){'
            . 'if(ul.querySelector("a[href*=\\"sort=2a\\"],a[href*=\\"sort=2d\\"],a[href*=\\"sort=3a\\"]")){'
            . 'ensureAnchor(ul);'
            . '}'
            . '});'
            . 'document.querySelectorAll("select").forEach(function(sel){'
            . 'var html=sel.innerHTML||"";'
            . 'if(/sort=\\d[ad]/i.test(html)||/value=.\\d[ad]/i.test(html)){'
            . 'ensureSelect(sel);'
            . '}'
            . '});'
            . 'if(ACTIVE){'
            . 'document.querySelectorAll(".menu-toggle-btn,.menu-wrapper button").forEach(function(btn){'
            . 'var t=(btn.textContent||"").replace(/\\s+/g," ").trim();'
            . 'if(/Artikelname|Product Name|Name|Modell|Model|Preis|Price/i.test(t)'
            . '&& btn.parentElement&&btn.parentElement.querySelector("a[href*=\\"sort=\\"]")){'
    ...
    private function onPageFooter(): void
    {
        ...
        if ($page === 'product_info') {
            ...
            $this->emitPlacements([
                ['key' => 'pdp-cascade', 'doc_id' => $productsId, 'limit' => 8],
            ]);
            return;
        }
        if ($page === 'index') {
    ...
    private function onShoppingCartHeader(): void
    {
        ...
        // Cart cascade: pass all line ids as excludes and up to three
        // anchors (most-recent-first when PHP preserves insert order).
        $ids = [];
    ...
    private function emitPlacements(array $rows): void
    {
        ...
        foreach ($rows as $row) {
            ...
            if (!empty($row['doc_ids']) && is_array($row['doc_ids'])) {
                $ids = array_map('intval', $row['doc_ids']);
                $ids = array_values(array_filter($ids, static fn ($id) => $id > 0));
                if ($ids !== []) {
                    $attrs[] = 'data-seekmodo-doc-ids="' . htmlspecialchars(implode(',', $ids), ENT_QUOTES, 'UTF-8') . '"';
                }
            }
            if (!empty($row['exclude_doc_ids']) && is_array($row['exclude_doc_ids'])) {
                $ex = array_map('intval', $row['exclude_doc_ids']);
                $ex = array_values(array_filter($ex, static fn ($id) => $id > 0));
                if ($ex !== []) {
                    $attrs[] = 'data-seekmodo-exclude-doc-ids="' . htmlspecialchars(implode(',', $ex), ENT_QUOTES, 'UTF-8') . '"';
                }
            }
            if (!empty($row['limit'])) {
    ...
    /** @var bool */
    private $recoLabelsEmitted = false;

    /**
     * Emit storefront recommendation headings once per request so
     * jscript_seekmodo_recommendations.js can localize section titles.
     */
    private function emitRecoLabelsOnce(): void
    {
        if ($this->recoLabelsEmitted) {
            return;
        }
        $this->recoLabelsEmitted = true;
        $labels = $this->recoLabels();
        $json = json_encode($labels, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        if (!is_string($json) || $json === '') {
            return;
        }
        echo '<script>window.SeekmodoRecoLabels=' . $json . ';</script>' . "\n";
    }

    /**
     * @return array<string, string>
     */
    private function recoLabels(): array
    {
        $defaults = [
        ...
        $map = [
            'pdp-related'       => 'TEXT_SEEKMODO_RECO_PDP_RELATED',
            'pdp-also-bought'   => 'TEXT_SEEKMODO_RECO_PDP_ALSO_BOUGHT',
            'pdp-also-viewed'   => 'TEXT_SEEKMODO_RECO_PDP_ALSO_VIEWED',
            'pdp-bundle'        => 'TEXT_SEEKMODO_RECO_PDP_BUNDLE',
            'cart'              => 'TEXT_SEEKMODO_RECO_CART',
            'cart_below'        => 'TEXT_SEEKMODO_RECO_CART',
            'cart-also-bought'  => 'TEXT_SEEKMODO_RECO_CART',
            'cart-bundle'       => 'TEXT_SEEKMODO_RECO_CART_BUNDLE',
            'home-trending'     => 'TEXT_SEEKMODO_RECO_HOME_TRENDING',
            'category-trending' => 'TEXT_SEEKMODO_RECO_CATEGORY_TRENDING',
            'bought'            => 'TEXT_SEEKMODO_RECO_PDP_ALSO_BOUGHT',
            'related'           => 'TEXT_SEEKMODO_RECO_PDP_RELATED',
            'popular'           => 'TEXT_SEEKMODO_RECO_POPULAR',
        ];
        $out = [];
