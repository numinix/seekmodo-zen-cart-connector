...
if (!function_exists('numinix_seekmodo_enhanced_native_order_sql')) {
     ...
     *
     * In-stock (qty > 0) always leads OOS — matches gateway
     * demote_all_oos / live-stock partition so unpaid / EN fallback
     * SERPs do not surface sold-out SKUs first.
     *
    ...
    function numinix_seekmodo_enhanced_native_order_sql(): string
    {
        ...
        // Qty proxy for SQL (no per-row zen_get_products_stock). Live
        // stock partition on the ID-list path still uses the storefront
        // stock helper for attribute / NPF accuracy.
        $parts = ['(CASE WHEN p.products_quantity > 0 THEN 1 ELSE 0 END) DESC', 'p.products_ordered DESC'];
        global $db;
...
if (!function_exists('numinix_seekmodo_enhanced_native_listing_order_sql')) {
     ...
     *
     * Relevance / popularity sorts demote OOS first. Explicit price /
     * name / newest / catalog-order sorts keep the shopper's chosen key
     * as primary so "price low→high" is not rewritten.
     */
    function numinix_seekmodo_enhanced_native_listing_order_sql(): string
    {
        $inStockFirst = '(CASE WHEN p.products_quantity > 0 THEN 1 ELSE 0 END) DESC';
        if (isset($_GET['sortby']) && (string) $_GET['sortby'] !== '') {
            switch ((int) $_GET['sortby']) {
                ...
                case 2:
                    return ' ORDER BY ' . $inStockFirst . ', p.products_ordered DESC, pd.products_name ASC';
                case 3:
    ...
    function numinix_seekmodo_run_typeahead_local(string $q, int $max = 8): ?array
    {
        ...
        foreach ($search['product_ids'] as $pid) {
            ...
            if (function_exists('numinix_seekmodo_plain_display_price')) {
                $item['price'] = numinix_seekmodo_plain_display_price((int) $pid);
            }
            if (function_exists('_numinix_seekmodo_typeahead_attach_image_url')) {
                _numinix_seekmodo_typeahead_attach_image_url($item, (int) $pid, null);
            } elseif (function_exists('numinix_seekmodo_suggest_product_image_url')) {
                $imageUrl = numinix_seekmodo_suggest_product_image_url((int) $pid);
                if ($imageUrl !== '') {
