...
if (!function_exists('numinix_seekmodo_parse_price_bound')) {
    /**
     * Parse a storefront pfrom/pto query value.
     *
     * Missing / blank / non-numeric means "no bound". Integer 0 is kept
     * (the first suggest price band starts at $0). Do not coerce a
     * missing value to 0 — `price_filter_by(0, 0)` emits `price:<=0`
     * and collapses the SERP to free products (STRIN guitar vs 10k).
     *
     * @param mixed $raw
     */
    function numinix_seekmodo_parse_price_bound($raw): ?float
    {
...
if (!function_exists('numinix_seekmodo_price_filter_by')) {
     ...
     * pfrom=0 is a valid lower bound (first price band) when the
     * shopper actually sent pfrom=0. A missing param must be null, not 0.
     *
     * @param mixed $pfrom
     * @param mixed $pto
     */
...
if (!function_exists('_numinix_seekmodo_is_unbound_category_filter')) {
    /**
     * Zen Cart advanced search uses categories_id=0 (and sometimes cPath=0)
     * to mean "all categories". That must not become Typesense
     * category_id:=0 — no products live in category 0, so /v1/search
     * returns found=0 and the SERP falls back to Enhanced Native while
     * suggest (no category param) still shows relevance-ranked hits.
     */
    function _numinix_seekmodo_is_unbound_category_filter(string $urlParam, $raw): bool
    {
        if ($urlParam !== 'categories_id' && $urlParam !== 'cPath') {
            return false;
        }
        if ($raw === null || $raw === '') {
            return true;
        ...
        // cPath="0" only; real paths like "7_12" stay bound.
        return false;
    }
}
if (!function_exists('numinix_seekmodo_build_filter_by')) {
    ...
    function numinix_seekmodo_build_filter_by(): ?string
    {
        ...
        foreach (_numinix_seekmodo_filter_registry() as $urlParam => $spec) {
            ...
            if (_numinix_seekmodo_is_unbound_category_filter($urlParam, $_GET[$urlParam])) {
                continue;
            }
...
if (!function_exists('numinix_seekmodo_run_search')) {
    ...
    function numinix_seekmodo_run_search(array $params): ?array
    {
        ...
        //   - `seekmodo_nocache=1` skips the cache READ so operators see
        //     a fresh gateway response, but still WRITE-THROUGH a
        //     successful envelope. Otherwise a debug refresh wastes the
        //     round-trip and leaves View All / SERP on a stale TTL entry.
        $cacheTtlS = 300;
        ...
        if ($cacheEnabled) {
            ...
            $keyParts = [
                ...
                // Unbound vs explicit $0 must not share a cache entry.
                // v2 hashed missing pfrom/pto as 0.0, so a 300s $0 clamp
                // could be replayed on unfiltered SERPs.
                'pfr'  => function_exists('numinix_seekmodo_parse_price_bound')
                    ? (numinix_seekmodo_parse_price_bound($params['pfrom'] ?? null) ?? '')
                    : (isset($params['pfrom']) && is_numeric($params['pfrom'])
                        ? (float) $params['pfrom'] : ''),
                'pto'  => function_exists('numinix_seekmodo_parse_price_bound')
                    ? (numinix_seekmodo_parse_price_bound($params['pto'] ?? null) ?? '')
                    : (isset($params['pto']) && is_numeric($params['pto'])
                        ? (float) $params['pto'] : ''),
                'lang' => isset($_SESSION['languages_id']) ? (int)$_SESSION['languages_id'] : 1,
            ];
            $cacheKey = 'sm_search_v4:' . $tenant . ':' . sha1(json_encode($keyParts, JSON_UNESCAPED_UNICODE));
            if ($useCacheRead) {
            ...
            $GLOBALS['_numinix_seekmodo_last_search_cache'] = 'disabled';
        }
        ...
        // Empty products[] means Enhanced Native will take over; caching
        // that envelope under the unbound cat=0 key poisoned later SERPs
        // that share the same key (STRIN categories_id=0).
        if ($cacheEnabled && $cacheKey !== null && is_array($normalized)
            && !empty($normalized['products']) && is_array($normalized['products'])
        ) {
            $metaSlim = [];
            ...
            if ($cacheBypass) {
                $GLOBALS['_numinix_seekmodo_last_search_cache'] = 'bypass-write';
            }
        }
...
if (!function_exists('_numinix_seekmodo_remember_search_event')) {
    ...
    function _numinix_seekmodo_remember_search_event(array $row): void
    {
        ...
        if (isset($row['search_event_id'])
            && (int) $row['search_event_id'] > 0
            && function_exists('numinix_seekmodo_ltr_remember_search_event_id')
        ) {
            numinix_seekmodo_ltr_remember_search_event_id((int) $row['search_event_id']);
        }
    }
}
...
if (!function_exists('_numinix_seekmodo_build_serp_passthrough')) {
    ...
    {
        $payload = _numinix_seekmodo_typesense_tuning_params(true);
        // Gateway Suggest does not apply top-level `lang` as a Typesense
        // filter; serp_passthrough.filter_by is allowlisted (WP parity).
        // Without this, multilingual storefronts get mixed-locale rows and
        // client i18n_filter can empty the dropdown (#615048).
        if (function_exists('numinix_seekmodo_storefront_is_multilingual')
            && numinix_seekmodo_storefront_is_multilingual()
            && function_exists('numinix_seekmodo_current_language_code')) {
            $slug = numinix_seekmodo_current_language_code();
            if ($slug !== null && $slug !== ''
                && preg_match('/^[a-z0-9_-]+$/i', $slug) === 1) {
                $payload['filter_by'] = 'lang:=' . $slug;
            }
        }
...
if (!function_exists('_numinix_seekmodo_build_search_payload')) {
    ...
    function _numinix_seekmodo_build_search_payload(array $params): array
    {
        ...
        // v1.3.44 — exact model/sku filter for hyphenated part tokens
        // (AKS looksLikeExactPartToken parity, including all-digit
        // multi-hyphen like 4-6340-20). Without this, BM25 +
        // drop_tokens can dump hundreds of irrelevant hits when no
        // token matches. Missing SKU → zero results, not a popularity dump.
        $payload = _numinix_seekmodo_apply_exact_sku_filter($payload, $keyword);
        ...
        // v1.3.20 — view-all SERP (`seekmodo_skip_category_redirect=1`) must
        // tell the gateway to skip keyword-redirect empty-hit short-circuits.
        // Without this, gateway returns found=0 for redirect terms (e.g. KIP
        // `pint`) and the observer falls back to Enhanced Native search, which
        // mismatches suggest meta.total + ranking.
        if (
            !empty($params['skip_merchandising_redirect'])
            || (
                isset($_GET['seekmodo_skip_category_redirect'])
                && (string) $_GET['seekmodo_skip_category_redirect'] === '1'
            )
        ) {
            $payload['skip_merchandising_redirect'] = true;
        }
...
if (!function_exists('_numinix_seekmodo_typesense_string_filter_values')) {
    /**
     * Quote string literals for a Typesense `field:=` / `field:=[...]`
     * filter clause (backticks; escape `\` and `` ` ``).
     *
     * @param list<string> $values
     */
    function _numinix_seekmodo_typesense_string_filter_values(array $values): string
    {
        $quoted = [];
...
if (!function_exists('_numinix_seekmodo_apply_products_id_lookup')) {
    /**
     * Route bare numeric queries to an exact Typesense filter.
     *
    ...
    function _numinix_seekmodo_apply_products_id_lookup(array $payload, string $keyword): array
    {
        ...
        $modelTokens = array_map(static fn (int $id): string => (string) $id, $ids);
        $stringVals = _numinix_seekmodo_typesense_string_filter_values($modelTokens);
        $modelClause = 'model:=' . $stringVals;
        $skuClause = 'sku:=' . $stringVals;
        $clause = '(' . $idClause . ' || ' . $modelClause . ' || ' . $skuClause . ')';
...
if (!function_exists('_numinix_seekmodo_looks_like_exact_part_token')) {
    /**
     * v1.3.44 — AKS `EzNumberBooster::looksLikeExactPartToken` parity.
     *
     * True when the query is a complete hyphenated part / SKU token
     * worth pinning via exact `model` / `sku` filter (Zen Cart has no
     * `part_search_tokens` field). Letter-only multi-segment prefixes
     * (e.g. `brk-sk-for`) return false so BM25 recall is kept.
     */
    function _numinix_seekmodo_looks_like_exact_part_token(string $trimmed): bool
    {
        if (!str_contains($trimmed, '-')) {
            return false;
        }
        // CHRY-1612, HON-543, GM-12345, 4-6340
        if (preg_match('/^[A-Z0-9]+-\d{3,16}$/i', $trimmed) === 1) {
            return true;
        }
        // Digit-led OEM: 72147-TZ6-A71
        if (preg_match('/^\d{3,}[A-Z0-9]*-[A-Z0-9]{2,}(?:-[A-Z0-9]{2,})*$/i', $trimmed) === 1) {
            return true;
        }
        // All-digit multi-hyphen: 4-6340-20 (three+ numeric segments)
        if (preg_match('/^\d+(?:-\d+){2,}$/', $trimmed) === 1) {
            return true;
        }
        // Letter-led with a digit: EZ-LK99, BRK-SK-CHR-04
        return preg_match('/^[A-Z]{2,}-[A-Z0-9\-]*\d[A-Z0-9\-]*$/i', $trimmed) === 1;
    }
}
...
if (!function_exists('_numinix_seekmodo_apply_exact_sku_filter')) {
     ...
     *
     * Zen Cart indexes `products_model` into both `model` and `sku`
     * (no `part_search_tokens`). When the query looks like a full
     * hyphenated part number, pin those fields so Typesense
     * `drop_tokens` cannot dump ~250 irrelevant hits on a miss
     * (`tokens_matched=0`). Missing catalog SKU → zero results.
     *
     * Honours the same master switch + trigger regex as
     * `_numinix_seekmodo_apply_sku_boost`. Keeps the shopper keyword
     * as `q` (RED-1862 — do not rewrite to `*`).
     *
     * @param array<string,mixed> $payload
     * @return array<string,mixed>
     */
    function _numinix_seekmodo_apply_exact_sku_filter(array $payload, string $keyword): array
    {
        ...
        if (!_numinix_seekmodo_looks_like_exact_part_token($trimmed)) {
            return $payload;
        }

        $sanitized = preg_replace('/[\x00-\x1F\x7F`]/', '', $trimmed) ?? '';
        if ($sanitized === '') {
            return $payload;
        }
        ...
        $vals = _numinix_seekmodo_typesense_string_filter_values($tokens);
        $clause = '(model:=' . $vals . ' || sku:=' . $vals . ')';

        $existingFilter = isset($payload['filter_by']) ? trim((string) $payload['filter_by']) : '';
        if ($existingFilter !== ''
            && (str_contains($existingFilter, 'model:=') || str_contains($existingFilter, 'sku:='))
            && str_contains($existingFilter, $sanitized)
        ) {
        ...
        if (!isset($payload['q']) || trim((string) $payload['q']) === '') {
            $payload['q'] = $sanitized;
        }
        if (!array_key_exists('prioritize_exact_match', $payload)) {
            $payload['prioritize_exact_match'] = true;
...
if (!function_exists('_numinix_seekmodo_build_suggest_payload')) {
     ...
     *
     * Keeping this next to `_numinix_seekmodo_build_search_payload`
     * makes it obvious which knobs apply to which surface. The
     * shopper attribution fields (session_id / ua / ip / referer)
     * are stamped by {@see _numinix_seekmodo_run_typeahead} the same
     * way as full-search, so both surfaces look identical to the
     * gateway's bot-check classifier.
     *
     * `serp_passthrough` carries the same Typesense tuning as the
     * SERP so gateway SerpPreview ranks suggest products with the
     * SearchTool pipeline (parity unless the SERP is on enhanced-
     * native fallback). The browser `<seekmodo-suggest>` attribute
     * already did this; the PHP / suggest-shim path must match.
     *
    ...
    function _numinix_seekmodo_build_suggest_payload(string $q, int $limit = 8): array
    {
        ...
        if (function_exists('_numinix_seekmodo_build_serp_passthrough')) {
            $serp = _numinix_seekmodo_build_serp_passthrough();
            $payload['serp_passthrough'] = is_array($serp) ? $serp : [];
        }
        ...
        if (isset($payload['filter_by'])
            && (str_contains((string) $payload['filter_by'], 'products_id:=')
                || preg_match('/\bid:=/', (string) $payload['filter_by']) === 1
                || str_contains((string) $payload['filter_by'], 'model:=')
                || str_contains((string) $payload['filter_by'], 'sku:='))
        ) {
            $payload['complete'] = true;
        }
        // Multi-word phrases (e.g. "Oil Evacuation Drain Hose") are
        // complete queries on older gateways that only flip SerpPreview
        // when complete=true; Space presence is a cheap signal.
        if (!isset($payload['complete']) && str_contains($q, ' ')) {
            $payload['complete'] = true;
        }
        return $payload;
    }
...
if (!function_exists('_numinix_seekmodo_listing_order_sql')) {
     ...
     *
     * Seekmodo returns a relevance-ranked ID list; the storefront
     * re-sorts that IN(...) set locally when the shopper picks a
     * sort option (KIP's `sortby` dropdown or Zen Cart's `sort`
    ...
    function _numinix_seekmodo_listing_order_sql(array $productIds): string
    {
        ...
        if (!_numinix_seekmodo_is_relevance_sort_value($requested)
            ...
            switch ((int) $m[1]) {
                ...
                case 2:
                    return ' ORDER BY pd.products_name ' . $dir;
                case 3:
                    return ' ORDER BY m.manufacturers_name ' . $dir . ', pd.products_name ASC';
                case 4:
                    return ' ORDER BY p.products_quantity ' . $dir . ', pd.products_name ASC';
                case 5:
                    return ' ORDER BY p.products_weight ' . $dir . ', pd.products_name ASC';
                case 6:
                    return ' ORDER BY p.products_price_sorter ' . $dir . ', pd.products_name ASC';
            }
        }
        $idCsv = implode(',', array_map('intval', $productIds));
        return ' ORDER BY FIELD(p.products_id, ' . $idCsv . ')';
