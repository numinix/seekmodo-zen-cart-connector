 ...
 *   price                 float   optional       — selling price; when
 *                                                 DISPLAY_PRICE_WITH_TAX=true this is
 *                                                 VAT-inclusive (zen_add_tax). Active
 *                                                 specials override products_price.
...
// NS-26042 — CLI push must not keep the Zen Cart session open across
// long catalog queries. Leaving an active mysqli session write path can
// trigger "Commands out of sync" (error 2014) when PHP shuts down /
// session_write_close runs after multi-result catalog Execute loops.
...
// cache-friendly.
//
// Zen Cart's queryFactory::Execute() buffers the *entire* result
// into `$obj->result[]` (see includes/classes/db/mysql/query_factory.php).
// A single SELECT of every active product + HTML description routinely
// exhausts the host's 256MB PHP memory_limit on catalogs of a few
// thousand SKUs. Walk with keyset pagination (products_id > last)
// so each page stays within one upsert batch's worth of rows.
...
// Page size matches the upsert batch so peak memory stays ~1 batch of
// docs + one DB page (not the full catalog). Cap at 500 even when the
// operator raises --batch (gateway MAX is 1000; 500 keeps headroom for
// description HTML during enrichment).
$scanPageSize = $batchSize > 0 ? min($batchSize, 500) : 500;
if ($scanPageSize < 50) {
    $scanPageSize = 50;
}
...
while (!$stopEarly) {
    $pageSql = $baseSql
        . ' AND p.products_id > ' . (int) $lastId
        . ' ORDER BY p.products_id ASC'
        . ' LIMIT ' . (int) $scanPageSize;
    $products = $db->Execute($pageSql);
    ...
    if (function_exists('numinix_seekmodo_release_query_result')) {
        numinix_seekmodo_release_query_result($products);
    }
    unset($products, $row);
    if (function_exists('numinix_seekmodo_catalog_doc_prime_category_ids')) {
        $pageIds = [];
        foreach ($pageRows as $row) {
            $pageIds[] = (int) ($row['products_id'] ?? 0);
        }
        numinix_seekmodo_catalog_doc_prime_category_ids($pageIds);
        unset($pageIds);
    }
    $pageCount = 0;
    foreach ($pageRows as $row) {
...
if (
    ...
    // Cast AFTER /1000 — `(int) $ms / 1000` is float under PHP precedence and
    // fatals record_indexer_run()'s int $durationS parameter (PHP 7.1+).
    $durationS = max(0, (int) round((((microtime(true) * 1000) - $pushRunStartedMs) / 1000)));
    numinix_seekmodo_record_indexer_run('full_push', $durationS, (int) $docsSent);
}
