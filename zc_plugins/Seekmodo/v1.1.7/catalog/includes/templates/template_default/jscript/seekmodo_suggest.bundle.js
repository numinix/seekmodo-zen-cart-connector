var SeekmodoSuggest=(function(exports){'use strict';var y=class extends Error{status;body;tool;constructor(r,e,t,n){super(r),this.name="SeekmodoError",this.status=e,this.body=t,this.tool=n;}},T=class extends y{constructor(r,e,t){super(`Seekmodo auth failed (HTTP ${r})`,r,e,t),this.name="SeekmodoAuthError";}},z=class extends y{code;bucket;limit;used;constructor(r,e){super("Seekmodo over quota (HTTP 402)",402,r,e),this.name="SeekmodoQuotaError";let t=r??{};this.code=t.code??"over_quota",this.bucket=t.bucket,this.limit=t.limit,this.used=t.used;}},q=class extends y{constructor(r,e,t){super(`Seekmodo server error (HTTP ${r})`,r,e,t),this.name="SeekmodoServerError";}},K=class extends y{constructor(r,e,t){super(`Seekmodo request rejected (HTTP ${r})`,r,e,t),this.name="SeekmodoRequestError";}},_=class extends y{constructor(r,e){super(`Seekmodo network failure${r instanceof Error?`: ${r.message}`:""}`,0,r,e),this.name="SeekmodoNetworkError";}};function k(r,e){if(r instanceof _)return k(r.body);if(r instanceof TypeError){let t=r.message.toLowerCase();return t.includes("failed to fetch")||t.includes("networkerror")||t.includes("network request failed")||t.includes("load failed")}if(r instanceof Error){let t=r.message.toLowerCase();return t.includes("cors")||t.includes("access-control-allow-origin")||t.includes("cross-origin")}return  false}var N="https://gateway.seekmodo.com",$=8e3,U=class{config;cachedToken=null;constructor(r){if(!r.tenantId)throw new Error("Seekmodo SDK: tenantId is required");if(typeof r.getToken!="function")throw new Error("Seekmodo SDK: getToken callback is required");this.config={tenantId:r.tenantId,getToken:r.getToken,baseUrl:(r.baseUrl??N).replace(/\/+$/,""),fetch:r.fetch??globalThis.fetch.bind(globalThis),timeoutMs:r.timeoutMs??$,signal:r.signal,onError:r.onError,getRegion:r.getRegion};}clearTokenCache(){this.cachedToken=null;}async call(r,e,t={}){try{return await this.callOnce(r,e,t,!1)}catch(n){if(n instanceof T){this.clearTokenCache();try{return await this.callOnce(r,e,t,!0)}catch(s){throw this.config.onError?.(s,{tool:r}),s}}throw this.config.onError?.(n,{tool:r}),n}}async callOnce(r,e,t,n){let s=await this.resolveToken(n),o=`${this.config.baseUrl}/v1/${encodeURIComponent(r)}`,i=new AbortController,l=t.timeoutMs??this.config.timeoutMs,c=setTimeout(()=>i.abort(),l),d=()=>i.abort();this.config.signal?.addEventListener("abort",d,{once:true}),t.signal?.addEventListener("abort",d,{once:true});let m={"Content-Type":"application/json",Authorization:`Bearer ${s}`,"X-Seekmodo-Tenant":this.config.tenantId,"X-Seekmodo-SDK":"@seekmodo/sdk@0.1.0"};if(this.config.getRegion)try{let v=await this.config.getRegion();typeof v=="string"&&v.length>0&&(m["Seekmodo-Region"]=v);}catch{}let u;try{u=await this.config.fetch(o,{method:"POST",headers:m,body:JSON.stringify(e),signal:i.signal});}catch(v){throw new _(v,r)}finally{clearTimeout(c),this.config.signal?.removeEventListener("abort",d),t.signal?.removeEventListener("abort",d);}let h=await u.text(),f=h?F(h):null;if(u.status===401||u.status===403)throw new T(u.status,f,r);if(u.status===402)throw new z(f,r);if(u.status>=500)throw new q(u.status,f,r);if(!u.ok)throw new K(u.status,f,r);return f}async resolveToken(r){let e=Date.now();if(!r&&this.cachedToken&&this.cachedToken.expiresAt-1e4>e)return this.cachedToken.token;let t=await this.config.getToken();if(typeof t=="string")return this.cachedToken={token:t,expiresAt:e+6e4},t;if(t&&typeof t=="object"&&typeof t.token=="string"&&typeof t.expiresAt=="number")return this.cachedToken={token:t.token,expiresAt:t.expiresAt},t.token;throw new Error("Seekmodo SDK: getToken must return a string or { token, expiresAt }")}};function F(r){try{return JSON.parse(r)}catch{return r}}var C=class{transport;recommend;bundle;constructor(r){this.transport=new U(r),this.recommend={related:(e,t)=>this.transport.call("recommend.related",{...e},t??{}),alsoBought:(e,t)=>this.transport.call("recommend.also_bought",{...e},t??{}),alsoViewed:(e,t)=>this.transport.call("recommend.also_viewed",{...e},t??{}),trending:(e,t)=>this.transport.call("recommend.trending",{...e},t??{})},this.bundle={suggest:(e,t)=>this.transport.call("bundle.suggest",{...e},t??{})};}search(r,e){return this.transport.call("search",{...r},e??{})}suggest(r,e){return this.transport.call("suggest",{...r},e??{})}searchByImage(r,e){return this.transport.call("search.byImage",{...r},e??{})}chat(r,e){return this.transport.call("chat",{...r},e??{})}event(r,e){return this.transport.call("events",{...r},e??{})}};var w=null,p=null;function b(r){if(typeof document>"u")return null;let t=document.head?.querySelector(`meta[name="${r}"]`)?.getAttribute("content");return t&&t.length>0?t:null}function R(){return w!==null||(w=V()),w}async function V(){let r=b("seekmodo:tenant");if(!r)throw new Error('@seekmodo/web-components: <meta name="seekmodo:tenant"> is required');let e=b("seekmodo:token"),t=b("seekmodo:refresh");if(!e&&!t)throw new Error('@seekmodo/web-components: either <meta name="seekmodo:token"> or <meta name="seekmodo:refresh"> must be set');e&&(p={token:e,expiresAt:Date.now()+3e4});let n=b("seekmodo:gateway")??void 0;return new C({tenantId:r,baseUrl:n,getRegion:()=>W(),getToken:async()=>{let s=Date.now();if(p&&p.expiresAt-1e4>s)return {token:p.token,expiresAt:p.expiresAt};if(!t){if(p)return {token:p.token,expiresAt:p.expiresAt};throw new Error("seekmodo:refresh meta missing; no way to refresh token")}let o=await fetch(t,{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/json"}});if(!o.ok)throw new Error(`seekmodo:refresh route returned HTTP ${o.status}`);let i=await o.json();if(!i.token||typeof i.expires_at!="number")throw new Error("seekmodo:refresh route returned a malformed envelope");return p={token:i.token,expiresAt:i.expires_at*1e3},{token:p.token,expiresAt:p.expiresAt}}})}var Q="seekmodo_region";function j(r){if(typeof r!="string")return null;let e=r.trim().toLowerCase();return /^[a-z0-9][a-z0-9_-]{1,63}$/.test(e)?e:null}function W(){if(typeof document>"u")return null;let r=document.cookie??"";if(r.length===0)return null;let e=Q.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),t=new RegExp(`(?:^|; )${e}=([^;]+)`).exec(r);if(!t)return null;try{return j(decodeURIComponent(t[1]))}catch{return null}}var E=class extends HTMLElement{root;rafId=null;constructor(){super(),this.root=this.attachShadow({mode:"open"});}scheduleRender(){this.rafId===null&&(this.rafId=requestAnimationFrame(()=>{this.rafId=null;try{this.render();}catch(e){console.warn("[seekmodo] render failure",e);try{this.renderError("internal_error");}catch{this.root.innerHTML="";}}}));}async getClient(){return R()}renderError(e){this.root.innerHTML="";}disconnectedCallback(){this.rafId!==null&&(cancelAnimationFrame(this.rafId),this.rafId=null);}};function a(r,e,t){let n=document.createElement(r);if(e){for(let[s,o]of Object.entries(e))if(!(o==null||o===false))if(s==="class")n.className=String(o);else if(s==="part")n.setAttribute("part",String(o));else if(s==="text")n.textContent=String(o);else if(s==="html")n.innerHTML=String(o);else if(s==="attrs"&&typeof o=="object"&&o!==null)for(let[i,l]of Object.entries(o))n.setAttribute(i,l);else n.setAttribute(s,String(o));}return n}function H(r,e){let t=null;return (...n)=>{t!==null&&clearTimeout(t),t=setTimeout(()=>r(...n),e);}}function g(r,e,t){r.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:true,composed:true}));}var S="Search suggestions couldn't load because this site is blocked from reaching Seekmodo (CORS). Ask your store administrator to allowlist this domain on the Seekmodo gateway, or enable the connector's same-origin suggest proxy.";function D(r,e,t){let n=document.createElement("style");n.textContent=e;let s=a("div",{class:"wrap seekmodo-cors-blocked",part:"wrap cors-blocked",attrs:{role:"status"}});s.append(a("div",{class:"cors-notice",part:"cors-notice",text:t?.message??S})),r.replaceChildren(n,s);}var M="seekmodo-cors-notice";function I(r,e){if(!r||typeof document>"u")return;let t=r.closest(".search-form")??r.parentNode;if(!t)return;t.style.position=t.style.position||"relative";let n=t.querySelector(`.${M}`);if(!n){n=document.createElement("div"),n.className=M,n.setAttribute("role","status"),n.style.cssText=["position:absolute","top:100%","left:0","right:0","z-index:10050","display:none","background:#fff8e6","border:1px solid #f0c040","border-top:none","padding:8px 12px","font-size:13px","line-height:1.4","color:#5c4a00","box-shadow:0 4px 12px rgba(0,0,0,.08)"].join(";"),t.appendChild(n);let s=()=>{if(!n)return;let o=(r.value||"").trim();n.style.display=o.length>=2?"block":"none";};r.addEventListener("input",s),r.addEventListener("focus",s);}n.textContent=e??S;}function B(){typeof window>"u"||(window.seekmodoShowCorsNotice=I,window.seekmodoScriptLoadFailed=(r,e)=>{let t=r??document.querySelectorAll('input[data-seekmodo-suggest],input[data-seekmodo-typeahead],input[name="s"],input[name="keyword"],input[name="search_query"],input[name="q"],input[type="search"]');for(let n=0;n<t.length;n++){let s=t[n];s instanceof HTMLInputElement&&I(s,e);}});}var O=["recent","did_you_mean","keywords","trending","products","categories"],G={recent:"Recently searched",trending:"Trending",keywords:"Suggestions",products:"Products",categories:"Categories",did_you_mean:"Did you mean",view_all:"View all {total} results",empty:"No matches yet \u2014 keep typing.",cors_blocked:S},P=`
  :host {
