var SeekmodoSuggest=(function(exports){'use strict';var y=class extends Error{status;body;tool;constructor(n,e,t,r){super(n),this.name="SeekmodoError",this.status=e,this.body=t,this.tool=r;}},U=class extends y{constructor(n,e,t){super(`Seekmodo auth failed (HTTP ${n})`,n,e,t),this.name="SeekmodoAuthError";}},pe=class extends y{code;bucket;limit;used;constructor(n,e){super("Seekmodo over quota (HTTP 402)",402,n,e),this.name="SeekmodoQuotaError";let t=n??{};this.code=t.code??"over_quota",this.bucket=t.bucket,this.limit=t.limit,this.used=t.used;}},he=class extends y{constructor(n,e,t){super(`Seekmodo server error (HTTP ${n})`,n,e,t),this.name="SeekmodoServerError";}},me=class extends y{constructor(n,e,t){super(`Seekmodo request rejected (HTTP ${n})`,n,e,t),this.name="SeekmodoRequestError";}},K=class extends y{constructor(n,e){super(`Seekmodo network failure${n instanceof Error?`: ${n.message}`:""}`,0,n,e),this.name="SeekmodoNetworkError";}};function L(n,e){if(n instanceof K)return L(n.body);if(n instanceof TypeError){let t=n.message.toLowerCase();return t.includes("failed to fetch")||t.includes("networkerror")||t.includes("network request failed")||t.includes("load failed")}if(n instanceof Error){let t=n.message.toLowerCase();return t.includes("cors")||t.includes("access-control-allow-origin")||t.includes("cross-origin")}return  false}var ge="https://gateway.seekmodo.com",fe=8e3,be=class{config;cachedToken=null;constructor(n){if(!n.tenantId)throw new Error("Seekmodo SDK: tenantId is required");if(typeof n.getToken!="function")throw new Error("Seekmodo SDK: getToken callback is required");this.config={tenantId:n.tenantId,getToken:n.getToken,baseUrl:(n.baseUrl??ge).replace(/\/+$/,""),fetch:n.fetch??globalThis.fetch.bind(globalThis),timeoutMs:n.timeoutMs??fe,signal:n.signal,onError:n.onError,getRegion:n.getRegion};}clearTokenCache(){this.cachedToken=null;}async call(n,e,t={}){try{return await this.callOnce(n,e,t,!1)}catch(r){if(r instanceof U){this.clearTokenCache();try{return await this.callOnce(n,e,t,!0)}catch(o){throw this.config.onError?.(o,{tool:n}),o}}throw this.config.onError?.(r,{tool:n}),r}}async callOnce(n,e,t,r){let o=await this.resolveToken(r),s=`${this.config.baseUrl}/v1/${encodeURIComponent(n)}`,a=new AbortController,d=t.timeoutMs??this.config.timeoutMs,m=setTimeout(()=>a.abort(),d),c=()=>a.abort();this.config.signal?.addEventListener("abort",c,{once:true}),t.signal?.addEventListener("abort",c,{once:true});let u={"Content-Type":"application/json",Authorization:`Bearer ${o}`,"X-Seekmodo-Tenant":this.config.tenantId,"X-Seekmodo-SDK":"@seekmodo/sdk@0.1.0"};if(this.config.getRegion)try{let g=await this.config.getRegion();typeof g=="string"&&g.length>0&&(u["Seekmodo-Region"]=g);}catch{}let l;try{l=await this.config.fetch(s,{method:"POST",headers:u,body:JSON.stringify(e),signal:a.signal});}catch(g){throw new K(g,n)}finally{clearTimeout(m),this.config.signal?.removeEventListener("abort",c),t.signal?.removeEventListener("abort",c);}let p=await l.text(),h=p?ve(p):null;if(l.status===401||l.status===403)throw new U(l.status,h,n);if(l.status===402)throw new pe(h,n);if(l.status>=500)throw new he(l.status,h,n);if(!l.ok)throw new me(l.status,h,n);return h}async resolveToken(n){let e=Date.now();if(!n&&this.cachedToken&&this.cachedToken.expiresAt-1e4>e)return this.cachedToken.token;let t=await this.config.getToken();if(typeof t=="string")return this.cachedToken={token:t,expiresAt:e+6e4},t;if(t&&typeof t=="object"&&typeof t.token=="string"&&typeof t.expiresAt=="number")return this.cachedToken={token:t.token,expiresAt:t.expiresAt},t.token;throw new Error("Seekmodo SDK: getToken must return a string or { token, expiresAt }")}};function ve(n){try{return JSON.parse(n)}catch{return n}}var q=class{transport;recommend;bundle;constructor(n){this.transport=new be(n),this.recommend={related:(e,t)=>this.transport.call("recommend.related",{...e},t??{}),alsoBought:(e,t)=>this.transport.call("recommend.also_bought",{...e},t??{}),alsoViewed:(e,t)=>this.transport.call("recommend.also_viewed",{...e},t??{}),trending:(e,t)=>this.transport.call("recommend.trending",{...e},t??{})},this.bundle={suggest:(e,t)=>this.transport.call("bundle.suggest",{...e},t??{})};}search(n,e){return this.transport.call("search",{...n},e??{})}suggest(n,e){return this.transport.call("suggest",{...n},e??{})}searchByImage(n,e){return this.transport.call("search.byImage",{...n},e??{})}chat(n,e){return this.transport.call("chat",{...n},e??{})}event(n,e){return this.transport.call("events",{...n},e??{})}};var S=null,b=null;function x(n){if(typeof document>"u")return null;let t=document.head?.querySelector(`meta[name="${n}"]`)?.getAttribute("content");return t&&t.length>0?t:null}function N(){return S!==null||(S=we()),S}async function we(){let n=x("seekmodo:tenant");if(!n)throw new Error('@seekmodo/web-components: <meta name="seekmodo:tenant"> is required');let e=x("seekmodo:token"),t=x("seekmodo:refresh");if(!e&&!t)throw new Error('@seekmodo/web-components: either <meta name="seekmodo:token"> or <meta name="seekmodo:refresh"> must be set');e&&(b={token:e,expiresAt:Date.now()+3e4});let r=x("seekmodo:gateway")??void 0;return new q({tenantId:n,baseUrl:r,getRegion:()=>Ee(),getToken:async()=>{let o=Date.now();if(b&&b.expiresAt-1e4>o)return {token:b.token,expiresAt:b.expiresAt};if(!t){if(b)return {token:b.token,expiresAt:b.expiresAt};throw new Error("seekmodo:refresh meta missing; no way to refresh token")}let s=await fetch(t,{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/json"}});if(!s.ok)throw new Error(`seekmodo:refresh route returned HTTP ${s.status}`);let a=await s.json();if(!a.token||typeof a.expires_at!="number")throw new Error("seekmodo:refresh route returned a malformed envelope");return b={token:a.token,expiresAt:a.expires_at*1e3},{token:b.token,expiresAt:b.expiresAt}}})}var ye="seekmodo_region";function ke(n){if(typeof n!="string")return null;let e=n.trim().toLowerCase();return /^[a-z0-9][a-z0-9_-]{1,63}$/.test(e)?e:null}function Ee(){if(typeof document>"u")return null;let n=document.cookie??"";if(n.length===0)return null;let e=ye.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),t=new RegExp(`(?:^|; )${e}=([^;]+)`).exec(n);if(!t)return null;try{return ke(decodeURIComponent(t[1]))}catch{return null}}var R=class extends HTMLElement{root;rafId=null;constructor(){super(),this.root=this.attachShadow({mode:"open"});}scheduleRender(){this.rafId===null&&(this.rafId=requestAnimationFrame(()=>{this.rafId=null;try{this.render();}catch(e){console.warn("[seekmodo] render failure",e);try{this.renderError("internal_error");}catch{this.root.innerHTML="";}}}));}async getClient(){return N()}renderError(e){this.root.innerHTML="";}disconnectedCallback(){this.rafId!==null&&(cancelAnimationFrame(this.rafId),this.rafId=null);}};function i(n,e,t){let r=document.createElement(n);if(e){for(let[o,s]of Object.entries(e))if(!(s==null||s===false))if(o==="class")r.className=String(s);else if(o==="part")r.setAttribute("part",String(s));else if(o==="text")r.textContent=String(s);else if(o==="html")r.innerHTML=String(s);else if(o==="attrs"&&typeof s=="object"&&s!==null)for(let[a,d]of Object.entries(s))r.setAttribute(a,d);else r.setAttribute(o,String(s));}return r}function F(n,e){let t=null;return (...r)=>{t!==null&&clearTimeout(t),t=setTimeout(()=>n(...r),e);}}function w(n,e,t){n.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:true,composed:true}));}var _="Search suggestions couldn't load because this site is blocked from reaching Seekmodo (CORS). Ask your store administrator to allowlist this domain on the Seekmodo gateway, or enable the connector's same-origin suggest proxy.";function j(n,e,t){let r=document.createElement("style");r.textContent=e;let o=i("div",{class:"wrap seekmodo-cors-blocked",part:"wrap cors-blocked",attrs:{role:"status"}});o.append(i("div",{class:"cors-notice",part:"cors-notice",text:t?.message??_})),n.replaceChildren(r,o);}var V="seekmodo-cors-notice";function Q(n,e){if(!n||typeof document>"u")return;let t=n.closest(".search-form")??n.parentNode;if(!t)return;t.style.position=t.style.position||"relative";let r=t.querySelector(`.${V}`);if(!r){r=document.createElement("div"),r.className=V,r.setAttribute("role","status"),r.style.cssText=["position:absolute","top:100%","left:0","right:0","z-index:10050","display:none","background:#fff8e6","border:1px solid #f0c040","border-top:none","padding:8px 12px","font-size:13px","line-height:1.4","color:#5c4a00","box-shadow:0 4px 12px rgba(0,0,0,.08)"].join(";"),t.appendChild(r);let o=()=>{if(!r)return;let s=(n.value||"").trim();r.style.display=s.length>=2?"block":"none";};n.addEventListener("input",o),n.addEventListener("focus",o);}r.textContent=e??_;}function W(){typeof window>"u"||(window.seekmodoShowCorsNotice=Q,window.seekmodoScriptLoadFailed=(n,e)=>{let t=n??document.querySelectorAll('input[data-seekmodo-suggest],input[data-seekmodo-typeahead],input[name="s"],input[name="keyword"],input[name="search_query"],input[name="q"],input[type="search"]');for(let r=0;r<t.length;r++){let o=t[r];o instanceof HTMLInputElement&&Q(o,e);}});}var B="split-rail",Le=15;function T(n,e){let r={"split-rail":5,"command-bar":5,"cinema-grid":6,magazine:6,classic:5}[n]??5,o=Math.max(e,r*3);return Math.min(Le,o)}var Z=`
...
`,G=.15,Y=.85,Se=.28,J="seekmodo:split-rail-mobile-ratio-v2",xe="(max-width: 900px)";function Re(){return i("div",{class:"split-divider",part:"split-divider",html:'<svg class="split-divider-icon" viewBox="0 0 36 12" aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg"><rect x="6" y="1" width="24" height="2" rx="1" fill="currentColor"/><rect x="6" y="5" width="24" height="2" rx="1" fill="currentColor"/><rect x="6" y="9" width="24" height="2" rx="1" fill="currentColor"/></svg>'})}function ee(n){let e=n.querySelector(".split-divider");if(!e)return ()=>{};let t=window.matchMedia(xe),r=Se;try{let h=sessionStorage.getItem(J);if(h){let g=parseFloat(h);g>=G&&g<=Y&&(r=g);}}catch{}let o=h=>{r=Math.min(Y,Math.max(G,h)),n.style.setProperty("--split-rail-top-grow",String(r)),n.style.setProperty("--split-rail-bottom-grow",String(1-r));},s=()=>{n.style.removeProperty("--split-rail-top-grow"),n.style.removeProperty("--split-rail-bottom-grow");},a=()=>{t.matches?o(r):s();};a();let d=false,m=0,c=r,u=h=>{!t.matches||h.button!==0||(d=true,m=h.clientY,c=r,e.classList.add("is-dragging"),e.setPointerCapture(h.pointerId),h.preventDefault());},l=h=>{if(!d)return;let f=n.getBoundingClientRect().height-e.offsetHeight;f<=0||o(c+(h.clientY-m)/f);},p=h=>{if(d){d=false,e.classList.remove("is-dragging");try{e.releasePointerCapture(h.pointerId);}catch{}try{sessionStorage.setItem(J,String(r));}catch{}}};return e.setAttribute("role","separator"),e.setAttribute("aria-orientation","horizontal"),e.setAttribute("aria-label","Resize suggestion panels"),e.setAttribute("tabindex","0"),e.addEventListener("pointerdown",u),e.addEventListener("pointermove",l),e.addEventListener("pointerup",p),e.addEventListener("pointercancel",p),t.addEventListener("change",a),()=>{e.removeEventListener("pointerdown",u),e.removeEventListener("pointermove",l),e.removeEventListener("pointerup",p),e.removeEventListener("pointercancel",p),t.removeEventListener("change",a),e.classList.remove("is-dragging"),s();}}var _e="https://seekmodo.com/email-assets/seekmodo-lockup.png";function te(n){return String(n.name??n.title??"").trim()}function re(n,e,t){e.productTitleTooltip&&t&&n.setAttribute("title",t);}function ne(n,e){return n.productTitleTooltip&&e?e:""}function oe(n,e,t){let r={block:"products",data:e,value:String(e.name??e.title??""),id:e.id!==void 0?String(e.id):void 0},o=n.rows.length;return n.rows.push(r),o}function Te(n,e,t){let r=i("div",{class:"thumb-frame",part:"thumb-frame"}),o=n.image_url??n.image;return o?r.append(i("img",{class:"thumb",part:"thumb",attrs:{src:o,alt:ne(e,t),loading:"lazy",decoding:"async"}})):r.append(i("div",{class:"thumb-empty",part:"thumb thumb--empty"})),r}function Ae(n,e,t){let r=n.image_url??n.image;return r?i("img",{class:"thumb",part:"thumb",attrs:{src:r,alt:ne(e,t),loading:"lazy",decoding:"async"}}):i("div",{class:"thumb-empty",part:"thumb thumb--empty"})}function ie(n,e,t="card-price"){if(n.price===void 0||n.price===null)return null;let r=i("div",{class:t,part:"price"});return n.on_sale&&typeof n.sale_price=="number"?(r.append(i("del",{text:e.formatPrice(n.price,n.currency)})),r.append(document.createTextNode(e.formatPrice(n.sale_price,n.currency)))):r.append(document.createTextNode(e.formatPrice(n.price,n.currency))),r}function se(n,e,t){n.classList.add("row"),n.setAttribute("data-seekmodo-surface","suggest"),n.setAttribute("data-seekmodo-block","products"),n.setAttribute("data-seekmodo-pos",String(t));let r=e.rows[t];r?.id&&n.setAttribute("data-seekmodo-id",r.id),n.addEventListener("click",()=>e.onRowClick(t));}function Ce(n,e,t,r=false){let o=oe(n,e),s=te(e),a=i("button",{class:"product-card",part:"row",type:"button"});a.append(Te(e,n,s));let d=i("span",{class:"card-title",part:"name",text:s});re(d,n,s),a.append(d);let m=ie(e,n,"card-price");return m&&a.append(m),se(a,n,o),a}function Me(n,e,t,r){let o=oe(n,e),s=te(e),a=i("button",{class:"hero-card",part:"row",type:"button"});r&&a.append(i("span",{class:"hero-badge",text:r})),a.append(Ae(e,n,s));let d=i("div",{class:"hero-info"}),m=i("span",{class:"card-title",part:"name",text:s});re(m,n,s),d.append(m);let c=ie(e,n);return c&&d.append(c),a.append(d),se(a,n,o),a}function z(n){let e=n.res.meta?.total??0,t=i("div",{class:"meta-bar",part:"meta-bar"}),r=i("div");r.append(i("span",{class:"count",text:`${e} results for `})),r.append(i("span",{class:"query",text:`"${n.lastQuery}"`})),t.append(r);let o=i("a",{class:"view-all view-all-cta",part:"view-all",attrs:{href:n.viewAllHref},text:n.label("view_all").replace("{total}",String(e))});return o.addEventListener("click",s=>{s.preventDefault(),n.onViewAll();}),t.append(o),t}function He(n){let e=n.res.did_you_mean;if(!e)return null;let t=i("div",{class:"did-you-mean-bar",part:"did-you-mean"});t.append(document.createTextNode(`Showing results for "${n.lastQuery}". Search instead for `));let r=i("button",{class:"swap",type:"button",text:e}),o=n.rows.length;return n.rows.push({block:"did_you_mean",data:{value:e},value:e}),r.addEventListener("click",()=>n.onRowClick(o)),t.append(r),t.append(document.createTextNode("?")),t}function I(n,e){let t=i("div",{class:"chip-row filter-bar",part:"filter-bar"});return t.append(i("span",{class:"filter-label",text:"Category"})),e.forEach((r,o)=>{let s=i("button",{class:`chip${o===0?" active":""}`,type:"button",text:`${r.name}${typeof r.count=="number"?` ${r.count}`:""}`}),a=n.rows.length;n.rows.push({block:"categories",data:r,value:String(r.name??"")}),s.addEventListener("click",()=>n.onRowClick(a)),t.append(s);}),t}function X(n,e,t="Try"){let r=i("div",{class:"chip-row",part:"filter-bar"});return r.append(i("span",{class:"filter-label",text:t})),e.forEach(o=>{let s=i("button",{class:"chip",type:"button",text:o.keyword}),a=n.rows.length;n.rows.push({block:"keywords",data:o,value:o.keyword}),s.addEventListener("click",()=>n.onRowClick(a)),r.append(s);}),r}function P(n,e,t,r,o){let s=n.rows.length;n.rows.push({block:e,data:t,value:r});let a=i("button",{class:"row",part:"row",type:"button"});return a.append(i("div",{class:"name",part:"name",text:r})),o&&a.append(i("span",{class:"badge",part:"badge",text:o})),a.setAttribute("data-seekmodo-surface","suggest"),a.setAttribute("data-seekmodo-block",e),a.setAttribute("data-seekmodo-pos",String(s)),a.addEventListener("click",()=>n.onRowClick(s)),a}function D(n,e){if(e.length===0)return null;let t=i("div",{class:"rail-section"});return t.append(i("div",{class:"group-title",part:"group-title",text:n})),e.forEach(r=>t.append(r)),t}function ze(n){if(!n.showBranding)return null;let e=i("a",{class:"brand-footer",part:"brand-footer",attrs:{href:n.brandUrl,target:"_blank",rel:"noopener noreferrer"}});return e.append(i("span",{class:"brand-by",text:"Powered by "})),e.append(i("img",{class:"brand-logo",part:"brand-logo",attrs:{src:n.brandLogoUrl||_e,alt:"Seekmodo",height:"16"}})),e}function k(n,e,t){let r=i("div",{class:`product-grid cols-${t}`,part:"product-grid"});return e.forEach((o,s)=>r.append(Ce(n,o,s,true))),r}function ae(n,e){let t=["wrap","wide"];n==="split-rail"&&t.push("split-rail-panel"),e.splitMobileResize&&t.push("split-rail-mobile"),e.productTitleTooltip&&t.push("product-title-tooltip");let r=i("div",{class:t.join(" "),part:"wrap"});r.append(i("slot",{attrs:{name:"header"}}));let o=(e.res.products??[]).slice(0,T(n,e.limit)),s=(e.res.keywords??[]).slice(0,e.limit),a=(e.res.categories??[]).slice(0,e.limit),d=(e.res.recent??[]).slice(0,5);(e.res.trending??[]).slice(0,5);if(n==="split-rail"){r.append(z(e));let u=e.splitMobileResize?"split-body split-body--mobile-resize":"split-body",l=i("div",{class:u}),p=i("aside",{class:"rail",part:"rail"}),h=s.map(v=>P(e,"keywords",v,v.keyword)),g=D(e.label("keywords"),h);g&&p.append(g);let f=d.map(v=>P(e,"recent",v,v.keyword)),E=D(e.label("recent"),f);E&&p.append(E);let ue=a.map(v=>P(e,"categories",v,String(v.name),typeof v.count=="number"?String(v.count):void 0)),O=D(e.label("categories"),ue);O&&p.append(O),l.append(p),e.splitMobileResize&&l.append(Re());let $=i("div",{class:"canvas"});$.append(k(e,o,5)),l.append($),r.append(l);}else if(n==="cinema-grid"){let u=He(e);u&&r.append(u),r.append(z(e)),a.length&&r.append(I(e,a)),s.length&&r.append(X(e,s));let l=i("div",{class:"canvas"});l.append(k(e,o,6)),r.append(l);}else if(n==="command-bar"){let u=e.res.meta?.total??0,l=i("div",{class:"command-header",part:"meta-bar"});l.append(i("div",{class:"query-display",text:`"${e.lastQuery}"`})),l.append(i("span",{class:"result-pill",text:`${u} products`}));let p=i("a",{class:"view-all-link",part:"view-all",attrs:{href:e.viewAllHref},text:"View all \u2192"});if(p.addEventListener("click",g=>{g.preventDefault(),e.onViewAll();}),l.append(p),r.append(l),e.res.did_you_mean){let g=i("div",{class:"chip-row"});g.append(i("span",{class:"filter-label",text:"Did you mean"}));let f=i("button",{class:"chip",type:"button",text:e.res.did_you_mean}),E=e.rows.length;e.rows.push({block:"did_you_mean",data:{value:e.res.did_you_mean},value:e.res.did_you_mean}),f.addEventListener("click",()=>e.onRowClick(E)),g.append(f),r.append(g);}s.length&&r.append(X(e,s,"Related")),a.length&&r.append(I(e,a));let h=i("div",{class:"canvas"});h.append(k(e,o,5)),r.append(h);}else if(n==="magazine"){r.append(z(e)),a.length&&r.append(I(e,a));let u=i("div",{class:"canvas"}),l=o.slice(0,3),p=o.slice(3);if(l.length){u.append(i("div",{class:"group-title",part:"group-title",text:"Best matches"}));let h=i("div",{class:"hero-row",part:"hero-row"});l.forEach((g,f)=>{h.append(Me(e,g,f,f===0?"Top match":void 0));}),u.append(h);}p.length?(u.append(i("div",{class:"group-title",part:"group-title",text:"More results"})),u.append(k(e,p,6))):l.length||u.append(k(e,o,6)),r.append(u);}let c=ze(e);return c&&r.append(c),r.append(i("slot",{attrs:{name:"footer"}})),r}function A(n){return n!=="classic"&&n!==""}function le(n,e,t){try{let r=typeof window<"u"?window.location.origin:"http://localhost",o=new URL(n,r);return o.searchParams.set(e,t),/^https?:\/\//i.test(n)?o.toString():`${o.pathname}${o.search}${o.hash}`}catch{let r=n.includes("?")?"&":"?";return `${n}${r}${encodeURIComponent(e)}=${encodeURIComponent(t)}`}}var de=["recent","did_you_mean","keywords","trending","products","categories"],Ie={recent:"Recently searched",trending:"Trending",keywords:"Suggestions",products:"Products",categories:"Categories",did_you_mean:"Did you mean",view_all:"View all {total} results",empty:"No matches yet \u2014 keep typing.",cors_blocked:_},ce=`
  :host {
