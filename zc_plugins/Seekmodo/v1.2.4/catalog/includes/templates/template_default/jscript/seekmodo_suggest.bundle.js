var SeekmodoSuggest=(function(exports){'use strict';var v=class extends Error{status;body;tool;constructor(r,e,t,n){super(r),this.name="SeekmodoError",this.status=e,this.body=t,this.tool=n;}},$=class extends v{constructor(r,e,t){super(`Seekmodo auth failed (HTTP ${r})`,r,e,t),this.name="SeekmodoAuthError";}},ne=class extends v{code;bucket;limit;used;constructor(r,e){super("Seekmodo over quota (HTTP 402)",402,r,e),this.name="SeekmodoQuotaError";let t=r??{};this.code=t.code??"over_quota",this.bucket=t.bucket,this.limit=t.limit,this.used=t.used;}},oe=class extends v{constructor(r,e,t){super(`Seekmodo server error (HTTP ${r})`,r,e,t),this.name="SeekmodoServerError";}},se=class extends v{constructor(r,e,t){super(`Seekmodo request rejected (HTTP ${r})`,r,e,t),this.name="SeekmodoRequestError";}},U=class extends v{constructor(r,e){super(`Seekmodo network failure${r instanceof Error?`: ${r.message}`:""}`,0,r,e),this.name="SeekmodoNetworkError";}};function k(r,e){if(r instanceof U)return k(r.body);if(r instanceof TypeError){let t=r.message.toLowerCase();return t.includes("failed to fetch")||t.includes("networkerror")||t.includes("network request failed")||t.includes("load failed")}if(r instanceof Error){let t=r.message.toLowerCase();return t.includes("cors")||t.includes("access-control-allow-origin")||t.includes("cross-origin")}return  false}var ie="https://gateway.seekmodo.com",ae=8e3,de=class{config;cachedToken=null;constructor(r){if(!r.tenantId)throw new Error("Seekmodo SDK: tenantId is required");if(typeof r.getToken!="function")throw new Error("Seekmodo SDK: getToken callback is required");this.config={tenantId:r.tenantId,getToken:r.getToken,baseUrl:(r.baseUrl??ie).replace(/\/+$/,""),fetch:r.fetch??globalThis.fetch.bind(globalThis),timeoutMs:r.timeoutMs??ae,signal:r.signal,onError:r.onError,getRegion:r.getRegion};}clearTokenCache(){this.cachedToken=null;}async call(r,e,t={}){try{return await this.callOnce(r,e,t,!1)}catch(n){if(n instanceof $){this.clearTokenCache();try{return await this.callOnce(r,e,t,!0)}catch(o){throw this.config.onError?.(o,{tool:r}),o}}throw this.config.onError?.(n,{tool:r}),n}}async callOnce(r,e,t,n){let o=await this.resolveToken(n),i=`${this.config.baseUrl}/v1/${encodeURIComponent(r)}`,a=new AbortController,c=t.timeoutMs??this.config.timeoutMs,u=setTimeout(()=>a.abort(),c),d=()=>a.abort();this.config.signal?.addEventListener("abort",d,{once:true}),t.signal?.addEventListener("abort",d,{once:true});let l={"Content-Type":"application/json",Authorization:`Bearer ${o}`,"X-Seekmodo-Tenant":this.config.tenantId,"X-Seekmodo-SDK":"@seekmodo/sdk@0.1.0"};if(this.config.getRegion)try{let g=await this.config.getRegion();typeof g=="string"&&g.length>0&&(l["Seekmodo-Region"]=g);}catch{}let p;try{p=await this.config.fetch(i,{method:"POST",headers:l,body:JSON.stringify(e),signal:a.signal});}catch(g){throw new U(g,r)}finally{clearTimeout(u),this.config.signal?.removeEventListener("abort",d),t.signal?.removeEventListener("abort",d);}let h=await p.text(),m=h?le(h):null;if(p.status===401||p.status===403)throw new $(p.status,m,r);if(p.status===402)throw new ne(m,r);if(p.status>=500)throw new oe(p.status,m,r);if(!p.ok)throw new se(p.status,m,r);return m}async resolveToken(r){let e=Date.now();if(!r&&this.cachedToken&&this.cachedToken.expiresAt-1e4>e)return this.cachedToken.token;let t=await this.config.getToken();if(typeof t=="string")return this.cachedToken={token:t,expiresAt:e+6e4},t;if(t&&typeof t=="object"&&typeof t.token=="string"&&typeof t.expiresAt=="number")return this.cachedToken={token:t.token,expiresAt:t.expiresAt},t.token;throw new Error("Seekmodo SDK: getToken must return a string or { token, expiresAt }")}};function le(r){try{return JSON.parse(r)}catch{return r}}var q=class{transport;recommend;bundle;constructor(r){this.transport=new de(r),this.recommend={related:(e,t)=>this.transport.call("recommend.related",{...e},t??{}),alsoBought:(e,t)=>this.transport.call("recommend.also_bought",{...e},t??{}),alsoViewed:(e,t)=>this.transport.call("recommend.also_viewed",{...e},t??{}),trending:(e,t)=>this.transport.call("recommend.trending",{...e},t??{})},this.bundle={suggest:(e,t)=>this.transport.call("bundle.suggest",{...e},t??{})};}search(r,e){return this.transport.call("search",{...r},e??{})}suggest(r,e){return this.transport.call("suggest",{...r},e??{})}searchByImage(r,e){return this.transport.call("search.byImage",{...r},e??{})}chat(r,e){return this.transport.call("chat",{...r},e??{})}event(r,e){return this.transport.call("events",{...r},e??{})}};var E=null,f=null;function S(r){if(typeof document>"u")return null;let t=document.head?.querySelector(`meta[name="${r}"]`)?.getAttribute("content");return t&&t.length>0?t:null}function K(){return E!==null||(E=ce()),E}async function ce(){let r=S("seekmodo:tenant");if(!r)throw new Error('@seekmodo/web-components: <meta name="seekmodo:tenant"> is required');let e=S("seekmodo:token"),t=S("seekmodo:refresh");if(!e&&!t)throw new Error('@seekmodo/web-components: either <meta name="seekmodo:token"> or <meta name="seekmodo:refresh"> must be set');e&&(f={token:e,expiresAt:Date.now()+3e4});let n=S("seekmodo:gateway")??void 0;return new q({tenantId:r,baseUrl:n,getRegion:()=>he(),getToken:async()=>{let o=Date.now();if(f&&f.expiresAt-1e4>o)return {token:f.token,expiresAt:f.expiresAt};if(!t){if(f)return {token:f.token,expiresAt:f.expiresAt};throw new Error("seekmodo:refresh meta missing; no way to refresh token")}let i=await fetch(t,{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/json"}});if(!i.ok)throw new Error(`seekmodo:refresh route returned HTTP ${i.status}`);let a=await i.json();if(!a.token||typeof a.expires_at!="number")throw new Error("seekmodo:refresh route returned a malformed envelope");return f={token:a.token,expiresAt:a.expires_at*1e3},{token:f.token,expiresAt:f.expiresAt}}})}var ue="seekmodo_region";function pe(r){if(typeof r!="string")return null;let e=r.trim().toLowerCase();return /^[a-z0-9][a-z0-9_-]{1,63}$/.test(e)?e:null}function he(){if(typeof document>"u")return null;let r=document.cookie??"";if(r.length===0)return null;let e=ue.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),t=new RegExp(`(?:^|; )${e}=([^;]+)`).exec(r);if(!t)return null;try{return pe(decodeURIComponent(t[1]))}catch{return null}}var L=class extends HTMLElement{root;rafId=null;constructor(){super(),this.root=this.attachShadow({mode:"open"});}scheduleRender(){this.rafId===null&&(this.rafId=requestAnimationFrame(()=>{this.rafId=null;try{this.render();}catch(e){console.warn("[seekmodo] render failure",e);try{this.renderError("internal_error");}catch{this.root.innerHTML="";}}}));}async getClient(){return K()}renderError(e){this.root.innerHTML="";}disconnectedCallback(){this.rafId!==null&&(cancelAnimationFrame(this.rafId),this.rafId=null);}};function s(r,e,t){let n=document.createElement(r);if(e){for(let[o,i]of Object.entries(e))if(!(i==null||i===false))if(o==="class")n.className=String(i);else if(o==="part")n.setAttribute("part",String(i));else if(o==="text")n.textContent=String(i);else if(o==="html")n.innerHTML=String(i);else if(o==="attrs"&&typeof i=="object"&&i!==null)for(let[a,c]of Object.entries(i))n.setAttribute(a,c);else n.setAttribute(o,String(i));}return n}function N(r,e){let t=null;return (...n)=>{t!==null&&clearTimeout(t),t=setTimeout(()=>r(...n),e);}}function w(r,e,t){r.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:true,composed:true}));}var x="Search suggestions couldn't load because this site is blocked from reaching Seekmodo (CORS). Ask your store administrator to allowlist this domain on the Seekmodo gateway, or enable the connector's same-origin suggest proxy.";function Q(r,e,t){let n=document.createElement("style");n.textContent=e;let o=s("div",{class:"wrap seekmodo-cors-blocked",part:"wrap cors-blocked",attrs:{role:"status"}});o.append(s("div",{class:"cors-notice",part:"cors-notice",text:t?.message??x})),r.replaceChildren(n,o);}var V="seekmodo-cors-notice";function F(r,e){if(!r||typeof document>"u")return;let t=r.closest(".search-form")??r.parentNode;if(!t)return;t.style.position=t.style.position||"relative";let n=t.querySelector(`.${V}`);if(!n){n=document.createElement("div"),n.className=V,n.setAttribute("role","status"),n.style.cssText=["position:absolute","top:100%","left:0","right:0","z-index:10050","display:none","background:#fff8e6","border:1px solid #f0c040","border-top:none","padding:8px 12px","font-size:13px","line-height:1.4","color:#5c4a00","box-shadow:0 4px 12px rgba(0,0,0,.08)"].join(";"),t.appendChild(n);let o=()=>{if(!n)return;let i=(r.value||"").trim();n.style.display=i.length>=2?"block":"none";};r.addEventListener("input",o),r.addEventListener("focus",o);}n.textContent=e??x;}function W(){typeof window>"u"||(window.seekmodoShowCorsNotice=F,window.seekmodoScriptLoadFailed=(r,e)=>{let t=r??document.querySelectorAll('input[data-seekmodo-suggest],input[data-seekmodo-typeahead],input[name="s"],input[name="keyword"],input[name="search_query"],input[name="q"],input[type="search"]');for(let n=0;n<t.length;n++){let o=t[n];o instanceof HTMLInputElement&&F(o,e);}});}var z="split-rail",me=15;function _(r,e){let n={"split-rail":5,"command-bar":5,"cinema-grid":6,magazine:6,classic:5}[r]??5,o=Math.max(e,n*3);return Math.min(me,o)}var G=`
  .wrap.wide { padding: 0; overflow-x: hidden; overflow-y: auto; }
  .wrap.wide:has(.split-body) {
  ...
  .wrap.wide:has(.split-body) .split-body {
