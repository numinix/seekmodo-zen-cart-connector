...
final class Pairing
{
        ...
        if ($php === '') {
            return [
                ...
                'err' => 'no php binary found in $PATH (host PHP '
                    . PHP_VERSION
                    . ' — define NUMINIX_SEEKMODO_PHP_BINARY to the CLI path, e.g. /opt/cpanel/ea-php83/root/usr/bin/php)',
                'cmd' => '',
            ];
        }
     ...
     *
     * Order:
     *   1. Explicit override NUMINIX_SEEKMODO_PHP_BINARY (configure.php
     *      or extra_configures) — used when hosts hide CLI from PATH.
     *   2. `command -v php` / `which php` when shell_exec works.
     *   3. Derive CLI from PHP_BINARY (php-fpm → php sibling).
     *   4. Version-matched EasyApache / CloudLinux paths for the
     *      running major.minor (Cannapot / ZC 1.5.7 often run ea-php83
     *      while older resolver candidates stopped at ea-php82).
     *   5. Well-known distro + multi-version fallbacks + a quick
     *      `ls` of ea-php* / alt-php* trees.
     *
    ...
    private static function resolve_php_binary(): string
    {
        if (defined('NUMINIX_SEEKMODO_PHP_BINARY')) {
            ...
            if ($override !== '' && self::is_usable_php_cli($override)) {
                return $override;
            }
        }
    ...
    /**
     * True when $path looks like a real PHP CLI (file exists; not php-fpm).
     */
    private static function is_usable_php_cli(string $path): bool
    {
        ...
        // Prefer executable when the host reports mode bits; some
        // open_basedir / NFS setups lie about is_executable so we
        // still accept a plain is_file match for basename "php*".
