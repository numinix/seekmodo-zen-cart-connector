...
final class EnvProbe
{
     ...
     * to HTTPS_CATALOG_SERVER (with HTTP_CATALOG_SERVER as fallback).
     * Returns `[severity, code, detail]`:
     *
    ...
    public static function lockedDomainStatus(): array
    {
        $locked = defined('NUMINIX_SEEKMODO_LOCKED_DOMAIN')
            ...
            : '';
        $catalog = '';
        if (defined('HTTPS_CATALOG_SERVER') && constant('HTTPS_CATALOG_SERVER') !== '') {
            $catalog = (string) constant('HTTPS_CATALOG_SERVER');
        } elseif (defined('HTTP_CATALOG_SERVER')) {
            $catalog = (string) constant('HTTP_CATALOG_SERVER');
        }
        $catalogHost = '';
        if ($catalog !== '') {
