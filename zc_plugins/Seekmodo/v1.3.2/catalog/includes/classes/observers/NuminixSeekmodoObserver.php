...
class NuminixSeekmodoObserver extends \base
{
        ...
        try {
            switch ($eventID) {
                ...
                case 'NOTIFY_CART_ADD_CART_END':
                    // add_cart() always passes ($products_id, $qty, …) but forks
                    // also use ?pid=, multi-add POST arrays, and AJAX paths where
                    // superglobals lack a bare products_id — resolve every shape.
                    $this->onCartAdd($param1, $param2);
                    break;
     ...
     */
    private function onCartAdd($param1, $param2 = null): void
    {
        if (!function_exists('numinix_seekmodo_mirror_add_to_cart')) {
            return;
        }
        $productsId = $this->resolveCartProductsId($param1);
        if ($productsId <= 0) {
            return;
        }
        $opts = [];
    ...
    private function emitSerpClickBeacon(): void
    {
        ...
        echo '<script>(function(){'
            ...
            . 'var surface=pos>0?"results":"competitor_serp_js";'
