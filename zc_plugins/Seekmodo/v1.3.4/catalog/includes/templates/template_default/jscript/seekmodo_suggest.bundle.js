var SeekmodoSuggest=(function(exports){'use strict';var E=class extends Error{status;body;tool;constructor(n,e,t,r){super(n),this.name="SeekmodoError",this.status=e,this.body=t,this.tool=r;}},K=class extends E{constructor(n,e,t){super(`Seekmodo auth failed (HTTP ${n})`,n,e,t),this.name="SeekmodoAuthError";}},me=class extends E{code;bucket;limit;used;constructor(n,e){super("Seekmodo over quota (HTTP 402)",402,n,e),this.name="SeekmodoQuotaError";let t=n??{};this.code=t.code??"over_quota",this.bucket=t.bucket,this.limit=t.limit,this.used=t.used;}},ge=class extends E{constructor(n,e,t){super(`Seekmodo server error (HTTP ${n})`,n,e,t),this.name="SeekmodoServerError";}},fe=class extends E{constructor(n,e,t){super(`Seekmodo request rejected (HTTP ${n})`,n,e,t),this.name="SeekmodoRequestError";}},N=class extends E{constructor(n,e){super(`Seekmodo network failure${n instanceof Error?`: ${n.message}`:""}`,0,n,e),this.name="SeekmodoNetworkError";}};function x(n,e){if(n instanceof N)return x(n.body);if(n instanceof TypeError){let t=n.message.toLowerCase();return t.includes("failed to fetch")||t.includes("networkerror")||t.includes("network request failed")||t.includes("load failed")}if(n instanceof Error){let t=n.message.toLowerCase();return t.includes("cors")||t.includes("access-control-allow-origin")||t.includes("cross-origin")}return  false}var be="https://gateway.seekmodo.com",ve=8e3,we=class{config;cachedToken=null;constructor(n){if(!n.tenantId)throw new Error("Seekmodo SDK: tenantId is required");if(typeof n.getToken!="function")throw new Error("Seekmodo SDK: getToken callback is required");this.config={tenantId:n.tenantId,getToken:n.getToken,baseUrl:(n.baseUrl??be).replace(/\/+$/,""),fetch:n.fetch??globalThis.fetch.bind(globalThis),timeoutMs:n.timeoutMs??ve,signal:n.signal,onError:n.onError,getRegion:n.getRegion};}clearTokenCache(){this.cachedToken=null;}async call(n,e,t={}){try{return await this.callOnce(n,e,t,!1)}catch(r){if(r instanceof K){this.clearTokenCache();try{return await this.callOnce(n,e,t,!0)}catch(o){throw this.config.onError?.(o,{tool:n}),o}}throw this.config.onError?.(r,{tool:n}),r}}async callOnce(n,e,t,r){let o=await this.resolveToken(r),i=`${this.config.baseUrl}/v1/${encodeURIComponent(n)}`,a=new AbortController,p=t.timeoutMs??this.config.timeoutMs,m=setTimeout(()=>a.abort(),p),d=()=>a.abort();this.config.signal?.addEventListener("abort",d,{once:true}),t.signal?.addEventListener("abort",d,{once:true});let u={"Content-Type":"application/json",Authorization:`Bearer ${o}`,"X-Seekmodo-Tenant":this.config.tenantId,"X-Seekmodo-SDK":"@seekmodo/sdk@0.1.0"};if(this.config.getRegion)try{let c=await this.config.getRegion();typeof c=="string"&&c.length>0&&(u["Seekmodo-Region"]=c);}catch{}let l;try{l=await this.config.fetch(i,{method:"POST",headers:u,body:JSON.stringify(e),signal:a.signal});}catch(c){throw new N(c,n)}finally{clearTimeout(m),this.config.signal?.removeEventListener("abort",d),t.signal?.removeEventListener("abort",d);}let h=await l.text(),g=h?ye(h):null;if(l.status===401||l.status===403)throw new K(l.status,g,n);if(l.status===402)throw new me(g,n);if(l.status>=500)throw new ge(l.status,g,n);if(!l.ok)throw new fe(l.status,g,n);return g}async resolveToken(n){let e=Date.now();if(!n&&this.cachedToken&&this.cachedToken.expiresAt-1e4>e)return this.cachedToken.token;let t=await this.config.getToken();if(typeof t=="string")return this.cachedToken={token:t,expiresAt:e+6e4},t;if(t&&typeof t=="object"&&typeof t.token=="string"&&typeof t.expiresAt=="number")return this.cachedToken={token:t.token,expiresAt:t.expiresAt},t.token;throw new Error("Seekmodo SDK: getToken must return a string or { token, expiresAt }")}};function ye(n){try{return JSON.parse(n)}catch{return n}}var q=class{transport;recommend;bundle;constructor(n){this.transport=new we(n),this.recommend={related:(e,t)=>this.transport.call("recommend.related",{...e},t??{}),alsoBought:(e,t)=>this.transport.call("recommend.also_bought",{...e},t??{}),alsoViewed:(e,t)=>this.transport.call("recommend.also_viewed",{...e},t??{}),trending:(e,t)=>this.transport.call("recommend.trending",{...e},t??{})},this.bundle={suggest:(e,t)=>this.transport.call("bundle.suggest",{...e},t??{})};}search(n,e){return this.transport.call("search",{...n},e??{})}suggest(n,e){return this.transport.call("suggest",{...n},e??{})}searchByImage(n,e){return this.transport.call("search.byImage",{...n},e??{})}chat(n,e){return this.transport.call("chat",{...n},e??{})}ask(n,e){return this.transport.call("ask",{...n},e??{})}catalogGet(n,e){return this.transport.call("catalog.get",{...n},e??{})}event(n,e){return this.transport.call("events",{...n},e??{})}};var _=null,b=null;function R(n){if(typeof document>"u")return null;let t=document.head?.querySelector(`meta[name="${n}"]`)?.getAttribute("content");return t&&t.length>0?t:null}function V(){return _!==null||(_=ke()),_}async function ke(){let n=R("seekmodo:tenant");if(!n)throw new Error('@seekmodo/web-components: <meta name="seekmodo:tenant"> is required');let e=R("seekmodo:token"),t=R("seekmodo:refresh");if(!e&&!t)throw new Error('@seekmodo/web-components: either <meta name="seekmodo:token"> or <meta name="seekmodo:refresh"> must be set');e&&(b={token:e,expiresAt:Date.now()+3e4});let r=R("seekmodo:gateway")??void 0;return new q({tenantId:n,baseUrl:r,getRegion:()=>Le(),getToken:async()=>{let o=Date.now();if(b&&b.expiresAt-1e4>o)return {token:b.token,expiresAt:b.expiresAt};if(!t){if(b)return {token:b.token,expiresAt:b.expiresAt};throw new Error("seekmodo:refresh meta missing; no way to refresh token")}let i=await fetch(t,{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/json"}});if(!i.ok)throw new Error(`seekmodo:refresh route returned HTTP ${i.status}`);let a=await i.json();if(!a.token||typeof a.expires_at!="number")throw new Error("seekmodo:refresh route returned a malformed envelope");return b={token:a.token,expiresAt:a.expires_at*1e3},{token:b.token,expiresAt:b.expiresAt}}})}var Ee="seekmodo_region";function Se(n){if(typeof n!="string")return null;let e=n.trim().toLowerCase();return /^[a-z0-9][a-z0-9_-]{1,63}$/.test(e)?e:null}function Le(){if(typeof document>"u")return null;let n=document.cookie??"";if(n.length===0)return null;let e=Ee.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),t=new RegExp(`(?:^|; )${e}=([^;]+)`).exec(n);if(!t)return null;try{return Se(decodeURIComponent(t[1]))}catch{return null}}var A=class extends HTMLElement{root;rafId=null;constructor(){super(),this.root=this.attachShadow({mode:"open"});}scheduleRender(){this.rafId===null&&(this.rafId=requestAnimationFrame(()=>{this.rafId=null;try{this.render();}catch(e){console.warn("[seekmodo] render failure",e);try{this.renderError("internal_error");}catch{this.root.innerHTML="";}}}));}async getClient(){return V()}renderError(e){this.root.innerHTML="";}disconnectedCallback(){this.rafId!==null&&(cancelAnimationFrame(this.rafId),this.rafId=null);}};function s(n,e,t){let r=document.createElement(n);if(e){for(let[o,i]of Object.entries(e))if(!(i==null||i===false))if(o==="class")r.className=String(i);else if(o==="part")r.setAttribute("part",String(i));else if(o==="text")r.textContent=String(i);else if(o==="html")r.innerHTML=String(i);else if(o==="attrs"&&typeof i=="object"&&i!==null)for(let[a,p]of Object.entries(i))r.setAttribute(a,p);else r.setAttribute(o,String(i));}return r}function j(n,e){let t=null;return (...r)=>{t!==null&&clearTimeout(t),t=setTimeout(()=>n(...r),e);}}function y(n,e,t){n.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:true,composed:true}));}var T="Search suggestions couldn't load because this site is blocked from reaching Seekmodo (CORS). Ask your store administrator to allowlist this domain on the Seekmodo gateway, or enable the connector's same-origin suggest proxy.";function W(n,e,t){let r=document.createElement("style");r.textContent=e;let o=s("div",{class:"wrap seekmodo-cors-blocked",part:"wrap cors-blocked",attrs:{role:"status"}});o.append(s("div",{class:"cors-notice",part:"cors-notice",text:t?.message??T})),n.replaceChildren(r,o);}var Q="seekmodo-cors-notice";function G(n,e){if(!n||typeof document>"u")return;let t=n.closest(".search-form")??n.parentNode;if(!t)return;t.style.position=t.style.position||"relative";let r=t.querySelector(`.${Q}`);if(!r){r=document.createElement("div"),r.className=Q,r.setAttribute("role","status"),r.style.cssText=["position:absolute","top:100%","left:0","right:0","z-index:10050","display:none","background:#fff8e6","border:1px solid #f0c040","border-top:none","padding:8px 12px","font-size:13px","line-height:1.4","color:#5c4a00","box-shadow:0 4px 12px rgba(0,0,0,.08)"].join(";"),t.appendChild(r);let o=()=>{if(!r)return;let i=(n.value||"").trim();r.style.display=i.length>=2?"block":"none";};n.addEventListener("input",o),n.addEventListener("focus",o);}r.textContent=e??T;}function Y(){typeof window>"u"||(window.seekmodoShowCorsNotice=G,window.seekmodoScriptLoadFailed=(n,e)=>{let t=n??document.querySelectorAll('input[data-seekmodo-suggest],input[data-seekmodo-typeahead],input[name="s"],input[name="keyword"],input[name="search_query"],input[name="q"],input[type="search"]');for(let r=0;r<t.length;r++){let o=t[r];o instanceof HTMLInputElement&&G(o,e);}});}function J(n,e,t){try{let r=typeof window<"u"?window.location.origin:"http://localhost",o=new URL(n,r);return o.searchParams.set(e,t),/^https?:\/\//i.test(n)?o.toString():`${o.pathname}${o.search}${o.hash}`}catch{let r=n.includes("?")?"&":"?";return `${n}${r}${encodeURIComponent(e)}=${encodeURIComponent(t)}`}}function xe(n,e){let t=(n??"").toLowerCase(),r=(e??"").toLowerCase();if(r.startsWith("http"))try{r=new URL(r).pathname;}catch{r="";}return t==="page"&&/\/tools\/[^/]+\/?$/.test(r)?"Tool":t==="page"&&/\/tools\/?$/.test(r)?"Tools":t==="page"?"Page":t==="post"?"Article":""}function k(n){let e=typeof n.post_type=="string"?n.post_type.toLowerCase():"",t=typeof n.url=="string"?n.url:typeof n.permalink=="string"?n.permalink:"";return {postType:e,label:xe(e,t)}}var F="split-rail",_e=15;function C(n,e){let r={"split-rail":5,"command-bar":5,"cinema-grid":6,magazine:6,classic:5}[n]??5,o=Math.max(e,r*3);return Math.min(_e,o)}var re=`
...
`,X=.15,Z=.85,Re=.28,ee="seekmodo:split-rail-mobile-ratio-v2",Ae="(max-width: 900px)";function Te(){return s("div",{class:"split-divider",part:"split-divider",html:'<svg class="split-divider-icon" viewBox="0 0 36 12" aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg"><rect x="6" y="1" width="24" height="2" rx="1" fill="currentColor"/><rect x="6" y="5" width="24" height="2" rx="1" fill="currentColor"/><rect x="6" y="9" width="24" height="2" rx="1" fill="currentColor"/></svg>'})}function ne(n){let e=n.querySelector(".split-divider");if(!e)return ()=>{};let t=window.matchMedia(Ae),r=Re;try{let g=sessionStorage.getItem(ee);if(g){let c=parseFloat(g);c>=X&&c<=Z&&(r=c);}}catch{}let o=g=>{r=Math.min(Z,Math.max(X,g)),n.style.setProperty("--split-rail-top-grow",String(r)),n.style.setProperty("--split-rail-bottom-grow",String(1-r));},i=()=>{n.style.removeProperty("--split-rail-top-grow"),n.style.removeProperty("--split-rail-bottom-grow");},a=()=>{t.matches?o(r):i();};a();let p=false,m=0,d=r,u=g=>{!t.matches||g.button!==0||(p=true,m=g.clientY,d=r,e.classList.add("is-dragging"),e.setPointerCapture(g.pointerId),g.preventDefault());},l=g=>{if(!p)return;let f=n.getBoundingClientRect().height-e.offsetHeight;f<=0||o(d+(g.clientY-m)/f);},h=g=>{if(p){p=false,e.classList.remove("is-dragging");try{e.releasePointerCapture(g.pointerId);}catch{}try{sessionStorage.setItem(ee,String(r));}catch{}}};return e.setAttribute("role","separator"),e.setAttribute("aria-orientation","horizontal"),e.setAttribute("aria-label","Resize suggestion panels"),e.setAttribute("tabindex","0"),e.addEventListener("pointerdown",u),e.addEventListener("pointermove",l),e.addEventListener("pointerup",h),e.addEventListener("pointercancel",h),t.addEventListener("change",a),()=>{e.removeEventListener("pointerdown",u),e.removeEventListener("pointermove",l),e.removeEventListener("pointerup",h),e.removeEventListener("pointercancel",h),t.removeEventListener("change",a),e.classList.remove("is-dragging"),i();}}var Ce="https://seekmodo.com/email-assets/seekmodo-lockup.png";function oe(n){return String(n.name??n.title??"").trim()}function ie(n,e,t){e.productTitleTooltip&&t&&n.setAttribute("title",t);}function se(n,e){return n.productTitleTooltip&&e?e:""}function ae(n,e,t){let r={block:"products",data:e,value:String(e.name??e.title??""),id:e.id!==void 0?String(e.id):void 0},o=n.rows.length;return n.rows.push(r),o}function Me(n,e,t){let r=s("div",{class:"thumb-frame",part:"thumb-frame"}),{postType:o}=k(n);o&&r.setAttribute("data-post-type",o);let i=n.image_url??n.image;if(i)r.append(s("img",{class:"thumb",part:"thumb",attrs:{src:i,alt:se(e,t),loading:"lazy",decoding:"async"}}));else {let a=s("div",{class:"thumb-empty",part:"thumb thumb--empty",text:le(o)});o&&a.setAttribute("data-content-type",o),r.append(a);}return r}function le(n){return n==="page"?"P":n==="post"?"A":"\xB7"}function He(n,e,t){let{postType:r}=k(n),o=n.image_url??n.image;if(o)return s("img",{class:"thumb",part:"thumb",attrs:{src:o,alt:se(e,t),loading:"lazy",decoding:"async"}});let i=s("div",{class:"thumb-empty",part:"thumb thumb--empty",text:le(r)});return r&&i.setAttribute("data-content-type",r),i}function de(n,e,t="card-price"){if(n.price===void 0||n.price===null)return null;let r=s("div",{class:t,part:"price"});return n.on_sale&&typeof n.sale_price=="number"?(r.append(s("del",{text:e.formatPrice(n.price,n.currency)})),r.append(document.createTextNode(e.formatPrice(n.sale_price,n.currency)))):r.append(document.createTextNode(e.formatPrice(n.price,n.currency))),r}function ce(n,e,t){n.classList.add("row"),n.setAttribute("data-seekmodo-surface","suggest"),n.setAttribute("data-seekmodo-block","products"),n.setAttribute("data-seekmodo-pos",String(t));let r=e.rows[t];r?.id&&n.setAttribute("data-seekmodo-id",r.id),n.addEventListener("click",()=>e.onRowClick(t));}function Ie(n,e,t,r=false){let o=ae(n,e),i=oe(e),{postType:a,label:p}=k(e),m=s("button",{class:"product-card",part:"row",type:"button"});a&&m.setAttribute("data-post-type",a),m.append(Me(e,n,i));let d=s("span",{class:"card-title",part:"name",text:i});ie(d,n,i),m.append(d),p&&m.append(s("span",{class:"card-meta",part:"card-meta",text:p}));let u=de(e,n,"card-price");return u&&m.append(u),ce(m,n,o),m}function ze(n,e,t,r){let o=ae(n,e),i=oe(e),a=s("button",{class:"hero-card",part:"row",type:"button"});r&&a.append(s("span",{class:"hero-badge",text:r})),a.append(He(e,n,i));let p=s("div",{class:"hero-info"}),m=s("span",{class:"card-title",part:"name",text:i});ie(m,n,i),p.append(m);let d=de(e,n);return d&&p.append(d),a.append(p),ce(a,n,o),a}function D(n){let e=n.res.meta?.total??0,t=s("div",{class:"meta-bar",part:"meta-bar"}),r=s("div");r.append(s("span",{class:"count",text:`${e} results for `})),r.append(s("span",{class:"query",text:`"${n.lastQuery}"`})),t.append(r);let o=s("a",{class:"view-all view-all-cta",part:"view-all",attrs:{href:n.viewAllHref},text:n.label("view_all").replace("{total}",String(e))});return o.addEventListener("click",i=>{i.preventDefault(),n.onViewAll();}),t.append(o),t}function Pe(n){let e=n.res.did_you_mean;if(!e)return null;let t=s("div",{class:"did-you-mean-bar",part:"did-you-mean"});t.append(document.createTextNode(`Showing results for "${n.lastQuery}". Search instead for `));let r=s("button",{class:"swap",type:"button",text:e}),o=n.rows.length;return n.rows.push({block:"did_you_mean",data:{value:e},value:e}),r.addEventListener("click",()=>n.onRowClick(o)),t.append(r),t.append(document.createTextNode("?")),t}function O(n,e){let t=s("div",{class:"chip-row filter-bar",part:"filter-bar"});return t.append(s("span",{class:"filter-label",text:"Category"})),e.forEach((r,o)=>{let i=s("button",{class:`chip${o===0?" active":""}`,type:"button",text:`${r.name}${typeof r.count=="number"?` ${r.count}`:""}`}),a=n.rows.length;n.rows.push({block:"categories",data:r,value:String(r.name??"")}),i.addEventListener("click",()=>n.onRowClick(a)),t.append(i);}),t}function te(n,e,t="Try"){let r=s("div",{class:"chip-row",part:"filter-bar"});return r.append(s("span",{class:"filter-label",text:t})),e.forEach(o=>{let i=s("button",{class:"chip",type:"button",text:o.keyword}),a=n.rows.length;n.rows.push({block:"keywords",data:o,value:o.keyword}),i.addEventListener("click",()=>n.onRowClick(a)),r.append(i);}),r}function B(n,e,t,r,o){let i=n.rows.length;n.rows.push({block:e,data:t,value:r});let a=s("button",{class:"row",part:"row",type:"button"});return a.append(s("div",{class:"name",part:"name",text:r})),o&&a.append(s("span",{class:"badge",part:"badge",text:o})),a.setAttribute("data-seekmodo-surface","suggest"),a.setAttribute("data-seekmodo-block",e),a.setAttribute("data-seekmodo-pos",String(i)),a.addEventListener("click",()=>n.onRowClick(i)),a}function $(n,e){if(e.length===0)return null;let t=s("div",{class:"rail-section"});return t.append(s("div",{class:"group-title",part:"group-title",text:n})),e.forEach(r=>t.append(r)),t}function De(n){if(!n.showBranding)return null;let e=s("a",{class:"brand-footer",part:"brand-footer",attrs:{href:n.brandUrl,target:"_blank",rel:"noopener noreferrer"}});return e.append(s("span",{class:"brand-by",text:"Powered by "})),e.append(s("img",{class:"brand-logo",part:"brand-logo",attrs:{src:n.brandLogoUrl||Ce,alt:"Seekmodo",height:"16"}})),e}function S(n,e,t){let r=s("div",{class:`product-grid cols-${t}`,part:"product-grid"});return e.forEach((o,i)=>r.append(Ie(n,o,i,true))),r}function ue(n,e){let t=["wrap","wide"];n==="split-rail"&&t.push("split-rail-panel"),e.splitMobileResize&&t.push("split-rail-mobile"),e.productTitleTooltip&&t.push("product-title-tooltip");let r=s("div",{class:t.join(" "),part:"wrap"});r.append(s("slot",{attrs:{name:"header"}}));let o=(e.res.products??[]).slice(0,C(n,e.limit)),i=(e.res.keywords??[]).slice(0,e.limit),a=(e.res.categories??[]).slice(0,e.limit),p=(e.res.recent??[]).slice(0,5);(e.res.trending??[]).slice(0,5);if(n==="split-rail"){r.append(D(e));let u=e.splitMobileResize?"split-body split-body--mobile-resize":"split-body",l=s("div",{class:u}),h=s("aside",{class:"rail",part:"rail"}),g=i.map(v=>B(e,"keywords",v,v.keyword)),c=$(e.label("keywords"),g);c&&h.append(c);let f=p.map(v=>B(e,"recent",v,v.keyword)),w=$(e.label("recent"),f);w&&h.append(w);let P=a.map(v=>B(e,"categories",v,String(v.name),typeof v.count=="number"?String(v.count):void 0)),L=$(e.label("categories"),P);L&&h.append(L),l.append(h),e.splitMobileResize&&l.append(Te());let U=s("div",{class:"canvas"});U.append(S(e,o,5)),l.append(U),r.append(l);}else if(n==="cinema-grid"){let u=Pe(e);u&&r.append(u),r.append(D(e)),a.length&&r.append(O(e,a)),i.length&&r.append(te(e,i));let l=s("div",{class:"canvas"});l.append(S(e,o,6)),r.append(l);}else if(n==="command-bar"){let u=e.res.meta?.total??0,l=s("div",{class:"command-header",part:"meta-bar"});l.append(s("div",{class:"query-display",text:`"${e.lastQuery}"`})),l.append(s("span",{class:"result-pill",text:`${u} products`}));let h=s("a",{class:"view-all-link",part:"view-all",attrs:{href:e.viewAllHref},text:"View all \u2192"});if(h.addEventListener("click",c=>{c.preventDefault(),e.onViewAll();}),l.append(h),r.append(l),e.res.did_you_mean){let c=s("div",{class:"chip-row"});c.append(s("span",{class:"filter-label",text:"Did you mean"}));let f=s("button",{class:"chip",type:"button",text:e.res.did_you_mean}),w=e.rows.length;e.rows.push({block:"did_you_mean",data:{value:e.res.did_you_mean},value:e.res.did_you_mean}),f.addEventListener("click",()=>e.onRowClick(w)),c.append(f),r.append(c);}i.length&&r.append(te(e,i,"Related")),a.length&&r.append(O(e,a));let g=s("div",{class:"canvas"});g.append(S(e,o,5)),r.append(g);}else if(n==="magazine"){r.append(D(e)),a.length&&r.append(O(e,a));let u=s("div",{class:"canvas"}),l=o.slice(0,3),h=o.slice(3);if(l.length){u.append(s("div",{class:"group-title",part:"group-title",text:"Best matches"}));let g=s("div",{class:"hero-row",part:"hero-row"});l.forEach((c,f)=>{g.append(ze(e,c,f,f===0?"Top match":void 0));}),u.append(g);}h.length?(u.append(s("div",{class:"group-title",part:"group-title",text:"More results"})),u.append(S(e,h,6))):l.length||u.append(S(e,o,6)),r.append(u);}let d=De(e);return d&&r.append(d),r.append(s("slot",{attrs:{name:"footer"}})),r}function M(n){return n!=="classic"&&n!==""}var pe=["recent","did_you_mean","keywords","trending","products","categories"],Oe={recent:"Recently searched",trending:"Trending",keywords:"Suggestions",products:"Products",categories:"Categories",did_you_mean:"Did you mean",view_all:"View all {total} results",empty:"No matches yet \u2014 keep typing.",cors_blocked:T},he=`
  :host {
